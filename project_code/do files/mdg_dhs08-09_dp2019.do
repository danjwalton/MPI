...
2019 Global Multidimensional Poverty Index - Madagascar DHS 2008-09 
...
/*Madagascar DHS 2008-09: In a subsample of 50% of selected households, 
all men aged 15 to 59 were interviewed. It is also in this subsample of 
households that anthropometric measurements were taken from women aged 15-49 
years (p.7). Anthropometric information was not collected from men 15-59.

During the survey, all children under five year of age present in the surveyed 
households had to be measured. 5,436 out of 6,289 children responding to these 
criteria have accurate data on age and size, which represents 86% of the children
under 5 (p. 202). For logistical reasons, the 4th DHS survey for Madagascar 
(EDSMD-IV) could not use electronic scales with digital display for weight 
measurement. They used mechanical scales with needle. These scales did not 
produce sufficiently accurate weights for children under 5 and, after 
verification of the collected data, the data on the weight of the children has 
not been validated and therefore it is not presented in this report. Only the 
size-based indicator for children has been analysed (p. 9)
 */ 
...
use "$path_in/MDKR51FL.DTA", clear 
...
gen double ind_id = v001*100000 + v002*100 + b16 
format ind_id %20.0g
...
drop if b5==0 
    ...
    // Madagascar DHS 2008-09: 698 observations deleted 
...
replace ind_id = v001*100000 + v002*100 + (line+90) if duplicate!=0 
...
count if _fwei==1 | _flen==1  
	/* Note: In the context of MDG DHS 2008-09, 299 children are replaced as '.'
...
save "$path_out/MDG08-09_KR.dta", replace
...
use "$path_in/MDBR51FL.DTA", clear
...
gen double ind_id = v001 *100000 + v002*100 + v003
format ind_id %20.0g
label var ind_id "Individual ID"
...
compare tot_child_died tot_child_died_2
	//Madagascar DHS 2008-09: these figures are identical
...
sort ind_id
save "$path_out/MDG08-09_BR.dta", replace	
...
use "$path_in/MDIR51FL.DTA", clear
...
sort ind_id
save "$path_out/MDG08-09_IR.dta", replace
...
gen str30 datalab = "girl_nutri_mdg" 
lab var datalab "Working file"
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores */
use "$path_out/girl_nutri_mdg_z.dta", clear 
...
sort ind_id
save "$path_out/MDG08-09_IR_girls.dta", replace



	//Erase files from folder:
erase "$path_out/girl_nutri_mdg_z.xls"
erase "$path_out/girl_nutri_mdg_prev.xls"
erase "$path_out/girl_nutri_mdg_z.dta"
...
use "$path_in/MDMR51FL.DTA", clear 
...
gen double ind_id = mv001*100000 + mv002*100 + mv003 	
format ind_id %20.0g
label var ind_id "Individual ID"
...
sort ind_id
save "$path_out/MDG08-09_MR.dta", replace
...
********************************************************************************
	/*Note: In the case of Madagascar DHS 2008-09, anthropometric data was NOT 
	collected for men, hence this step is irrelevant */
...
sort ind_id
save "$path_out/MDG08-09_MR_boys.dta", replace
...
use "$path_in/MDPR51FL.DTA", clear

*** Generate a household unique key variable at the household level using: 
	***hv001=cluster number 
	***hv002=household number
gen double hh_id = hv001*1000 + hv002 
format hh_id %20.0g
...
gen double ind_id = hv001*100000 + hv002*100 + hvidx 
format ind_id %20.0g
label var ind_id "Individual ID"
codebook ind_id
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MDG08-09_BR.dta"

drop _merge

erase "$path_out/MDG08-09_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MDG08-09_IR.dta"
...
erase "$path_out/MDG08-09_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/MDG08-09_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Madagascar DHS 2008-09, 25 girls 15-19 were identified as measured 
...
erase "$path_out/MDG08-09_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MDG08-09_MR.dta"
...
tab hb13 hb65 if hv118==1 & men_MR ==., miss 
	// No anthropometric data for men in Madagascar DHS 2008-09

drop _merge

erase "$path_out/MDG08-09_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
	/*Note: Anthropometric data was not collected as part of the Madagascar DHS 
	2008-09 dataset.*/
merge 1:1 ind_id using "$path_out/MDG08-09_MR_boys.dta"

drop _merge

erase "$path_out/MDG08-09_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MDG08-09_KR.dta"
...
erase "$path_out/MDG08-09_KR.dta"
...
tab resident, miss
	...
	In the context of Madagascar DHS 2008-09, 951 (1.1%) individuals who were 
...
/*
In the context of Madagascar DHS 2008-09, height and weight measurements 
were collected from children (0-5) and women (15-49) living in 1/2 of the sampled 
households.  */
...
***********************************************
	/*NOTE: In the DHS datasets, we use this variable as a control 
	variable for the nutrition indicator if nutrition data is 
	present for children, women and men. Since anthropometric data was
	not collected for men, the variable in this section is created as 
	an empty variable*/
gen no_eligibles = .
...
//Subnational region
	/*NOTE: The sample for the Madagascar DHS 2008-09 was designed to provide
	estimates of key indicators for the country as a whole, for urban and rural 
	areas separately, and for each of the 22 regions and the capital (p. xxi). 
	The variable that dscribes these regions is "hv024". */
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Madagascar, the official school entrance age is 6 years.  
...
********************************************************************************
//Note: Madagascar DHS 2008-09 has no anthropometric data for men
...
lab var m_bmi "Male's BMI"
	//Madagascar DHS 2008-09 has no anthropometric data for men
...
*** Step 2.6 Sanitation ***
********************************************************************************
	/*Note: Madagascar DHS 2008-09: Table 2.2 (p.13) indicate that improved 
	toilet facilities are 'flush connected to piped sewer system'; 'flush to 
	spetic tank'; 'toilet connectedto a pit latrine'; 'improved ventilated 
	latrines'; 'latrines with slab'. Nothing is said on 'other'. Nothing is 
	said in the 'composting toilet but we coded as 'improved'. In table 2.2, 
	there is no difference between pit latrine with washable or not washable
	slab, they are both considered 'improved'.  */
...
save "$path_out/mdg_dhs08-09.dta", replace 
