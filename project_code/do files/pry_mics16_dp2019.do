...
2019 Global Multidimensional Poverty Index - Paraguay MICS 2016 [STATA do-file]. 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Paraguay_MICS5_Datasets
...
gen str30 datalab = "children_nutri_pry" 
...
use "$path_out/children_nutri_pry_z_rc.dta", clear 
...
sort ind_id
save "$path_out/pry16_CH.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_pry_z_rc.xls"
erase "$path_out/children_nutri_pry_prev_rc.xls"
erase "$path_out/children_nutri_pry_z_rc.dta"
...
sort ind_id
save "$path_out/pry16_BH.dta", replace	
...
sort ind_id
save "$path_out/pry16_WM.dta", replace
...
********************************************************************************
	//Note: Paraguay MICS 2016 has no male recode information.
...
save "$path_out/pry16_HH.dta", replace
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/pry16_BH.dta"
drop _merge
erase "$path_out/pry16_BH.dta" 
 
 
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/pry16_WM.dta"
tab hl7, miss 
gen temp = (hl7>0) 
...
drop temp
drop _merge
erase "$path_out/pry16_WM.dta"


*** Merging HH Recode 
*****************************************
merge m:1 hh_id using "$path_out/pry16_HH.dta"
tab hh9 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed 
drop _merge
erase "$path_out/pry16_HH.dta"
...
*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/pry16_CH.dta"
count if ln==0 
...
label define reg_lab 1 "Alto Paraguay" 2 "Alto Paraná" 3 "Asunción" 4 "Boquerón" ///
5 "Caaguazú" 6 "Central" 7 "Itapúa" 8 "Resto" 9 "San Pedro"
...
/* The admission age to compulsory education in Paraguay is 5 years. 
Preschool education takes place from age 0 and lasts 6 years. Primary education 
takes place from age 6-11(grades 1-6)and it lasts 6 years. Lower secondary 
education takes place from age 12-14 (grades 7-9). Upper secondary education 
takes place from age 15-17(grades 10-12). Compulsory education lasts 13 years.
...
http://www.ibe.unesco.org/fileadmin/user_upload/Publications/WDE/2010/pdf-versions/Paraguay.pdf
*/
...
/*For Paraguay MICS 2016 the variable eduhighyear has already the correct years 
for those in primary (3) and lower secondary (4).*/ 
gen eduyears = eduhighyear
replace eduyears = 0 if edulevel<=3 & eduhighyear==.   
	/*Assuming 0 year if they only attend preschool (10) or primary (11) but 
	the last year is unknown*/
	
	/*Note: For Paraguay MICS 2016 the variable eduhighyear has already the 
	correct years for those lower secondary (4). */
	
replace eduyears = eduhighyear + 9 if (edulevel ==5) 
	/*Higher secondary after 6 years of primary and 3 of lower secondary*/
replace eduyears = eduhighyear + 12 if (edulevel ==6 | edulevel==7)   
	/*University education assumed to start after 12 years of general 
	education */
	
replace eduyears = . if (edulevel==3 & eduhighyear > 12) 
	//For Paraguay primary+secondary education lasts 12 years. 
...
gen child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Paraguay MICS 2016, the official school entrance age is 6. 
	So, age range used is 6-14 (=6+8) 
...
********************************************************************************
	//NOTE: Paraguay MICS 2016: No information on child mortality from men 
...
drop temp_f
	...
	/* In the case of Paraguay, this variable takes missing value because the 
...
tab toilet toilet_mdg, miss
	...
	/* The country report for Paraguay indicate that the categoy 'flush do not 
	where' as improved saniation facility. As such, for the purpose of the 
	global MPI we follow the country report.*/ 
...
gen water_mdg = 1 if water==11 | water==12 | water==13 | water==14 |  ///
	...
	For Paraguay according to MICS 2016 final report on page 110, the 
	category '61. Carro-tanque,camion cisterna' is considered improved */
...
replace water_mdg = 0 if water_mdg==1 & timetowater >= 30 & ///
	...
	**Please exclude missing observations
...
clonevar psu = hh1
	/* For Praguay MICS 2016 there are two strata variables: strata2 with 23 
	stratums based on urban vs rural, regions and areas with indigenous 
	population; and strata1 with 45 stratas based on urban vs rural, regions, 
	areas with indigenous population and households with children under 5. For 
	the purpose of the global MPI, we use variable strata2.*/
clonevar strata = strata2
...
save "$path_out/pry_mics16.dta", replace 
