...
2019 Global Multidimensional Poverty Index - Namibia DHS 2013 [STATA do-file]. 
...
/*Nambia DHS 2013: Anthropometric data was collected from all eligible 
women 15-49 years, men age 15-64 and children under 5 years living in half of 
...
use "$path_in/NMKR61FL.DTA", clear 
...
drop if b5==0 
	...
	//Nambia DHS 2013: 228 observations deleted
...
gen str30 datalab = "children_nutri_nam" 
...
use "$path_out/children_nutri_nam_z_rc.dta", clear 
...
sort ind_id
save "$path_out/NAM13_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_nam_z_rc.xls"
erase "$path_out/children_nutri_nam_prev_rc.xls"
erase "$path_out/children_nutri_nam_z_rc.dta"
...
use "$path_in/NMBR61FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Namibia DHS 2013: these figures are identical
...
sort ind_id
save "$path_out/NAM13_BR.dta", replace	
	
	
********************************************************************************
*** Step 1.3  IR - WOMEN's RECODE  
*** (All eligible females 15-64 years in the household)
********************************************************************************

use "$path_in/NMIR61FL.DTA", clear
...
sort  ind_id
save "$path_out/NAM13_IR.dta", replace
...
gen str30 datalab = "girl_nutri_nam" 
lab var datalab "Working file"
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores */
use "$path_out/girl_nutri_nam_z.dta", clear 
...
sort ind_id
	...
	//Erase files from folder:
erase "$path_out/girl_nutri_nam_z.xls"
erase "$path_out/girl_nutri_nam_prev.xls"
erase "$path_out/girl_nutri_nam_z.dta"


	//Save a temp file for merging with PR:
save "$path_out/NAM13_IR_girls.dta", replace
...
sort ind_id
save "$path_out/NAM13_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Namibia 2013, anthropometric data was collected 
...
use "$path_in/NMPR61FL.DTA", clear
...
keep if _merge==3
	...
	keep the individuals who matched at _merge==3, that is, the 4,333 
...
*** Variable: BODY WEIGHT (KILOGRAMS) ***
	/*In the case of Namibia DHS 2013, the usual mv437 variable (body weight) 
...
*** Variable: HEIGHT (CENTIMETERS)
	/*In the case of Namibia DHS 2013, the usual mv438 variable (height) 
...
/* We use datalab to specify the name that will prefix the output files that 
will be produced from using this ado file*/
gen str30 datalab = "boy_nutri_nam" 
lab var datalab "Working file"	
...
use "$path_out/boy_nutri_nam_z.dta", clear 
...
sort ind_id
save "$path_out/NAM13_MR_boys.dta", replace


	//Erase files from folder:
erase "$path_out/boy_nutri_nam_z.xls"
erase "$path_out/boy_nutri_nam_prev.xls"
erase "$path_out/temp.dta"
erase "$path_out/boy_nutri_nam_z.dta"
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/NAM13_BR.dta"

drop _merge

erase "$path_out/NAM13_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/NAM13_IR.dta"
...
erase "$path_out/NAM13_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/NAM13_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss 
	/*Note: In Namibia DHS 2013, 17 girls 15-19 were identified as measured 
...
erase "$path_out/NAM13_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/NAM13_MR.dta"
...
erase "$path_out/NAM13_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
merge 1:1 ind_id using "$path_out/NAM13_MR_boys.dta"
...
tab hb40 if hb65==1 & hb13 ==0 & hv118==1 & teen_MR ==. & (hv105>=15 & hv105<=19)	
	/*Note: In Namibia DHS 2013, 11 boys 15-19 were identified as measured 
	and are present in the PR recode, but they were not present in the MR recode. 
	It should be noted that they have BMI information but not BMI-for-age.*/
	
drop _merge

erase "$path_out/NAM13_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/NAM13_KR.dta"
...
tab resident, miss
	...
	In the context of Namibia DHS 2013, 1,149 (2.69%) individuals who were 
...
/* In the context of Nambia DHS 2013, height and weight measurements were
collected from children (0-5) and women (15-64) living in 1/2 of the sampled 
households.*/
...
*** No Eligible Women 15-64 years
*****************************************
	/* Note that in the case of Namibia DHS 2013, data was collected from 
	eligible women aged 15-64 years. The eligibility variable captures the age 
	group correctly */
*tab age hv117	
...
//Subnational region
	/*NOTE: The sample for the Nambia DHS 2013 to provide estimates of key 
	population and health indicators, including fertility and mortality rates, 
	for the country as a whole and for urban and rural areas. In addition, the 
	sample was designed to provide estimates of most key variables for the 13 
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Namibia, the official school entrance age is 6 years (p.24).  
...
********************************************************************************
/*Note: Namibia DHS 2013 has anthropometric data for women and men aged 
15 - 64 years. In the case of women, this deviates from the DHS standard, where 
usually anthropometric data is collected only from women between the age of 
15-49 years. The principles of the global MPI has been to utilise any
individual level nutrition data that is available up to the age of 70 years.
As such, in the context of Namibia, we make use the nutrition data for all 
adults between 15 - 64 years */
...
*** Standard MPI: BMI Indicator for Women 15-64 years ***
******************************************************************* 
...
*** Standard MPI: BMI Indicator for Men 15-64 years ***
******************************************************************* 
...
tab cookingfuel cooking_mdg, miss	
	...
	/*Note that in Namibia DHS 2013, the category 'other' cooking fuel is not 
...
save "$path_out/nam_dhs13.dta", replace 
