...
2019 Global Multidimensional Poverty Index -  Mauritania MICS 2015
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Mauritania_MICS5_Datasets
...
/*Mauritania MICS 2015: Page 25 indicates that anthropometric data was collected 
only for childen under 5 years. */ 
...
gen str30 datalab = "children_nutri_mrt" 
...
*** Variable: AGE ***
	/*Note: We use age in months since there is no missing values when compared 
	  to age in days */
codebook cage caged
tab caged, miss 
tab cage, miss
...
use "$path_out/children_nutri_mrt_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: In the context of Mauritania MICS 2015, 168 children are replaced
	as '.' because they have extreme z-scores which are biologically implausible. */
...
erase "$path_out/children_nutri_mrt_z_rc.xls"
erase "$path_out/children_nutri_mrt_prev_rc.xls"
erase "$path_out/children_nutri_mrt_z_rc.dta"
...
tab wb2, miss
	//Women 15-49 years, 902 women with missing age
...
clonevar marital = mstatus
recode marital (3=4)(4=3)
recode marital (9=.)
label values marital lab_mar	
label var marital "Marital status of household member"
tab marital mstatus, miss
...
*****************************************
	...
	to generate this variable in this do-file so as to be consistent in the
	global MPI work */	 	
gen	no_hem_eligible = .
...
//Subnational region
	/* The Mauritania MISC 2015 sample was designed to provide estimates of a 
	large number of indicators on the situation of children and women in 
	Mauritania at the national level, for urban and rural areas, and at the 
	regional level including the thirteen wilayas of the country: Hodh Echargui, 
	Hodh El Gharbi, Assaba, Gorgol, Brakna, Trarza, Adrar, Nouadhibou, Tagant, 
	Guidimagha, Tiris Zemour, Inchiri and Nouakchott with the exception of two 
	of the wilayas grouped into one area of ​​study (p.43). The wilayas of 
	Inchiri and Tiris-Zemmour have been grouped together (p.14).*/
codebook hh7, tab (99) 
decode hh7, gen(temp)
...
In http://uis.unesco.org/country/MR
-preschool: 3-5
-primary: 6-11
-secondary:12-18
-tertiary: 19-23
...
PRIMARY EDUCATION 
Access to basic education is open to children aged at least 6 and 9 at most. 
The duration of the studies is 6 years and schooling is compulsory. Two 
repetitions are allowed. The age limit to remain in this level of education is 
16 years old. 
Basic education comprises 3 cycles: preparatory cycle, elementary cycle and 
middle cycle, of 2 years each. The certificate of primary studies settles the 
end of the studies. The transition from primary to secondary is subject to a 
contest. 
SECONDARY EDUCATION
Before the reform of 1999, the first and the second cycles of secondary 
education had each one 3 years of studies. Since 2007, the first cycle of 
secondary education has a duration of 4 years. Students get a certificate of 
studies of first cycle (BEPC) and the baccalaureat (second cycle). Technical 
secondary education has two streams. Access to a 2-year course of professional 
education.  Students need to hold the BEPC or justify the required level from 1 
out of the 3 classes of the second cycle of secondary education. If they pass 
this 2-year course, students get a certificate of Professional Vocational 
Certificate (BEP). The second stream is reserved for candidates with a final 
level (those who have the level of the last year of second cycle of secondary 
education). After 3 years of study they get a Technician's Baccalaureat 
(Technician's Certificate) (p. 9 of the report) &  
http://www.ibe.unesco.org/fileadmin/user_upload/Publications/WDE/2010/pdf-versions/Mauritania.pdf
*/


tab ed4b ed4a, miss
tab age ed6a if ed5==1, miss
	/*Note: in the case of Mauritania MICS 2015, there are inconsistencies such 
	as individuals showing too much schooling for their age and also too much 
	age for the level of schooling (if we follow the report that says that 16 
	years old is the age limit for primary education). This issue will be 
	addressed in the subsequent set of commands, that is, cleaning the 
	inconsistencies*/

clonevar edulevel = ed4a 
	//Highest educational level attended
replace edulevel = . if ed4a==. | ed4a==8 |ed4a==98 | ed4a==99  | ed4a==9 
...
replace eduyears = eduhighyear + 6 if (edulevel ==2 | edulevel==3)   
	/*Secondary education assumed to start after 6 years of primary education */
replace eduyears = eduhighyear + 12 if (edulevel ==4) 
	/*Superior education assumed to start after at least 12 years of general 
	education */
replace eduyears = 0 if edulevel==5 | edulevel==6
	/*Since Coranique and Mahadra are informal education, this corresponds to 
	zero education years.*/
replace eduyears = 6 if (edulevel == 2 | edulevel==3) & eduhighyear ==. 
replace eduyears = 12 if edulevel==4 & eduhighyear==. 
	/*We replace the missing education years of observations with secondary and 
	superior education with the maximum number of years of education of the 
	previous level.*/
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Mauritania, the official school entrance age is 6 years 
...
*** Standard MPI ***
****************************************
	/*Note: Mauritania MICS 2015 considers 'flush to unknown place / not sure /
	do not know where' as improved sanitation (Table WS.5, p.144).
	In Mauritania, the report considers the category open defecation as neither 
	improve nor non-improved. We follow the standard set in the MDGs, where 
...
clonevar ndwater = ws2  
	//Non-drinking water. Check if there is observation	
tab ws2 if water==91 	
...
*** Standard MPI ***
****************************************
	/* Note: Mauritania MICS 2015: rainwater is considered unimproved, 
	according to the survey report. In addition, households with missing 
	information on source of drinking water is identified as deprived
	(Table WS.1, p.137). We have followed the country report.   */
...
replace water_mdg = 0 if water==32 | water==42 | water==51 | water==61 | ///
						 water==71 | water==81 | water==96 | water==99 
...
replace water_mdg = 0 if water==91 & ///
						 (ndwater==32 | ndwater==42 | ndwater==51 | ///
						  ndwater==61 | ndwater==71 | ndwater==81 | ///
						  ndwater==96 | ndwater==99) 
...
lab var floor_imp "Household has floor that it is not earth/sand/dung/stone"
...
****************************************
gen	cooking_mdg = 1
replace cooking_mdg = 0 if cookingfuel>=7 & cookingfuel<95 
...
save "$path_out/mrt_mics15.dta", replace 
