...
2019 Global Multidimensional Poverty Index - Swaziland MICS 2014
...
/* Swaziland MICS 2014: The 2014 Swaziland MICS consists of four main
questionnaires including a household questionnaire, women (age 15-49 years)
and men (age 15-59 years) questionnaires and a questionnaire for children under
age five (p.1). P2 indicates that anthropometric data was collected only for
...
gen str30 datalab = "children_nutri_swz" 
...
use "$path_out/children_nutri_swz_z_rc.dta", clear 
...
erase "$path_out/children_nutri_swz_z_rc.xls"
erase "$path_out/children_nutri_swz_prev_rc.xls"
erase "$path_out/children_nutri_swz_z_rc.dta"
...
codebook ln bhln, tab (100)
	/* We use ln because that is the standard variable used in the other 
	datatsets. Also, ln matches the wm4 variable in wm recode file.*/
gen double ind_id = hh1*100000 + hh2*100 + ln
format ind_id %20.0g
label var ind_id "Individual ID"
...
tab wb2, miss 
	//Note: 239 women (15-49 years) with no info on age.
tab cm1 cm8, miss
...
replace marital = 5 if mstatus == 2 & ma6==3
	//4: Separated/not living together	
...
lookfor region
codebook hh7, tab (99)
	//Accurate to the four administrative regions of Eswatini (p.3).
decode hh7, gen(temp)
...
/* As of the 2005 Constitution, there is no compulsory schooling in Eswatini,
though every Swazi child has the right to free education in public schools at
least up to the end of primary school. 
Basic general education consists of 7 years of primary and 5 years of secondary, 
12 years in total. The official admission age to primary education is 6 years. 
Primary education takes place from age 6-12 (grades 1-7). Secondary education 
takes place from age 13-15 (grades 8-17).
Reference: 
http://www.ibe.unesco.org/fileadmin/user_upload/Publications/WDE/2010/pdf-versions/Swaziland.pdf 
*/

tab ed4b ed4a, miss
tab age ed6a if ed5==1, miss
	/*In the case of Swaziland MICS 2014, there is inconsistency such as 
...
replace eduyears = eduhighyear + 7 if (edulevel==2)   
	//Secondary after 7 years of primary education 
	
replace eduyears = eduhighyear + 12 if (edulevel==3)   
	/*Professional education assumed to start after 12 years of general 
	education */
replace eduyears = eduhighyear + 12 if (edulevel==4) 
	/*University after 12 years of education, 7 years of primary + 5 years of 
	secondary */
replace eduyears = . if (edulevel==2 & eduhighyear>13) 
	/* According to the report (p. 5), non-compulsory, basic education lasts
	12 years. */
...
gen	child_schoolage = (age>=6 & age<=14)
	/*Note: In Eswatini, the official school entrance age is 6 years.  
...
gen	toilet_mdg = ((toilet<23 | toilet==31) & shared_toilet!=1) 
	/*Note: Following the MDG definition, 'flush don't know where' is considered 
	as non-improved. However, in Eswatini, this particular category is 
	identified as improved sanitation facilities (p.85 country report), so the 
	line below is adjusted to include only "flush somewhere else" as deprived */
...
tab toilet toilet_mdg, miss
	/*Note: In Eswatini, the report considers the category open defecation (95) 
...
codebook ndwater, tab (30) 
	/*Non-drinking water: There is only 2 observations. So we have taken the 
	variable as non existing */
...
gen	roof_imp = 1 
replace roof_imp = 0 if roof<31 | roof==96 
...
*** Generate coutry and survey details for estimation ***
	//Note that Swaziland changed its name to Eswatini in April 2018.
char _dta[cty] "eSwatini"
char _dta[ccty] "SWZ"
char _dta[year] "2014" 	
...
save "$path_out/swz_mics14.dta", replace 
