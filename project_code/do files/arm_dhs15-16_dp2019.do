...
2019 Global Multidimensional Poverty Index - Armenia DHS 2015-16 [STATA do-file]. 
...
/*Armenia DHS 2015-16: Anthropometric information were recorded for all women 
age 15-49 and all children age 6-59 months (p.5). Anthropometric information 
was not collected from men 15-49, though they were interviewed in a 
subsample of 1/2 of eligible households (p.5)*/
...
use "$path_in/AMKR72FL.DTA", clear 
...
drop if b5==0 
	//Children who are not alive excluded 
	//Armenia DHS 2015-16: 14 observations deleted
...
gen str30 datalab = "children_nutri_arm" 
...
standards */
use "$path_out/children_nutri_arm_z_rc.dta", clear 
...
count if _fwei==1
count if _flen==1
	/* Note: In the context of Armenia DHS 2006, 29 children are replaced 
...
sort ind_id
save "$path_out/ARM15-16_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_arm_z_rc.xls"
erase "$path_out/children_nutri_arm_prev_rc.xls"
erase "$path_out/children_nutri_arm_z_rc.dta"
...
use "$path_in/AMBR72FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Armenia DHS 2015-16: these figures are identical
...
sort ind_id
save "$path_out/ARM15-16_BR.dta", replace	
...
use "$path_in/AMIR72FL.DTA", clear
...
sort ind_id
save "$path_out/ARM15-16_IR.dta", replace
...
gen str30 datalab = "girl_nutri_arm" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_arm_z.dta", clear 
...
sort ind_id
save "$path_out/ARM15-16_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_arm_z.xls"
erase "$path_out/girl_nutri_arm_prev.xls"
erase "$path_out/girl_nutri_arm_z.dta"
...
use "$path_in/AMMR72FL.DTA", clear 
...
********************************************************************************
/*Note: In the case of Armenia 2015-16, anthropometric data was NOT collected 
for men.*/
...
use "$path_in/AMPR72FL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ARM15-16_BR.dta"
drop _merge
erase "$path_out/ARM15-16_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ARM15-16_IR.dta"
...
erase "$path_out/ARM15-16_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/ARM15-16_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss 
	/*Note: In Armenia DHS 2015-16, 3 girls 15-19 were identified as measured, 
	eligible, and completed interview and are present in the PR recode, but they 
	were not present in the IR recode. It should be noted that they have BMI 
	information but not BMI-for-age.*/
drop _merge

erase "$path_out/ARM15-16_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ARM15-16_MR.dta"
tab men_MR hv118, miss col
drop _merge
erase "$path_out/ARM15-16_MR.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ARM15-16_KR.dta"
...
erase "$path_out/ARM15-16_KR.dta"
...
tab resident, miss
	...
	In the context of Armenia DHS 2015-16, 87 (0.3%) individuals who were 
...
/* In the context of Armenia DHS 2015-16, height and weight measurements were
collected from children (0-5) and women (15-49) in all the sampled 
households. Hence there is no presence of subsmaple.  */
...
/* Households are identified as having 'no eligible' members if there is no 
...
//Subnational region
	/* NOTE: The sample for the Armenia DHS 2015-16 was designed to produce 
	representative estimates of key indicators at the national level, for 
	Yerevan, and for total urban and total rural areas. Many indicators can be 
	estimated at the regional (marz) level (there are 11 regions), but this is 
	not representative (p.5). As such, for the purpose of the global MPI, 
	despite the presence of subnational information (hv024), we generated the 
	'region' variable as an empty variable. */ 
...
encode temp, gen(region_2)
codebook region_2, tab (100)
tab hv024 region_2, miss 
lab var region_2 "Regions in Armenia - not representative"
drop temp
...
gen	child_schoolage = (age>=6 & age<=14)
	/*Note: In Armenia, the official school entrance age is 6 years.  
...
********************************************************************************
/* Note: Armenia DHS 2015-16 has no anthropometric data for men, 15-49 years */
...
/* In Armenia, all the households that use bottled water for drinking get their 
non-drinking water piped into dwelling, so they are considered non-deprived */
...
gen	floor_imp = 1
replace floor_imp = 0 if floor==11 
...
save "$path_out/arm_dhs15-16.dta", replace 
