...
2019 Global Multidimensional Poverty Index - Haiti DHS 2016-17 [STATA do-file]. 
...
/* Haiti DHS 2016-17: Anthropometric information was collected for all children
under 5 years. Among a subsample of 2/3 households, these parameters were also 
recorded among women aged 15-49. In the other subsample of one in three 
households, weight and height were also recorded for men and women aged 
35-64 and children aged 5-14. (p.6) */
...
use "$path_in/HTPR70FL.DTA", clear 
...
count if hv120==1	
	/*All 6,896 children under 5 are eligible for anthropometric 
	measurement. */	
count if hc1!=.
	//All 6,896 children under 5 have information on age in months.	
tab hv105 if hv120==1, miss
	/*However, 3 children are identified as older than 5 years following the 
	hv105 variable (age in years). To compute the z-scores, we are 
	reconstructing the age variable using birth and interview dates, which is 
	more accurate. */
tab hc13 if hc1!=., miss
	//6,834 (99%) of the 6,896 children have been measured
tab hc13 if hc1!=. & hc3<=9990, miss
tab hc13 if hc1!=. & hc2<=9990, miss
	/*For example, height and weight data is available for all 6,834 children 
...
gen str30 datalab = "children_nutri_hti" 
...
standards */
use "$path_out/children_nutri_hti_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1 
	/*Haiti DHS 2016-17: 33 children were replaced as missing because
	they have extreme z-scores which are biologically implausible. */
 
count if stunting!=. & hc13==0 & hv102==1
	/*Note: 6,687 children under 5 who are usual residents was measured and have 
	height-for-age (stunting) indicator. Table 11.1 (sum 'number of children' by 
	age in months, p.202) indicate that height-for-age indicator covered 6,618 
	children. */ 
...
save "$path_out/HTI16-17_PR_child.dta", replace
	

	//Erase files from folder:
erase "$path_out/children_nutri_hti_z_rc.xls"
erase "$path_out/children_nutri_hti_prev_rc.xls"
erase "$path_out/children_nutri_hti_z_rc.dta"


********************************************************************************
*** Step 1.1a  PR - INDIVIDUAL RECODE  
*** (Children 5-14 years in the household)
********************************************************************************
/*Haiti DHS 2016-17: The survey collected nutrition data from children 
5-14 years. These eligible children were selected from households that were 
not eligible for domestic voilence module. As such we will compute the 
BMI-for-age for this group.*/
...
*** hv001=cluster number;  
*** hv002=household number; 
*** hvidx=individual's line number
gen double ind_id = hv001*1000000 + hv002*100 + hvidx 
format ind_id %20.0g
label var ind_id "Individual ID"


*** Identify anthropometric sample for children 5-14 years
count if hv105>=5 & hv105<=14 
	//There are 14,515 children 5-14 years in the sample
tab hv044 if hv105>=5 & hv105<=14
	/*Total number of children 5-14 years who live in household not selected 
	for domestic violence module and have anthropometric data: 4,858 */
desc shb105 shb106 shb107 shccmca hv807c
count if shb105!=. & hv105>=5 & hv105<=14 & hv044==0
count if shb106!=. & hv105>=5 & hv105<=14 & hv044==0
	//Height and weight data is available for 4,809 children 

	
*** Keep relevant sample
keep if hv105>=5 & hv105<=14 & hv044==0 
count
	//Total children 5-14 years: 4,858
...
gen age_month = (hv807c - shccmca)
lab var age_month "Age in months, individuals 5-14 years"
sum age_month 
...
*** Variable: BODY WEIGHT (KILOGRAMS) ***
codebook shb105, tab (999)
gen weight = shb105/10 
sum weight


*** Variable: HEIGHT (CENTIMETERS)	
codebook shb106, tab (999)
gen height = shb106/10 
replace height = . if shb106>=9994
sum height
...
gen str30 datalab = "child14_nutri_hti" 
lab var datalab "Working file"
...
use "$path_out/child14_nutri_hti_z.dta", clear 	
	
	
gen	z_bmi = _zbfa
replace z_bmi = . if _fbfa==1 
	/*Haiti DHS 2016-17: 12 individuals were replaced as missing 
...
*** Standard MPI indicator ***
	...
	otherwise */	
gen	child14_low_bmiage = (z_bmi < -2.0) 
replace child14_low_bmiage = . if z_bmi==.
lab var child14_low_bmiage "Child 5-14 low bmi 2sd - WHO"


*** Destitution indicator ***
 	/*Takes value 1 if BMI-for-age is under 3 stdev below the median & 0 
	otherwise */	
gen	child14_low_bmiage_u = (z_bmi < -3.0)
replace child14_low_bmiage_u = . if z_bmi==.
lab var child14_low_bmiage_u "Child 5-14 very low bmi 3sd - WHO"

tab child14_low_bmiage, miss
tab child14_low_bmiage_u, miss

gen child14_PR=1 
	//Identification variable for teenagers (15-19 years) in PR recode 


	//Retain relevant variables:	
keep ind_id child14_PR child14_low_bmiage*
order ind_id child14_PR child14_low_bmiage*
sort ind_id
save "$path_out/HTI16-17_PR_child14.dta", replace
	

	//Erase files from folder:
erase "$path_out/child14_nutri_hti_z.xls"
erase "$path_out/child14_nutri_hti_prev.xls"
erase "$path_out/child14_nutri_hti_z.dta"
...
use "$path_in/HTBR70FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Haiti DHS 2016-17: these figures are identical
...
sort ind_id
save "$path_out/HTI16-17_BR.dta", replace	
...
use "$path_in/HTIR70FL.DTA", clear
...
tab v012,miss
	//Check age	of women interviewed
tab v201 if v012<50
tab v201 if v012>49
	//Child mortality information was collected from women 15-49 years	
	
keep if v012<50 
	/*We only retain women 15-49 years in this section since fertility and 
	mortality questions relate to this age group */
count	
	//14,371 women 15-49 years	
		

keep ind_id women_IR v003 v005 v012 v201 v206 v207 
order ind_id women_IR v003 v005 v012 v201 v206 v207 
sort ind_id
save "$path_out/HTI16-17_IR.dta", replace
...
/* We use datalab to specify the name that will prefix the output files that 
will be produced from using this ado file*/
gen str30 datalab = "girl_nutri_hti" 
lab var datalab "Working file"
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores to compute BMI-for-age*/
use "$path_out/girl_nutri_hti_z.dta", clear 

	
gen	z_bmi = _zbfa
replace z_bmi = . if _fbfa==1 
	/*Haiti DHS 2016-17: 1 girl 15-19 years were replaced as missing 
...
sort ind_id
save "$path_out/HTI16-17_PR_girls.dta", replace
	

	//Erase files from folder:
erase "$path_out/girl_nutri_hti_z.xls"
erase "$path_out/girl_nutri_hti_prev.xls"
erase "$path_out/girl_nutri_hti_z.dta"
...
use "$path_in/HTMR70FL.DTA", clear 
...
tab mv012,miss
	//Check age	of men interviewed
tab mv201 if smsample==0, miss	
keep if smsample==1
	/*We only retain men 15-64 years who have responded to the child mortality 
	question */
count	
	//9,795 men 15-49 years
...
sort ind_id
save "$path_out/HTI16-17_MR.dta", replace
...
/*Haiti DHS 2016-17: Anthropometric data was collected only from men 35-64. 
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/HTI16-17_BR.dta"
drop _merge
erase "$path_out/HTI16-17_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/HTI16-17_IR.dta"
tab women_IR hv117 if hv105>=15 & hv105<=49, miss col
tab ha65 if hv117==1 & women_IR==. & hv105>=15 & hv105<=49, miss 
	//Total number of eligible women not interviewed
drop _merge
erase "$path_out/HTI16-17_IR.dta"
...
tab v201 if hv117==1 & hv105>=15 & hv105<=49, miss
tab v201 ha65 if hv117==1, miss
	/*Note: Some 1% eligible women 15-49 years did not provide information on 
	their birth history. This will result in missing value for the child mortality 
	indicator that we will construct later */

	
*** Merging 15-19 years: girls 
*****************************************
merge 1:1 ind_id using "$path_out/HTI16-17_PR_girls.dta"
tab girl_PR hv044 if hv105>=15 & hv105<=19 & hv104==2, miss col
drop _merge
erase "$path_out/HTI16-17_PR_girls.dta"

	
*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/HTI16-17_MR.dta"
tab men_MR hv118 if hv044==1 & hv105>=15 & hv105<=49, miss col
tab hb65 if hv118==1 & men_MR ==. & hv105>=15 & hv105<=49 
	//Total of eligible men not interviewed
drop _merge
erase "$path_out/HTI16-17_MR.dta"


*** Merging 15-19 years: boys 
*****************************************
//Haiti DHS 2016-17: Anthropometric data was not collected for boys 15-19 years 
...
*** Merging 5-14 years 
*****************************************
merge 1:1 ind_id using "$path_out/HTI16-17_PR_child14.dta"
tab child14_PR hv044 if hv105>=5 & hv105<=14, miss col
drop _merge
erase "$path_out/HTI16-17_PR_child14.dta"


*** Merging child under 5 
*****************************************
merge 1:1 ind_id using "$path_out/HTI16-17_PR_child.dta"
tab hv120, miss  
tab hc13 if hv120==1, miss
drop _merge
erase "$path_out/HTI16-17_PR_child.dta"
...
drop if resident!=1 
tab resident, miss
	/*Note: Haiti DHS 2016-17, 1,015 (1.70%) individuals who were 
...
/*
Haiti DHS 2016-17: height and weight measurements were collected from all 
children (0-5). They were also collected from women (15-49) living in 2/3 of the 
sampled households, as well as from children (5-14), women (35-64) and men 
(35-64) living in 1/3 of the sampled households. This suggest that 
anthropometric data was collected from all households that were sampled. Hence
there is no subsample selection.  
Table 11.1 (p.202), Table 11.1.1-2 (p.203-4), Table 11.11 (p.216) summarises 
nutritional status of children and women respectively by the 11 departments in 
Haiti, suggesting the anthropometric and child mortality estimates are 
...
applicable population, that is, children 0-14 years, women 15-64 years or 
...
*** No eligible women 15-49 years (living in 2/3 of HHs sampled)
*** No eligible women 35-64 years (living in 1/3 of HHs sampled)  
*** for adult nutrition indicator
***********************************************
tab hv105 if ha13!=. & hv044==1, miss 
tab ha13, miss
tab ha13 hv044 if hv105>=15 & hv105<=49 & hv104==2, miss
	/*Height and weight data was collected from 9,610 women 15-49 living in 
	2/3 of the sampled households, selected for domestic violence module. */
	
tab hv105 if shwresia!=. & hv044==0, miss 
tab shwresia, miss
tab shwresia hv044 if hv105>=35 & hv105<=64 & hv104==2, miss
	/*Height and weight data was collected from 2,545 women 35-64 living in 
	1/3 of the sampled households, not selected for domestic violence module */	
	
gen fem_nutri_eligible = (ha13!=.)
replace fem_nutri_eligible = 1 if shwresia!=.
...
*****************************************
count if hv105>=15 & hv105<=49 & hv104==2
tab hv117, miss
tab hv103 hv117 if hv105>=15 & hv105<=49 & hv104==2, miss
	/*Only women who slept the night before in the household were 
	eligible for female interview (women's questionnaire). */
gen	fem_eligible = 1 if hv117==1 & hv105>=15 & hv105<=49
...
*** No eligible men 35-64 years (living in 1/3 of HHs sampled) 
*** for adult nutrition indicator (if relevant)
***********************************************	
tab hv105 if shmresia!=. & hv044==0, miss 
tab shmresia, miss
tab shmresia hv044 if hv105>=35 & hv105<=64 & hv104==1, miss
	/*Height and weight data was collected from 2,132 men (35-64) living in 1/3 of 
	the sampled households, not selected for the domestic violence module */
gen male_nutri_eligible = (shmresia!=.)	
...
replace male_eligible=0 if hv118==1 & hv105>=15 & hv105<=34 & hv104==1 & hv044==0
	//Replace men who did not answer the child mortality question as ineligible 
...
*** No eligible children 5-14 (living in 1/3 of HHs sampled)
*** for nutrition indicator
*****************************************
tab hv044 if hv105>=5 & hv105<=14, miss
	/*Height and weight data was collected from 4,792 children 5-14 years who 
	  are usual residents living in 1/3 of the sampled households, not selected 
	  for the domestic violence module */

gen	child14_eligible = 1 if hv105>=5 & hv105<=14 & hv044==0
bysort	hh_id: egen hh_n_children_eligible = sum(child14_eligible)  
	//Number of eligible children for anthropometrics
gen	no_child14_eligible = (hh_n_children_eligible==0) 
	//Takes value 1 if there were no eligible children for anthropometrics
lab var no_child14_eligible "Household has no children (5-14) eligible for anthropometric"
drop child14_eligible hh_n_children_eligible
tab no_child14_eligible, miss
...
***********************************************
gen	no_child_fem_eligible = (no_child_eligible==1 & ///
							 no_child14_eligible==1 & no_fem_nutri_eligible==1)
...
*** for nutrition indicator 
***********************************************
gen no_eligibles = (no_fem_nutri_eligible==1 & no_male_nutri_eligible==1 & ///
					no_child_eligible==1 & no_child14_eligible==1)
...
//Subnational region
	/*Haiti DHS 2016-17: The sample is stratified to provide an adequate 
	representation of the urban and rural environments as well as the 11 
	fields of study, corresponding to the 10 departments, and to the 
	Metropolitan Area (p.3). */   
clonevar region = hv024
codebook region, tab (99)
tab hv024 region, miss
lab var region "Region for subnational decomposition"
label define lab_reg ///
0 "Aire Métropolitaine" ///
1 "Rest-Ouest" ///
2 "Sud-Est" ///
3 "Nord" ///
4 "Nord-Est" ///
...
replace attendance = 0 if (attendance==9 | attendance==.) & hv109==0 
	//Those with no educational attainment are replaced with a '0'
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Haiti, the official school entrance age to primary school is 
...
********************************************************************************
codebook ha40 shwbma shmbma


foreach var in ha40 shwbma shmbma {
			 gen inf_`var' = 1 if `var'!=.
			 bysort sex: tab age inf_`var' 
			 //women: 15-49 years; 35-64 years & men: 35-64 years
			 drop inf_`var'
			 }
***

*** BMI Indicator for Women 15-49 years ***
*** BMI Indicator for Women 35-64 years ***
******************************************************************* 
gen	f_bmi = ha40/100
	//women 15-19 years
replace f_bmi = shwbma/100 if f_bmi==.
	//women 35-64 years
...
*** BMI Indicator for Men 35-64 years ***
******************************************************************* 
gen m_bmi = shmbma/100
...
*** 			  and BMI for individuals 20-64 years ***
******************************************************************* 
gen low_bmi_byage = 0
...
replace low_bmi_byage = 0 if low_bmiage==0 & age_month!=.
	...
	information for adults and BMI-for-age for teenagers.*/	
...
********************************************************************************
*** Step 2.3b Children 5-14 Nutrition ***
*** (living in 1/3 of HHs sampled) ***
********************************************************************************


*** Standard MPI: BMI-for-age for children 5-14  ***
************************************************************************
bysort hh_id: egen temp = max(child14_low_bmiage)
gen	hh_no_low_bmiage_c14 = (temp==0) 
	//Takes value 1 if no child 5-14 in the hh has low BMI-for-age 
replace hh_no_low_bmiage_c14 = . if temp==.
replace hh_no_low_bmiage_c14 = 1 if no_child14_eligible==1 
	//Households with no eligible child 5-14 will receive a value of 1 
lab var hh_no_low_bmiage_c14 "Household has no children 5-14 with low BMI-for-age"
drop temp
...
replace hh_nutrition_uw_st = 0 if hh_no_low_bmiage==0 | hh_no_uw_st==0 | hh_no_low_bmiage_c14==0
replace hh_nutrition_uw_st = . if hh_no_low_bmiage==. & hh_no_uw_st==. & hh_no_low_bmiage_c14==.
...
replace hh_nutrition_uw_st = . if hh_no_low_bmiage==. & hh_no_uw_st==1 & no_child_eligible==1 & ///
								  hh_no_low_bmiage_c14==1 & no_child14_eligible==1
	/*Replace indicator as missing if household has eligible adult with missing 
	nutrition information and no eligible child 0-5 or 5-14 for anthropometric 
	measures */ 
replace hh_nutrition_uw_st = . if hh_no_uw_st==. & hh_no_low_bmiage==1 & no_adults_eligible==1 & ///
								  hh_no_low_bmiage_c14==1 & no_child14_eligible==1
	/*Replace indicator as missing if household has eligible child 0-5 with 
	missing nutrition information and no eligible adult or child 5-14 for 
	anthropometric measures. */ 	
replace hh_nutrition_uw_st = . if hh_no_low_bmiage_c14==. & hh_no_uw_st==1 & no_child_eligible==1 & ///
								   hh_no_low_bmiage==1 & no_adults_eligible==1
	/*Replace indicator as missing if household has eligible child 5-14 with 
	missing nutrition information and no eligible child 0-5 or adult for 
	anthropometric measures. */ 	
...
replace roof_imp = 0 if roof<=23 | roof==96  
	//Deprived if no roof, thatch/palm leaf, palm/bamboo, wood plank, other	
...
sort ind_id
compress
save "$path_out/hti_dhs16-17.dta", replace 
