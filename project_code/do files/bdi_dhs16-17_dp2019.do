...
2019 Global Multidimensional Poverty Index - Burundi DHS 2016-17 [STATA do-file]. 
...
/*In Burundi, anthropometric measures are available for women (15-49) 
and children (<5) eligible in a subsample of 1/2 of households (p2. Chapter 1)*/
...
use "$path_in/BUKR70FL.DTA", clear 
...
drop if b5==0 
	//Burundi 2016-17: 720 observations were deleted
...
gen str30 datalab = "children_nutri_bdi" 
...
use "$path_out/children_nutri_bdi_z_rc.dta", clear 
...
/* Note: In the context of Burundi DHS 2016-17, 18 children are replaced as '.'
...
sort ind_id
duplicates report ind_id
save "$path_out/BDI16-17_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_bdi_z_rc.xls"
erase "$path_out/children_nutri_bdi_prev_rc.xls"
erase "$path_out/children_nutri_bdi_z_rc.dta"
...
use "$path_in/BUBR70FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Burundi DHS 2016-17: these figures are identical
...
sort ind_id
save "$path_out/BDI16-17_BR.dta", replace	
...
use "$path_in/BUIR70FL.DTA", clear
...
sort  ind_id
save "$path_out/BDI16-17_IR.dta", replace
...
gen str30 datalab = "girl_nutri_bdi" 
lab var datalab "Working file"
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores */
use "$path_out/girl_nutri_bdi_z.dta", clear 
...
sort ind_id
save "$path_out/BDI16-17_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_bdi_z.xls"
erase "$path_out/girl_nutri_bdi_prev.xls"
erase "$path_out/girl_nutri_bdi_z.dta"
...
use "$path_in/BUMR70FL.DTA", clear 
...
sort ind_id
save "$path_out/BDI16-17_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Burundi 2016-17, no anthropometric data was collected 
...
use "$path_in/BUPR70FL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/BDI16-17_BR.dta"

drop _merge

erase "$path_out/BDI16-17_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/BDI16-17_IR.dta"
...
erase "$path_out/BDI16-17_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/BDI16-17_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Burundi 2016-2017, 39 girls 15-19 were identified as measured 
...
erase "$path_out/BDI16-17_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/BDI16-17_MR.dta"
...
tab hb13 hb65 if hv118==1 & men_MR ==., miss 
	//No anthropometric data for men in Burundi DHS 2016-17

drop _merge

erase "$path_out/BDI16-17_MR.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/BDI16-17_KR.dta"
...
erase "$path_out/BDI16-17_KR.dta"
...
tab resident, miss
	...
	In the context of Burundi DHS 2016-17, 651 (0.83%) individuals who were 
...
/*
In the context of Burundi DHS 2016-17, height and weight measurements were
...
//Subnational region
	/*NOTE: The sample for the Burundi DHS 2016-17 was designed to provide 
	estimates of key indicators for the country as a whole, for urban and rural 
	areas separately, and for each of the 18 districts (p.2). */ 
lookfor region
...
replace eduyears = . if eduyears>30
	//Recoded any unreasonable years of highest education as missing value
...
gen	child_schoolage = (age>=7 & age<=15)
	/*There is no data on the official entrance age for compulsory education 
	in Burundi. We have used the official entrance age for primary school, which 
	is 7. Thus the age range would be 7-15 
...
********************************************************************************
/*Note: Burundi DHS 2016-17 has no anthropometric data for men, 15-54 years */
...
foreach var in ha40 hb40 {
			 ...
			 //This is restricted only to females aged 15-49
			 drop inf_`var'
			 }
***
...
/*In Burindi, there is no "other" category */
...
replace water_mdg = 0 if water==71 & ///
						(ndwater==32 | ndwater==61 | ndwater==62 | ndwater==96) 
...
lab var cooking_mdg "Househod has cooking fuel by MDG standards"
/*
...
save "$path_out/bdi_dhs16-17.dta", replace 
