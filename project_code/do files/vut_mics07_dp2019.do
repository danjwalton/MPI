...
2019 Global Multidimensional Poverty Index - Vanuatu MICS 2007 [STATA do-file]. 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Vanuatu_2007_MICS_Datasets
...
/*Vanuatu MICS 2007: P19 indicate that anthropometric data was collected only 
...
gen str30 datalab = "children_nutri_vut" 
...
codebook an1, tab (999)
clonevar weight = an1	
replace weight = . if an1>=99 
tab	an4 an1 if an1>=99 | an1==., miss 
...
codebook an2, tab (999)
clonevar height = an2
replace height = . if an2>=999 
tab	an4 an2 if an2>=999 | an2==., miss
desc height 
summ height
...
codebook an2a
gen measure = "l" if an2a==1 
	//Child measured lying down
replace measure = "h" if an2a==2 
	//Child measured standing up
replace measure = " " if an2a==9 | an2a==0 | an2a==. 
...
use "$path_out/children_nutri_vut_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Vanuatu MICS 2007, 58 children were replaced as
...
sort ind_id
save "$path_out/vut07_CH.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_vut_z_rc.xls"
erase "$path_out/children_nutri_vut_prev_rc.xls"
erase "$path_out/children_nutri_vut_z_rc.dta"
...
********************************************************************************
	...
	/* Note: There is no BH dta file for Vanuatu MICS 2007. Hence this section 
...
tab wm9, miss
	/*Check the age profile of women in the sample: 15-49 years*/
tab cm1 cm7, miss
	/*Women who has never ever given birth will not have information on 
	child mortality*/
	
lookfor marital	
codebook mstatus ma4, tab (10)
tab mstatus ma4, miss 
gen marital = 1 if mstatus == 3 & ma4==.
	//1: Never married
replace marital = 2 if mstatus == 1 & ma4==.
	//2: Currently married
replace marital = 3 if mstatus == 2 & ma4==1
	//3: Widowed	
replace marital = 4 if mstatus == 2 & ma4==2
	//4: Divorced	
replace marital = 5 if mstatus == 2 & ma4==3
...
tab marital, miss
tab ma4 marital, miss
tab mstatus marital, miss
rename marital marital_wom	
	

	//Retain relevant variables:	
keep wm7 cm1 cm7 cm8a cm8b ind_id women_WM *_wom 
order wm7 cm1 cm7 cm8a cm8b ind_id women_WM *_wom
sort ind_id
save "$path_out/vut07_WM.dta", replace
...
********************************************************************************
	...
	//Note: Vanuatu MICS 2007 has no male recode information.
...
save "$path_out/vut07_HH.dta", replace
...
*** Merging BR Recode 
*****************************************
	//No BH file for Vanuatu MICS 2007
 
 
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/vut07_WM.dta"
tab hl6, miss 
gen temp = (hl6>0) 
...
drop temp
drop _merge
erase "$path_out/vut07_WM.dta"


*** Merging HH Recode 
*****************************************
merge m:1 hh_id using "$path_out/vut07_HH.dta"
tab hh9 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed 
drop _merge
erase "$path_out/vut07_HH.dta"
...
*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/vut07_CH.dta"
drop _merge
erase "$path_out/vut07_CH.dta"
...
***************************************** 
gen	child_eligible = (hl8>0 | child_CH==1) 
...
***********************************************
	//Since there is no male data this variable is assigned with missing value 
...
********************************************************************************
	...
	/*Note: In Vanuatu, the official entrance age to primary education is 
	6 years old. Primary education takes place from age 6-11(grades 1-6). 
	Lower secondary education takes place from age 12-15 years (grades 7-9). 
	Upper secondary education takes place from age 15-18(grades 10-12).
	Reference: http://data.uis.unesco.org */   

tab ed3b ed3a, miss
tab ed6b ed6a, miss
tab age ed3a if ed4==1, miss

clonevar edulevel = ed3a 
	//Highest educational level attended
replace edulevel = . if ed3a==. | ed3a==8 |ed3a==9  
	//ed4a=8/98/99 are missing values 
replace edulevel = 0 if ed2==2 
	//Those who never attended school are replaced as '0'
label var edulevel "Highest educational level attended"


clonevar eduhighyear = ed3b 
	//Highest grade of education completed
replace eduhighyear = .  if ed3b==. | ed3b==98 | ed3b==99 
	//ed4b=97/98/99 are missing values
replace eduhighyear = 0  if ed2==2 
	//Those who never attended school are replaced as '0'
...
gen	eduyears = eduhighyear
	/* In the Vanuatu MICS dataset the grades range from 1 to 18 and are not 
	defined separately by level of education. Thus, the variable eduyears can be 
	obtained directly from the variable eduhighyear (ed3b). */
...
tab attendance if age >=5 & age<=16, miss
	//Replace attendance with '0' for individuals who are not of school age
tab attendance if age >=5 & age<=16, miss 
	/*Replace attendance with '.' for individuals of schooling age, but have 
	missing observation on school attendance */	
tab attendance, miss
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Vanuatu, the official school entrance age to primary school is 
...
egen temp_f = rowtotal(cm8a cm8b), missing
	//Total child mortality reported by eligible women
replace temp_f = 0 if cm1==1 & cm7==2 | cm1==2  
...
drop temp_f
	...
	/* In the case of Vanuatu, this variable takes missing value because the 
...
replace water_mdg = 0 if water_mdg==1 & timetowater >= 30 & timetowater!=. & ///
						 timetowater!=995 & timetowater!=998 & timetowater!=999 
...
replace floor_imp = 0 if floor<=13 | floor==96 
	/* Note: 5% of people in Vanuatu live in houses with coral flooring. We 
	considered them deprived in flooring. */
...
clonevar television = hc9c 
gen bw_television   = .
clonevar radio = hc9b 
clonevar telephone =  hc9e
clonevar mobiletelephone = hc9d 
gen refrigerator = .	
clonevar car = hc10e  	
...
clonevar psu = hh1
egen strata = group(region)
	/*Note: The sample for MICS 2007 is a probability-based, stratified sample 
	of 3,000 households selected in 120 clusters, each consisting of 25 
	households. The sample was designed to provide reliable estimates for the 
	key MICS indicators at the national level, for urban and rural areas 
	separately, as well as for the six provinces of Malampa, Penama, Sanma, 
	Shefa, Tafea and Torba. The cities of Port Vila and Luganville are urban 
	cities under Sheifa and Sanma provinces respectively. The total areas of 
	other provinces were considered as rural. The sample was allocated to the 
	provinces/cities and by urban-rural in an optimum fashion to secure enough 
	cases in each domain and to obtain reliable estimates. Independent samples 
	for each domain resulted in eight separate surveys (six provinces and two 
	cities) in order to produce valid estimates for each domain simultaneously
	(MICS Report, p.19)" */
...
*** Rename key global MPI indicators for estimation ***
	/* Note: In the case of Vanuatu MICS 2007, there is no birth history file. 
...
save "$path_out/vut_mics07.dta", replace 
