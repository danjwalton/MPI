...
2019 Global Multidimensional Poverty Index - Maldives DHS 2016-17 [STATA do-file]. 
...
/*Maldives DHS 2016-17:  Height and weight measurements were collected from  
women age 15-49, men age 15-49, and children under age 5 in all selected 
households (p.3 & p.4). In 2009, only ever-married women and men were 
interviewed, but in 2016-17, all usual residents were eligible for 
interview (p.3). */
...
use "$path_in/MVPR71FL.DTA", clear 
...
tab hv120,miss
	/*3,700 children under 5 are eligible for anthropometric 
	measurement. */		
count if hc1!=.
	//All 3,700 children under 5 have information on age in months
tab hv105 if hc1!=.
	/*A cross check with the age in years reveal that all are within the 5 year 
	age group */
tab hc13 if hv120==1, miss
	//2,601 (70.3%) of the 3,700 children have been measured
tab hc13 if hc3<=9990, miss
tab hc13 if hc2<=9990, miss
	/*For example, height and weight data is available for the 2,601 children 
	who have been successfully measured. This closely corresponds with the
	report (p.155) which indicate that some 32% of children under age 5 had 
	missing information on either height or weight. */
...
gen str30 datalab = "children_nutri_mdv" 
...
standards */
use "$path_out/children_nutri_mdv_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1 
	/*Note: Maldives DHS 2016-17: 62 children were replaced as missing because
	they have extreme z-scores which are biologically implausible. */
 
count if stunting!=. & hc13==0 & hv102==1 
	/*2,518 children under 5 who are usual residents was measured and 
	have height-for-age (stunting) indicator. Table 11.1 (sum 'number of 
	children' by age in months, p.165) indicate that height-for-age 
	indicator covered 2,246 children. */ 
...
sort ind_id
save "$path_out/MDV16-17_PR_child.dta", replace
	

	//Erase files from folder:
erase "$path_out/children_nutri_mdv_z_rc.xls"
erase "$path_out/children_nutri_mdv_prev_rc.xls"
erase "$path_out/children_nutri_mdv_z_rc.dta"
...
use "$path_in/MVBR71FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Maldives DHS 2016-17: these figures are identical
...
sort ind_id
save "$path_out/MDV16-17_BR.dta", replace	
...
use "$path_in/MVIR71FL.DTA", clear
...
codebook v201 v206 v207,tab (999)
	//Women 15-49 years have information on child mortality 
...
sort ind_id
save "$path_out/MDV16-17_IR.dta", replace
...
tab ha13 if hv105>=15 & hv105<=19 & hv104==2, miss 
	...
	male survey and have anthropometric data: 1,178 */
tab ha13 hv117 if hv105>=15 & hv105<=19 & hv104==2, miss
tab ha13 hv103 if hv105>=15 & hv105<=19 & hv104==2, miss
	/*13 of the 1,178 women 15-19 years are identified as non-eligible
...
gen str30 datalab = "girl_nutri_mdv" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_mdv_z.dta", clear 
...
sort ind_id
save "$path_out/MDV16-17_PR_girls.dta", replace
	

	//Erase files from folder:
erase "$path_out/girl_nutri_mdv_z.xls"
erase "$path_out/girl_nutri_mdv_prev.xls"
erase "$path_out/girl_nutri_mdv_z.dta"
...
use "$path_in/MVMR71FL.DTA", clear 
...
codebook mv201 mv206 mv207,tab (999)
	//Men 15-49 years have information on child mortality
...
sort ind_id
save "$path_out/MDV16-17_MR.dta", replace
...
tab hb13 if hv105>=15 & hv105<=19 & hv104==1, miss 
	//Total number of boys 15-19 years with anthropometric data: 1,225 
tab hb13 hv118 if hv105>=15 & hv105<=19 & hv104==1, miss
tab hb13 hv103 if hv105>=15 & hv105<=19 & hv104==1, miss
	/*25 of the 1,225 men 15-19 years are identified as non-eligible
...
*** Keep relevant sample		
keep if hv105>=15 & hv105<=19 & hv104==1 
count
	//Total boys 15-19 years: 1,225
...
/* We use datalab to specify the name that will prefix the output files that 
will be produced from using this ado file*/
gen str30 datalab = "boy_nutri_mdv" 
lab var datalab "Working file"
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores to compute BMI-for-age*/
use "$path_out/boy_nutri_mdv_z.dta", clear 

	
gen	z_bmi = _zbfa
replace z_bmi = . if _fbfa==1 
	/*Maldives DHS 2016-17: 1 boy 15-19 years were replaced as missing 
...
sort ind_id
save "$path_out/MDV16-17_PR_boys.dta", replace


	//Erase files from folder:
erase "$path_out/boy_nutri_mdv_z.xls"
erase "$path_out/boy_nutri_mdv_prev.xls"
erase "$path_out/boy_nutri_mdv_z.dta"
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MDV16-17_BR.dta"
drop _merge
erase "$path_out/MDV16-17_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MDV16-17_IR.dta"
tab women_IR hv117, miss col
tab ha65 if hv117==1 & women_IR ==., miss 
	//Total number of eligible women not interviewed
drop _merge
erase "$path_out/MDV16-17_IR.dta"
...
tab v201 ha65 if hv117==1, miss
	/*Note: Some 16% eligible women did not provide information on their birth 
...
*****************************************
merge 1:1 ind_id using "$path_out/MDV16-17_PR_girls.dta"
tab ha13 girl_PR if hv105>=15 & hv105<=19 & hv104==2, miss col
drop _merge
erase "$path_out/MDV16-17_PR_girls.dta"		
	
	
*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MDV16-17_MR.dta"
tab men_MR hv118, miss col
tab hb65 if hv118==1 & men_MR ==. 
	//Total of eligible men not interviewed
drop _merge
erase "$path_out/MDV16-17_MR.dta"


*** Merging 15-19 years: boys 
*****************************************
merge 1:1 ind_id using "$path_out/MDV16-17_PR_boys.dta"
tab hb13 boy_PR if hv105>=15 & hv105<=19 & hv104==1, miss col
drop _merge
erase "$path_out/MDV16-17_PR_boys.dta"


*** Merging child under 5 
*****************************************
merge 1:1 ind_id using "$path_out/MDV16-17_PR_child.dta"
tab hv120, miss  
tab hc13 if hv120==1, miss
drop _merge
erase "$path_out/MDV16-17_PR_child.dta"
...
drop if resident!=1 
tab resident, miss
	/* Maldives DHS 2016-17: 400 (1.22%) individuals who were 
...
Maldives DHS 2016-17: height and weight measurements was collected from women 
and men 15-49 and children 0-5 living in all households sampled in the survey. 
...
***********************************************
tab hb13, miss
tab hb13 if hv105>=15 & hv105<=49 & hv104==1, miss
...
//Area: urban or rural	
	/* In the Maldives, there is no urban-rural designation for reidential 
	households. In place of urban-rural, for this survey, the residence variable 
	was defined as Malé region and other atolls. This corresponds with the 
	urban-rural residence categories used in the 2009 survey. Households 
	labelled as “urban” in 2009 are equivalent to those labelled “Malé region” 
	in 2016-17 and those labelled as “rural" in 2009 are labelled as 
	“other atolls” in this survey (p.2). */
desc hv025
...
tab hv115 hv120, miss
recode marital (0=1)(1=2)(.=1)
...
//Subnational region
	/*Maldives DHS 2016-17 (p.2): The sample is designed to yield representative 
	information for most indicators for the country as a whole, for residence,
	and for each of Maldives’s six regions. */  
...
tab no_missing_edu, miss
	//Maldives DHS 2016-17: Some 2.5% individuals lack data on years of edu  
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Maldives, the official school entrance age to primary school is 
	6 years. So, age range is 6-14 (=6+8) 
	Source: Country report (p.16) primary entrance age
	*/
...
foreach var in ha40 hb40 {
			 ...
			 /* Maldives DHS 2016-17: Anthropometric data for women and 
			 men 15-49 years */
			 drop inf_`var'
			 }
***
...
gen	water_mdg = 1 if water==11 | water==12 | water==14 | water==31 | ///
					 water==51 | water==52 | water==71  
...
replace water_mdg = 0 if water==32  | water==96 				
	//Deprived if it is unprotected well, other. 
...
replace roof_imp = 0 if roof<=12 | roof==96  
	//Deprived if no roof, thatch/palm leaf, other		
...
codebook hv208 sh121d 
clonevar television = hv208 
lab var television "Household has television"	
replace television=1 if television==0 & sh121d==1
replace television=1 if television==. & sh121d==1	
...
lab var animal_cart "Household has animal cart"	
	//Maldives DHS 2016-17: no data on animal cart
...
/*The high missing value observed for the nutrition indicator overlaps with the 
report. According to the Maldives DHS 2016-17 report, some 32% of children under 
age 5 were missing either height or weight (p.155). The response rates for the 
anthropometric measurements for women and men were quite low. Of all the women 
and men age 15-49 in the surveyed households, valid heights and weights were 
obtained for only about 75% of women and 55% of men. The main reason for 
non-response is that the individual was not home. A sizeable proportion of 
respondents refused to be measured (p.162). */ 
 
 
*** Generate coutry and survey details for estimation ***
char _dta[cty] "Maldives"
char _dta[ccty] "MDV"
...
save "$path_out/mdv_dhs16-17.dta", replace 
