...
2019 Global Multidimensional Poverty Index - Tunisia MICS 2011-12 [STATA do-file]. 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Tunisia_2012_MICS_Datasets
...
/*Tunisia MICS 2011-12: Anthropometric data was collected only for childen 
under 5 years as indicated by the questionnaire for children under-5 that was 
administered to mothers (or caretakers) (p.21) */
...
gen str30 datalab = "children_nutri_tun" 
...
use "$path_out/children_nutri_tun_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: In the context of Tunisia MICS 2011-12, 66 children were replaced 
...
sort ind_id
save "$path_out/tun11-12_CH.dta", replace


	//Erase files from folder:	
erase "$path_out/children_nutri_tun_z_rc.xls"
erase "$path_out/children_nutri_tun_prev_rc.xls"
erase "$path_out/children_nutri_tun_z_rc.dta"
...
label var ind_id "Individual ID"
	...
	//Generated age at death
desc bh4c bh9*	
gen bh9c     = bh9n           if bh9u==2
replace bh9c = bh9n/30.416667 if bh9u==1
replace bh9c = bh9n*12        if bh9u==3
...
sort ind_id
save "$path_out/tun11-12_BH.dta", replace	
...
tab cm1 cm8, miss  
	...
	child mortality
	Note: In Tunisia 2011-12, never married women do not have information about 
		  births or mortality*/
...
sort ind_id
save "$path_out/tun11-12_WM.dta", replace
...
/* Note: There is no male data file for Tunisia MICS 2011-12. Hence this section 
...
save "$path_out/tun11-12_HH.dta", replace
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/tun11-12_BH.dta"
drop _merge
erase "$path_out/tun11-12_BH.dta" 
 
 
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/tun11-12_WM.dta"
tab hl7, miss 
gen temp = (hl7>0) 
...
drop temp
drop _merge
erase "$path_out/tun11-12_WM.dta"


*** Merging HH Recode 
*****************************************
merge m:1 hh_id using "$path_out/tun11-12_HH.dta"
tab hh9 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed 
drop _merge
erase "$path_out/tun11-12_HH.dta"


*** Merging MN Recode 
*****************************************
	//There is no male data file for Tunisia MICS 2011-12 
...
*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/tun11-12_CH.dta"
drop _merge
erase "$path_out/tun11-12_CH.dta"
...
//Subnational region
	/*Sample for Tunisia MICS 2011-12 was designed to provide estimates on a 
	large number of indicators concerning the situation of children and women 
	at national level, for urban and rural areas, for the big six socioeconomic 
	regions of the country (Grand Tunis, North East, North West, Central East, 
	South East, South West) and for each of the three West Central 
	governorates (p.21) */
codebook hh7, tab (99)
decode hh7, gen(temp)
...
3 years of compulsory secondary, and 
...
Lower secondary education takes place from age 12-15 (grades 7-9). 
Upper secondary education takes place from age 15-19(grades 10-13).
...
replace eduyears = eduhighyear + 9 if (edulevel==3) 
	/*Higher secondary after 9 years of education: 6 years of primary + 3 years 
	of lower secondary. */
replace eduyears = eduhighyear + 13 if (edulevel==4) 
	/*We assume diploma, bachelor, and higher come after 13 years of 
	school (primary and secondary) */
replace eduyears =10 if ed4a==5 & ed4b==1 
	//format. prof cap 
replace eduyears =11 if ed4a==5 & ed4b==2 
	//format. prof cap
replace eduyears =12 if ed4a==6 & ed4b==1 
	//format. prof btp 
replace eduyears =13 if ed4a==6 & ed4b==2 
	//format. prof btp 
replace eduyears =14 if ed4a==7 & ed4b==1 
	//format. prof bts 
replace eduyears =15 if ed4a==7 & ed4b==2 
	//format. prof bts 
	
/* Vocational training diplomas
Speciality courses offered by vocational training centres are sanctioned by 
approved professional diplomas:

Certificate of Vocational Aptitude (CAP): 
Enrolment: having completed junior high school studies (9 years of basic 
education) or equivalent.
 
Diploma of Vocational Technician (BTP): 
Enrolment: having a Certificate of Vocational Aptitude (CAP) in the specialty 
or completed  two years in  High school.
 
Diploma of Higher Technician (BTS):
Enrolment: having a Diploma of Vocational Technician (BTP) in the specialty  
or completed  High school studies or equivalent. */
...
gen child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Tunisia, the official school entrance age for primary school is 
...
drop temp_f
	...
	/* In the case of Tunisia, this variable takes missing value because the 
...
somewhere else' are considered as non-improved. However, in Tunisia, 'flush 
don't know where'is identified as improved sanitation facilities (Table WS.5, p.68). 
So in this case we have followed the country report and identified 'flush do 
not know where' as improved, while 'flush to somewhere else' are considered as 
non-improved. In addition, the report considers the category open defecation as 
neither improve nor non-improved. We follow the standard set in the MDGs, where 
this is clearly a deprived standard */


gen toilet_mdg = 1 if (toilet==11 | toilet==12 | toilet==13 | ///
					   toilet==15 | toilet==21 | toilet==22) & shared_toilet!=1
...
save "$path_out/tun_mics11-12.dta", replace 
