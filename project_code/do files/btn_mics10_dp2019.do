...
2019 Global Multidimensional Poverty Index - Bhutan MICS 2010 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Bhutan_2010_MICS_Datasets
...
/*Bhutan MICS 2010: Anthropometric data was collected only for childen under 
5 years. */
...
gen str30 datalab = "children_nutri_btn" 
...
standards */
use "$path_out/children_nutri_btn_z_rc.dta", clear 
...
erase "$path_out/children_nutri_btn_z_rc.xls"
erase "$path_out/children_nutri_btn_prev_rc.xls"
erase "$path_out/children_nutri_btn_z_rc.dta"
...
********************************************************************************
/* Note: There is no no birth history data file for Bhutan MICS 2010. Hence 
...
/* Note: There is no male data file for Bhutan MICS 2010. Hence this section has 
been deactivated */
...
duplicates tag hh_id, gen(duplicates)
...
*** Merging BR Recode 
*****************************************
	//There is no birth history data file for Bhutan MICS 2010  
...
replace hh_id = hh1*100 + hh2 if ln==0 
	//hd_id for children without household line
...
//Subnational region 
	/*Note: The sample for the Bhutan MICS 2010 was designed to provide 
	estimates for a large number of indicators on the situation of women and 
	children, for urban and rural areas, for three Regions and 20 Dzongkhags 
	(districts). Owing to a smaller base population in Gasa, only 200 households 
	were taken for the survey compared to 800 households in other Dzongkhags; 
	thus, the results pertaining to this Dzongkhag should be treated with 
	caution (p.5). Following this note in the report, we consulted the national 
	statistics office in Bhutan. We were advised to combine the district of 
	Gasa and Punakha.*/
...
label define lab_reg 11"Bumthang" 12"Chukha" 13"Dagana" 15"Haa" ///
16"Lhuntse" 17"Mongar" 18"Paro" 19"Pemagatshel" 20"Gasa and Punakha" ///
21"Samdrup Jongkhar" 22"Samtse" 23"Sarpang" 24"Thimphu" 25"Trashigang" ///
26"Trashiyangtse" 27"Trongsa" 28"Tsirang" 29"Wangdue Phodrang" 30"Zhemgang"
...
********************************************************************************
	/*NOTE: Currently the 2005 education model holds in Bhutan. Information on 
	basic education is not given in UNESCO's website. Duration by education 
	levels : 7 years of compulsory primary, 4 years of lower secondary, and 2 
	years of upper secondary. The admission age to compulsory education is 6 
	years. Primary education takes place from age 6-13(grades 1-7). Lower 
	secondary education takes place from age 13-17 (grades 8-11). Upper 
	secondary education takes place from age 17-19(grades 12-13). Reference: 
	http://www.ibe.unesco.org/fileadmin/user_upload/Publications/WDE/2010/pdf-versions/Mongolia.pdf */

tab ed4b ed4a, miss
tab age ed6a if ed5==1, miss
	/*In the case of Bhutan MICS 2010, there is inconsistency such as 
...
replace eduyears = 0 if eduyears == 17 
	//eduyears == 17 is no grade
...
gen	child_schoolage = (age>=6 & age<=14)
	/*Note: In Bhutan, the official school entrance age is not mentioned on the 
	UNESCO website. However, the country report indicate that the offcial 
	entrance age for primary schooling is 6 years (p.149). 
...
********************************************************************************
	//NOTE: Bhutan MICS 2010: No information on child mortality from men 
...
************************************************************************
	/*In the case of Bhutan, there is no birth history data. This means, there 
...
********************************************************************
	/*Note: In the case of Bhutan, the category 'flush to unknown place' is 
	identified as imporved sanitation facility, while the category 'flush to 
	somewhere else' is identified as unimproved facility (Table WS.5, p.88).
	The coding in this dofile follows the report. However, everyone who reported
	'flush to unknown place' was using shared facility, hence by this criteria, 
	they were replaced as having non-improved sanitation facility*/
...
*** Rename key global MPI indicators for estimation ***
	/* Note: In the case of Bhutan MICS 2010, there is no birth history file. 
...
save "$path_out/btn_mics10.dta", replace 
