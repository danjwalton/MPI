...
2019 Global Multidimensional Poverty Index -  Yemen DHS 2013 [STATA do-file]. 
...
/* Yemen DHS 2013: "For the main survey, four questionnaires were used: a 
household questionnaire, two individual questionnaires (one for ever-married 
women and an abbreviated version for never-married women), and a maternal 
mortality questionnaire." (p.2)

"Anthropometry. In all households, height and weight measurements were recorded 
for children age 0-59 months and women age 15-49 years (p. 3). */
...
use "$path_in/YEKR61FL.DTA", clear 
...
drop if b5==0 
	/*Children who are not alive are excluded. 710 observations deleted. */
...
gen str30 datalab = "children_nutri_yem" 
...
standards */
use "$path_out/children_nutri_yem_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: In the context of Yemen DHS 2013, 386 children were replaced 
...
sort ind_id
duplicates report ind_id
save "$path_out/yem13_KR.dta", replace



	//Erase files from folder:
erase "$path_out/children_nutri_yem_z_rc.xls"
erase "$path_out/children_nutri_yem_prev_rc.xls"
erase "$path_out/children_nutri_yem_z_rc.dta"
...
use "$path_in/YEBR61FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Yemen DHS 2013: these figures are identical
...
sort ind_id
save "$path_out/yem13_BR.dta", replace	
...
use "$path_in/YEIR61FL.DTA", clear
...
gen women_IR=1 
	...
	//Retain relevant variables:
keep ind_id women_IR v003 v005 v012 v201 v206 v207 v501 
order ind_id women_IR v003 v005 v012 v201 v206 v207 v501 
sort ind_id
save "$path_out/yem13_IR.dta", replace
...
count if v012>=15 & v012<=19
	//Total number of girls in the IR recode: 6,415
...
gen str30 datalab = "girl_nutri_yem" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_yem_z.dta", clear 
...
sort ind_id
save "$path_out/yem13_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_yem_z.xls"
erase "$path_out/girl_nutri_yem_prev.xls"
erase "$path_out/girl_nutri_yem_z.dta"
...
********************************************************************************
	...
	/*Note: There is no male recode file for Yemen DHS 2013. Hence the 
...
use "$path_in/YEPR61FL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/yem13_BR.dta"

drop _merge

erase "$path_out/yem13_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/yem13_IR.dta"

tab women_IR hv117, miss col
tab ha65 if hv117==1 & women_IR==., miss 
	//Total number of eligible women not interviewed: 382 
tab ha65 ha13 if women_IR== . & hv117==1, miss   

drop _merge

erase "$path_out/yem13_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/yem13_IR_girls.dta"

tab teen_IR hv117 if hv105>=15 & hv105<=19, miss col
tab ha65 if hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss 
	//Total number of eligible girls not interviewed: 489
tab ha65 ha13 if hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss 
tab ha40 if ha65==1 & ha13==0 & hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Yemen DHS 2013, 54 girls 15-19 were identified as measured and 
	are present in the PR recode, but they were not present in the IR recode. 
	It should be noted that they have BMI information but not BMI-for-age.*/
drop _merge

erase "$path_out/yem13_IR_girls.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out\yem13_KR.dta"
...
tab resident, miss
	...
	In the context of Yemen DHS 2013, 1,219 (1.01%) individuals who were 
...
/*
In the case of Yemen, all women 15-49 and all children under 5 were measured 
(p.3 of report). Thus, there is no presence of subsample for anthropometrics 
measures */
...
*****************************************
	/* All women 15-49 were eligible for anthropometric measurement. But only 
	ever-married women were eligible for child mortality questions. Thus, we 
	need two variables that differentiates the applicable populations for 
...
gen	fem_eligible_2 = (hv117==1 & (v501==1 | v501==3 | v501==4))
bysort hh_id: egen hh_n_fem_eligible_2 = sum(fem_eligible_2) 	
gen	no_fem_eligible_mort = (hh_n_fem_eligible_2==0) 									
lab var no_fem_eligible_mort "Household has no eligible women for child mortality"
tab no_fem_eligible_mort, miss



*** No Eligible Men 
*****************************************
	//Note: There is no male recode file for Yemen DHS 2013
...
*****************************************
	...
	data. However, in the case of Yemen, hemoglobine testing was only done for 
	1/3 of the households. This subsample DOES NOT represent the sample for 
	anthropometric measurement. ALL WOMEN 15-49 AND ALL CHILDREN under 5 were 
	measured (p.3 of report). */
gen no_hem_eligible = . 	
...
//Subnational region
	/*NOTE: The sample for the Yemen DHS 2013 was designed to provide 
	estimates at the national and governorate levels (p.2). Indicators will 
	also be shown for the Northern, Central, and Southern regions of the country. 
	So instead of "hv024" (that contains 4 regions), we use the "shhdist" 
	variable that contains 28 districts */ 
...
codebook hv108, tab(30)
	...
	/* Note: For Yemen DHS 2013, the standard hv108 (education completed in 
	single years) as well as other variables regarding years of schooling
	are not available. So we construct total number of years of education 
	using: sh17_a level of education attended and sh17_b highest grade 
	completed (years)*/

 /* Yemen
  Source: http://stats.uis.unesco.org/unesco/TableViewer/tableView.aspx?ReportId=163
...
gen eduyears = 0  if sh17_a==0 | (sh17_a==1 & sh17_b==0) | ///
					(sh17_a==2 & sh17_b==0) | sh16==0 
replace eduyears = 1  if sh17_a==1 & sh17_b==1
replace eduyears = 2  if sh17_a==1 & sh17_b==2
replace eduyears = 3  if sh17_a==1 & sh17_b==3
replace eduyears = 4  if sh17_a==1 & sh17_b==4
replace eduyears = 5  if sh17_a==1 & sh17_b==5
replace eduyears = 6  if sh17_a==1 & sh17_b==6

replace eduyears = 1  if sh17_a==2 & sh17_b==1
replace eduyears = 2  if sh17_a==2 & sh17_b==2
replace eduyears = 3  if sh17_a==2 & sh17_b==3
replace eduyears = 4  if sh17_a==2 & sh17_b==4
replace eduyears = 5  if sh17_a==2 & sh17_b==5
replace eduyears = 6  if sh17_a==2 & sh17_b==6
replace eduyears = 7  if sh17_a==2 & sh17_b==7
replace eduyears = 8  if sh17_a==2 & sh17_b==8
replace eduyears = 9  if sh17_a==2 & sh17_b==9

replace eduyears = 9  if sh17_a==3

replace eduyears = 6  if sh17_a==4 & sh17_b==0
replace eduyears = 10 if sh17_a==4 & sh17_b==1
replace eduyears = 11 if sh17_a==4 & sh17_b==2
replace eduyears = 12 if sh17_a==4 & sh17_b==3

replace eduyears = 12 if sh17_a==5

replace eduyears = 12 if sh17_a==6 & sh17_b==0
replace eduyears = 13 if sh17_a==6 & sh17_b==1
replace eduyears = 14 if sh17_a==6 & sh17_b==2
replace eduyears = 15 if sh17_a==6 & sh17_b==3
replace eduyears = 16 if sh17_a==6 & sh17_b==4
replace eduyears = 17 if sh17_a==6 & sh17_b==5
replace eduyears = 18 if sh17_a==6 & sh17_b==6
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Yemen, the official school entrance age is 6 years.  
...
********************************************************************************
	//Note: Yemen DHS 2013 has no anthropometric data for men
...
replace low_bmi_byage = 1 if f_low_bmi==1
	...
	available. In the case of Yemen DHS 2013, this information is exclusively 
...
replace hh_no_low_bmiage = 1 if no_fem_eligible==1
	...
	Note: In the case of Yemen DHS 2013, anthropometric data was collected 
...
********************************************************************************
	...
	/*In the context of Yemen DHS 2013, men did not provide information on 
	child mortality */
...
replace childu18_died_per_wom_5y = 0 if no_fem_eligible_mort==1 
	/*Assign a value of "0" for:
...
*** Standard MPI ***
****************************************
	//Note: In Yemen,the report considers the category latrine as non-improved. 
...
*** Standard MPI ***
****************************************
	/*In Yemen, the country report (Table 2.1, p.8) summarise that spring and 
	protected surface water are unimporved sources of drinking water. Generally 
	these are identified as improved sources. However, in the context of Yemen,
	if the water is fetched from a source that is not immediately accessible to 
	the household, it may be contaminated during transport or storage. Hence 
	following the report, we have also identified these sources as non-improved 
	drinking water. */
codebook water, tab(99)

gen	water_mdg = 1 if water==14 | water==15 | water==21 | water==51 | water==71  
...
replace water_mdg = 0 if water==32 | water==43 | water==44 | water==45 | ///
						 water==61 | water==72 | water==96 
...
replace water_mdg = 0 if water==71 & ///
						(ndwater==32 | ndwater==43 | ndwater==44 | ///
						 ndwater==45 | ndwater==61 | ndwater==72 | ndwater==96)
...
gen	floor_imp = 1
replace floor_imp = 0 if floor<=21 | floor==96  	
...
gen	wall_imp = 1 
replace wall_imp = 0 if wall<=21 | wall==96  	
...
Note: In Yemen, roofing material made of metal plates or a combination of 
metal plates with mud are identified as non-improved materials. */ 

clonevar roof = hv215
codebook roof, tab(99)		
gen	roof_imp = 1 
replace roof_imp = 0 if roof<=22 | roof==96  	
...
save "$path_out/yem_dhs13.dta", replace 
