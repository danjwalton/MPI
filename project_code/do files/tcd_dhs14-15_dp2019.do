...
2019 Global Multidimensional Poverty Index - Chad DHS 2014-15 [STATA do-file]. 
...
/*Chad DHS 2014-15: In a subsample of one in three households, all men aged 
15-59 were eligible to be surveyed. In two thirds of households, not selected 
...
use "$path_in/TDKR71FL.DTA", clear 
...
drop if b5==0 
	...
	//Chad DHS 2014-15: 1,722 observations deleted
...
gen str30 datalab = "children_nutri_tcd" 
...
replace weight = . if hw2>=9990 
	//Chad DHS 2014-15: 248 real changes made, 248 to missing
...
replace height = . if hw3>=9990 
	...
	//Chad DHS 2014-15: 292 real changes made, 292 to missing
...
standards */
use "$path_out/children_nutri_tcd_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Chad DHS 2014-15, 258 children were replaced as
...
erase "$path_out/children_nutri_tcd_z_rc.xls"
erase "$path_out/children_nutri_tcd_prev_rc.xls"
erase "$path_out/children_nutri_tcd_z_rc.dta"

	
	//Save a temp file for merging with PR:
save "$path_out/TCD14-15_KR.dta", replace
...
use "$path_in/TDBR71FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Chad DHS 2014-15: these figures are identical
...
sort ind_id
save "$path_out/TCD14-15_BR.dta", replace	
...
use "$path_in/TDIR71FL.DTA", clear
...
sort ind_id
save "$path_out/TCD14-15_IR.dta", replace
...
gen str30 datalab = "girl_nutri_tcd" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_tcd_z.dta", clear 
...
sort ind_id
	...
	//Erase files from folder:
erase "$path_out/girl_nutri_tcd_z.xls"
erase "$path_out/girl_nutri_tcd_prev.xls"
erase "$path_out/girl_nutri_tcd_z.dta"

	
	//Save a temp file for merging with PR:
save "$path_out/TCD14-15_IR_girls.dta", replace
...
use "$path_in/TDMR71FL.DTA", clear 
...
sort ind_id
save "$path_out/TCD14-15_MR.dta", replace
...
********************************************************************************
	/*Note: In the case of Chad DHS 2014-15, anthropometric data was NOT 
	collected for men. */
...
use "$path_in/TDPR71FL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TCD14-15_BR.dta" 

drop _merge

erase "$path_out/TCD14-15_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TCD14-15_IR.dta"
...
erase "$path_out/TCD14-15_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/TCD14-15_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13==0 & hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Chad DHS 2014-15, 73 girls 15-19 were identified as measured 
...
erase "$path_out/TCD14-15_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TCD14-15_MR.dta"
...
tab hb13 hb65 if hv118==1 & men_MR==., miss 
	//No anthropometric data for man in Chad DHS 2014-15

drop _merge

erase "$path_out/TCD14-15_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
	//No anthropometric data for men in Tchad DHS 2014-15
	

*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TCD14-15_KR.dta"
...
erase "$path_out/TCD14-15_KR.dta"
...
tab resident, miss
	...
	In the context of Chad DHS 2015-16, 645 (0.64%) individuals who were 
...
/*In the context of Chad DHS 2014-15, height and weight measurements were
collected from all women 15-49 years and all children under five living in 
2/3 of the sampled households which was not selected for the male survey (p.8).*/

codebook hv027, tab (10)
	/*hv027: Household selected for male interview. In this variable, '0' means 
	households that are not selected for male survey but were selected for 
	anthropometric measures*/
	
gen	subsample = (hv027==0)
replace subsample = . if hv027==.
...
*****************************************
	...
	data. In Chad DHS 2014-15, this variable has no observations. */
gen	no_hem_eligible = .
...
/*Note in the case of Chad DHS 2014-15, we recoded the 120 individuals who 
responded as living together as married */
...
//Subnational region
	/*NOTE: The sample for the Chad DHS 2014-15 was designed to provide 
	estimates of key indicators for the country as a whole, for urban and rural 
	areas separately, for the city of N'Djamena, and for 20 other regions. The 
	regions of Borkou and Tibesti have been grouped into one area of ​​study, 
	as well as the regions of Ennedi West and Ennedi East.*/
lookfor region
...
codebook region, tab (100)
	//relabel names that use the Latin alphabet
label define lab_reg 1"Barh El Gazal" 2"Batha" 3"Borkou/Tibesti" ///
4"Chari Baguirmi" 5"Ennedi Est & Ennedi Ouest" 6"Guéra" ///
7"Hadjer-Lamis" 8"Kanem" 9"Lac" ///
10"Logone Occidental" 11"Logone Oriental" 12"Mandoul" 13"Mayo Kebbi Est" ///
14"Mayo Kebbi Ouest" 15"Moyen Chari" 16"N’Djaména" 17"Ouaddaï" 18"Salamat" ///
19"Sila" 20"Tandjilé" 21"Wadi Fira"
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Chad, the official school entrance age is 6 years.  
...
********************************************************************************
//Note: Chad DHS 2014-15 has no anthropometric data for men. 
...
foreach var in ha40 hb40 {
			 ...
			 tab fem_eligible inf_`var'
			 drop inf_`var'
			 }

***
	/* NOTE: In Chad DHS 2014-15, anthropometric data was collected from 171 
	women despite that they were not eligible for the interview. Following the 
	sampling procedure, these women are replaced as non-eligible in the final 
	nutrition indicator. */ 
drop fem_eligible
...
lab var m_bmi "Male's BMI"
	//Chad DHS 2014-15 has no anthropometric data for men 
...
replace hh_no_low_bmiage = 1 if no_fem_eligible==1
	/*Households take a value of '1' if there is no eligible population. In 
	cases where there is BMI & BMI-for-age information from women and men, then 
...
replace toilet_mdg = 0 if toilet == 13 | toilet == 14
	/*Household is assigned a value of '0' if it uses non-improved sanitation: 
	"flush to somewhere else" and "flush don't know where" 
	
	Note: In the case of Chad, flush to somewhere else is coded as '13', which 
	differs from the standard DHS coding */
...
clonevar ndwater = hv202  
	/*There is very small number of observation for non-drinking water for 
	Chad DHS 2014-15 */
...
save "$path_out/tcd_dhs14-15.dta", replace 
