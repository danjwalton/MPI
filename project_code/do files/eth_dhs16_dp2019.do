...
2019 Global Multidimensional Poverty Index - Ethiopia DHS 2016 [STATA do-file]. 
...
/*Ethiopia DHS 2016: Height and weight measurements were collected from all 
children age 0-59 months, women age 15-49, and men age 15-59 (p.2-3) who were 
sampled as part of the survey */
...
use "$path_in/ETPR70FL.DTA", clear 
...
tab hv120,miss	
	/*10,946 children under 5 are eligible for anthropometric 
	measurement. */	
count if hc1!=.
	//All 10,946 children under 5 have information on age in months
tab hv105 if hc1!=.
	/*A cross check with the age in years reveal that all are within the 5 year 
	age group */
tab hc13 if hv120==1, miss
	//9,854 (90.02%) of the 10,946 children have been measured
tab hc13 if hc3<=9990, miss
tab hc13 if hc2<=9990, miss
	/*For example, height and weight data is available for all 9,854 children 
...
count if _fwei==1 | _flen==1 	
	/* 139 children were replaced as missing because they have extreme 
...
sort ind_id
save "$path_out/ETH16_PR_child.dta", replace
...
use "$path_in/ETBR70FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Ethiopia DHS 2016: these figures are identical
...
use "$path_in/ETIR70FL.DTA", clear
...
codebook v201 v206 v207,tab (999)
	/*Ethiopia DHS 2016: Fertility and mortality question 
...
tab ha13 if hv105>=15 & hv105<=19 & hv104==2, miss
	...
	were selected for the biomarker questionnaire: 3,833 */
tab ha13 hv117 if hv105>=15 & hv105<=19 & hv104==2, miss
tab ha13 hv103 if hv105>=15 & hv105<=19 & hv104==2, miss
	/*98 of the 3,833 women 15-19 years are identified as non-eligible
...
replace z_bmi = . if _fbfa==1 
	/*Ethiopia DHS 2016: 1 girls 15-19 years were replaced as missing 
...
sort ind_id
save "$path_out/ETH16_PR_girls.dta", replace
...
use "$path_in/ETMR70FL.DTA", clear 
...
codebook mv201 mv206 mv207,tab (999)
	/*Ethiopia DHS 2016: Fertility and mortality question 
	was only collected from men 15-59 years.*/
...
tab hb13 if hv105>=15 & hv105<=19 & hv104==1, miss
	...
	were selected for the biomarker questionnaire: 3,135 */
tab hb13 hv118 if hv105>=15 & hv105<=19 & hv104==1, miss
tab hb13 hv103 if hv105>=15 & hv105<=19 & hv104==1, miss
	/* 151 of the 3,135 men 15-19 years are identified as non-eligible
...
gen	z_bmi = _zbfa
replace z_bmi = . if _fbfa==1 
	/*Ethiopia DHS 2016: 2 boys 15-19 years were replaced as missing 
	because she has extreme z-scores which are biologically implausible. */
lab var z_bmi "z-score bmi-for-age WHO"
...
sort ind_id
save "$path_out/ETH16_PR_boys.dta", replace	
...
tab v201 ha65 if hv117==1, miss
	/*Note: Some 5.43% eligible women did not provide information on their birth 
...
*****************************************
merge 1:1 ind_id using "$path_out/ETH16_PR_girls.dta"
tab ha13 girl_PR if hv105>=15 & hv105<=19 & hv104==2, miss col
drop _merge
erase "$path_out/ETH16_PR_girls.dta"	
...
*****************************************
merge 1:1 ind_id using "$path_out/ETH16_PR_boys.dta"
tab hb13 boy_PR if hv105>=15 & hv105<=19 & hv104==1, miss col
drop _merge
erase "$path_out/ETH16_PR_boys.dta"	


*** Merging child under 5 
*****************************************
merge 1:1 ind_id using "$path_out/ETH16_PR_child.dta"
tab hv120, miss  
tab hc13 if hv120==1, miss
drop _merge
erase "$path_out/ETH16_PR_child.dta"
...
tab resident, miss
	...
	In the context of Ethiopia DHS 2016, 1,323 (1.76%) individuals who were 
...
/*
In the context of Ethiopia DHS 2016, height and weight measurements were
collected from all children (0-5), women (15-49) and men (15-59) in the sampled 
households. As such there is no any selection of subsample for nutrition in this 
survey. 
*/
...
***********************************************
tab hb13 if hv105>=15 & hv105<=54 & hv104==1, miss
...
//Subnational region
	/*NOTE: The sample for the Ethiopia DHS 2016, is divided into nine 
	geographical regions and two administrative cities (p.333). The survey 
	sample is representative at the national and regional levels, and for urban 
	and rural areas (p.32)*/ 
lookfor region
...
gen	child_schoolage = (age>=7 & age<=15)
	/*Note: In Ethiopia DHS 2016, the official school entrance age is 7 years.  
...
********************************************************************************
/*Note: Ethiopia DHS 2016 has anthropometric data for women and men*/

codebook ha40 hb40 
foreach var in ha40 hb40 {
			 gen inf_`var' = 1 if `var'!=.
			 bysort sex: tab age inf_`var' 
			 /* Ethiopia DHS 2016 has anthropometric data for women 
			 15-49 years & men 15-59 years*/
			 drop inf_`var'
			 }
***
...
clonevar ndwater = hv202  
	//Non-drinking water: there is observation	
...
save "$path_out/eth_dhs16.dta", replace 
