...
2019 Global Multidimensional Poverty Index - Ghana DHS 2014 [STATA do-file]. 
...
/*Ghana DHS 2014: Anthropometric information were recorded for all eligible 
women 15-49, men 15-59, and children 0-5 in a subsample of 1/2 of households 
(p.6) */ 
...
use "$path_in/GHKR72FL.DTA", clear 
...
drop if b5==0
	...
	//Ghana DHS 2014: 289 observations deleted
...
gen str30 datalab = "children_nutri_gha" 
...
use "$path_out/children_nutri_gha_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Ghana DHS 2014, 10 children were replaced as
...
sort ind_id
save "$path_out/GHA14_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_gha_z_rc.xls"
erase "$path_out/children_nutri_gha_prev_rc.xls"
erase "$path_out/children_nutri_gha_z_rc.dta"
...
compare tot_child_died tot_child_died_2
	//Ghana DHS 2014: these figures are identical
...
sort ind_id
save "$path_out/GHA14_BR.dta", replace	
...
sort  ind_id
save "$path_out/GHA14_IR.dta", replace
...
count if v012>=15 & v012<=19
	//Total number of girls in the IR recode: 1,756	
...
gen str30 datalab = "girl_nutri_gha" 
lab var datalab "Working file"
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores */
use "$path_out/girl_nutri_gha_z.dta", clear 
...
sort ind_id
save "$path_out/GHA14_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_gha_z.xls"
erase "$path_out/girl_nutri_gha_prev.xls"
erase "$path_out/girl_nutri_gha_z.dta"
...
sort ind_id
save "$path_out/GHA14_MR.dta", replace

********************************************************************************
*** Step 1.6a  MR - MEN'S RECODE  
***(Boys 15-19 years in the household) 
********************************************************************************
/*Note: In the case of Ghana DHS 2014, anthropometric data was collected 
...
count if hv104==1 & hv105>=15 & hv105<=19
	//Total number of boys 15-19 years in the PR recode: 2,071
...
count if mv012>=15 & mv012<=19
	//Total number of boys in the MR recode: 878 boys
...
/* We use datalab to specify the name that will prefix the output files that 
will be produced from using this ado file*/
gen str30 datalab = "boy_nutri_gha" 
lab var datalab "Working file"	
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores */
use "$path_out/boy_nutri_gha_z.dta", clear 
...
sort ind_id
	...
	//Erase files from folder:
erase "$path_out/boy_nutri_gha_z.xls"
erase "$path_out/boy_nutri_gha_prev.xls"
erase "$path_out/temp.dta"
erase "$path_out/boy_nutri_gha_z.dta"


	//Save a temp file for merging with PR:
save "$path_out/GHA14_MR_boys.dta", replace
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GHA14_BR.dta"

drop _merge

erase "$path_out/GHA14_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GHA14_IR.dta"

tab women_IR hv117, miss col
tab ha65 if hv117==1 & women_IR ==.,m 
	//Total number of eligible women not interviewed
tab ha65 ha13 if women_IR == . & hv117==1,m   

drop _merge

erase "$path_out/GHA14_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/GHA14_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Ghana DHS 2014, 16 girls 15-19 were identified as measured 
...
erase "$path_out/GHA14_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GHA14_MR.dta"
...
tab hb13 hb65 if hv118==1 & men_MR ==., miss 
	//In Ghana DHS 2014, 221 men who were eligible were not interviewed, but none of them have anthropometric data

drop _merge

erase "$path_out/GHA14_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
merge 1:1 ind_id using "$path_out/GHA14_MR_boys.dta"
...
tab hb13 if teen_MR == . & hv118==1 & (hv105>=15 & hv105<=19), miss  
	//In Ghana DHS 2014, 16 boys have anthropometric measurements but are not in the MR file
	
drop _merge

erase "$path_out/GHA14_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GHA14_KR.dta"
...
drop if resident!=1 
	...
	In the context of Ghana DHS 2014, 654 (1.48%) individuals who were 
...
/*
In the context of Ghana DHS 2014, height and weight measurements were
collected from children (0-5), women (15-49), and men (15-59) living in 1/2 of 
...
adult men 15-59 years. These households will not have information on relevant 
...
//Subnational region
	/*NOTE: The sample for the Ghana DHS 2014 was designed to provide 
	estimates of key indicators for the country as a whole, for urban and rural 
	areas separately, and for each of the 10 administrative regions (p.5).*/ 
...
********************************************************************************
/*Note: Ghana DHS 2014: Anthropometric information were recorded for all 
eligible women 15-49, men 15-59, and children 0-5 in a subsample of 1/2 
of households */
...
recode shared_toilet (2=1)
	/* In Ghana DHS 2014, the coding for hv225 was 0=no; 1=yes, other household 
	only; and 2=yes, public. According to the survey report, both 1 and 2 are
	considered 'shared' and thus deprived (p. 13) */
...
/*In Ghana,the report considers the category other (96) as neither improve 
nor non-improved. But we consider it as non-improved to be consistent with the 
...
clonevar ndwater = hv202  
	/* In Ghana DHS 2014, variable hv202 exists, but all observations are 
	missing. The survey report says that "to ensure consistency with 2008 GDHS 
	findings, bottled/sachet water is considered as improved (p.13) */
...
gen	water_mdg = 1 if water==11 | water==12 | water==13 | ///
					 water==21 | water==31 | water==41 | ///
					 water==51 | water==71 | water==72
	...
	/* In Ghana, there is an additional category, 72=="sachet water". According 
	to the survey report (p.13), this is considered improved */
...
clonevar bw_television = sh110d
	/* Ghana DHS 2014: sh110d is "has a black/white television" and hv208 is 
...
replace telephone=1 if telephone==. & mobiletelephone==1
	...
	//Combine information on coloured tv and black and white tv 
...
save "$path_out/gha_dhs14.dta", replace 
