...
2019 Global Multidimensional Poverty Index - Egypt DHS 2014 [STATA do-file]. 
...
/*Egypt DHS 2014: Anthropometric data is recorded for every EVER-MARRIED woman 
aged 15-49, children 0-5 and never married youth 5-19 years in all the sampled 
households (p.172). Thus there is no presence of subsample.*/
...
use "$path_in/EGKR61FL.DTA", clear 
...
count if v135==1
	//Children who are usual residents
...
gen str30 datalab = "children_nutri_egy" 
...
standards */
use "$path_out/children_nutri_egy_z_rc.dta", clear 
...
sort ind_id
duplicates report ind_id
save "$path_out/EGY14_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_egy_z_rc.xls"
erase "$path_out/children_nutri_egy_prev_rc.xls"
erase "$path_out/children_nutri_egy_z_rc.dta"
...
Note: In Egypt DHS 2014, only ever-married women have birth information */


use "$path_in/EGBR61FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Egypt DHS 2014: these figures are identical
...
sort ind_id
save "$path_out/EGY14_BR.dta", replace	

	
********************************************************************************
*** Step 1.3  IR - EVER MARRIED WOMEN's RECODE  
*** (All eligible females 15-49 years in the household)
********************************************************************************

use "$path_in/EGIR61FL.DTA", clear
...
sort ind_id
save "$path_out/EGY14_IR.dta", replace	



********************************************************************************
*** Step 1.4  IR - EVER MARRIED WOMEN'S RECODE  
*** (Young married women 15-19 in the household)
********************************************************************************
...
lab var age_month "Age in months, married women 15-19 years"	
...
gen str30 datalab = "girl_nutri_egy" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_egy_z.dta", clear 
...
gen ywom_IR = 1  
	...
	//Retain relevant variables:	
keep ind_id ywom_IR age_month low_bmiage*
order ind_id ywom_IR age_month low_bmiage*
sort ind_id
save "$path_out/EGY14_ywom.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_egy_z.xls"
erase "$path_out/girl_nutri_egy_prev.xls"
erase "$path_out/girl_nutri_egy_z.dta"


********************************************************************************
*** Step 1.5  NEVER MARRIED YOUTH 
*** (Girls & Boys: 5 - 19 years in the household)
********************************************************************************
	/*Egypt DHS 2014 collected nutrition data from youth aged 5-19 years. As 
	such we will compute the BMI-for-age for this group of individuals*/

use "$path_in/EGPR61FL.DTA", clear
...
//Note: Keep only youth between age 5-19 years

desc shy1
	//Youth's age in years
count if shy1>=5 & shy1<=19
	//Total number of youth: 35,701	
keep if shy1>=5 & shy1<=19	
...
tab shy3a, miss 
	//"1" for male ;"2" for female
tab shy3a, nol 
clonevar gender = shy3a
desc gender
tab gender


*** Variable: AGE IN MONTHS ***	
gen age_month = (hv008 - shy32)
lab var age_month "Age in months, individuals 5-19 years"
sum age_month 
...
*** Variable: BODY WEIGHT (KILOGRAMS) ***
codebook shy2, tab (1000)
	//shy2: youth's weight in kilograms
gen weight = shy2/10
	//We divide it by 10 in order to express it in kilograms
replace weight = . if shy2>=9990 
	//All missing values or out of range are replaced as "."
summ weight


*** Variable: HEIGHT (CENTIMETERS)
codebook shy3, tab (10000)
	//shy3: youth's height in kilograms
gen	height = shy3/10 
	//We divide it by 10 in order to express it in centimeters
replace height = . if shy3>=9990 
	//All missing values or out of range are replaced as "."
summ height
...
/* We use datalab to specify the name that will prefix the output files that 
will be produced from using this ado file*/
gen str30 datalab = "youth_nutri_egy" 
lab var datalab "Working file"
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores to compute BMI-for-age*/
use "$path_out/youth_nutri_egy_z.dta", clear 
...
*** Standard MPI indicator ***
gen	low_bmiage_youth = (z_bmi < -2.0) 
	/*Takes value 1 if BMI-for-age is under 2 stdev below the median & 0 
	otherwise */
replace low_bmiage_youth = . if z_bmi==.
lab var low_bmiage_youth "Youth (5-19) low bmi 2sd - WHO"


gen youth_PR = 1 
	//Identification variable for observations in Youth PR recode (5-19 years)	


	//Retain relevant variables:	
keep ind_id youth_PR age_month low_bmiage_youth*
order ind_id youth_PR age_month low_bmiage_youth*
sort ind_id
save "$path_out/EGY14_youth.dta", replace


	//Erase files from folder:
erase "$path_out/youth_nutri_egy_z.xls"
erase "$path_out/youth_nutri_egy_prev.xls"
erase "$path_out/youth_nutri_egy_z.dta"
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/EGY14_BR.dta"

drop _merge

erase "$path_out/EGY14_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/EGY14_IR.dta"
...
erase "$path_out/EGY14_IR.dta"


*** Merging IR Recode: 15-19 years young married women
*****************************************
merge 1:1 ind_id using "$path_out/EGY14_ywom.dta"

tab ywom_IR hv117 if hv105>=15 & hv105<=19, miss col
tab ha65 if hv117==1 & ywom_IR==. & (hv105>=15 & hv105<=19), miss 
	//Total number of eligible girls not interviewed
tab ha65 ha13 if hv117==1 & ywom_IR==. & (hv105>=15 & hv105<=19), miss 
tab ha40 if ha65==1 & ha13==0 & hv117==1 & ywom_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Egypt DHS 2014, 14 young married women 15-19 were identified as 
	measured and are present in the PR recode, but they were not present in the 
	IR recode. It should be noted that they have BMI information but not 
	BMI-for-age.*/
drop _merge

erase "$path_out/EGY14_ywom.dta"


*** Merging PR Recode: 5-19 years never-married youths 
*****************************************
merge 1:1 ind_id using "$path_out/EGY14_youth.dta"

tab youth_PR if shy1>=5 & shy1<=19, miss 
tab shy13 if youth_PR==. & (shy1>=5 & shy1<=19), miss 
	//Total number of eligible youth not measured: 0 individuals

drop _merge

erase "$path_out/EGY14_youth.dta"



*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/EGY14_KR.dta"
...
tab resident, miss
	...
	In the context of Egypt DHS 2014, 2,811 (2.34%) individuals who were 
...
/*
In the context of Egypt DHS 2014, is recorded for every EVER-MARRIED woman 
aged 15-49, children 0-5 and never married youth 5-19 years in all the sampled 
households (p.172). Thus there is no presence of subsample.*/
...
*****************************************
	//NOTE: Egypt DHS 2014 have no male recode file. 
	//If this is the case, this variable takes missing value	
gen no_male_eligible = . 
lab var no_male_eligible "Household has no eligible man"
tab no_male_eligible, miss


*** No Eligible Youths 5-19 years
*****************************************
gen youth_eligible = (shanth1==1 | shanth2==1)
bysort hh_id: egen hh_n_youth_eligible = sum(youth_eligible)  
	//Number of eligible youths for anthropometrics
gen no_youth_eligible = (hh_n_youth_eligible==0) 
	//Takes value 1 if there were no eligible youth for anthropometrics
lab var no_youth_eligible "Household has no youth eligible"
tab no_youth_eligible, miss
...
***********************************************
	/*NOTE: In the context of Egypt DHS 2014, anthropometric data was collected 
	from women 15-49, youth 5-19, and child under 5. So the 'no_eligibles'  
	variable represents households that  do not have either of these eligible 
	population*/	
gen no_eligibles = (no_fem_eligible==1 & no_youth_eligible==1 & no_child_eligible==1)
lab var no_eligibles "Household has no eligible women, youth, or children"
tab no_eligibles, miss


*** No Eligible Subsample 
*****************************************
	/*NOTE: Anthropometric data was collected from all women and children. The
	hv042 variable suggest that hemoglobin data was collected from 1/3 of the
	sampled households. However, anthropometric data was collected from all 
	married women 15-49 years, their children under 5, and all never married 
	youth 5-19 years. As such, in the context of Egypt DHS 2014, the variable 
	'no_hem_eligible' takes missing value*/	
gen no_hem_eligible = . 	
lab var no_hem_eligible "Household has no eligible individuals for hemoglobin measurements"
tab no_hem_eligible, miss


drop fem_eligible hh_n_fem_eligible youth_eligible hh_n_youth_eligible ///
...
//Subnational region
codebook shgov, tab (100)	
clonevar region = shgov
...
gen	child_schoolage = (age>=6 & age<=14)
	/*Note: In Egypt, the official school entrance age is 6 years.  
...
********************************************************************************
/*Note: Malawi DHS 2015-16 has no anthropometric data for men, 15-54 years */
...
********************************************************************************
*** Step 2.3b Youth Nutrition ***
********************************************************************************


*** Standard MPI: Youth BMI-for-age Indicator ***
************************************************************************
bysort hh_id: egen temp = max(low_bmiage_youth)
gen	hh_no_low_bmiage_y = (temp==0) 
	//Takes value 1 if no child in the hh is underweight 
replace hh_no_low_bmiage_y = . if temp==.
replace hh_no_low_bmiage_y = 1 if no_youth_eligible==1 
	//Households with no eligible youth will receive a value of 1 
lab var hh_no_low_bmiage_y "Household has no youth with low BMI-for-age"
drop temp
...
replace hh_nutrition_uw_st = 0 if hh_no_low_bmiage==0 | hh_no_low_bmiage_y==0 | hh_no_uw_st==0
replace hh_nutrition_uw_st = . if hh_no_low_bmiage==. & hh_no_low_bmiage_y==. & hh_no_uw_st==.
...
********************************************************************************
	/*In Egypt, child mortality information was collected only from ever-married 
	women */
...
replace temp_f = 0 if hv115==0 & hv104==2 & hv105>=15 & hv105<=49 & v201==.
...
replace childu18_died_per_wom_5y = 0 if hv115==0 & hv104==2 & hv105>=15 & hv105<=49 & childu18_died_per_wom_5y== .
	//This line replaces never-married women with 0 child death. 		
...
gen	toilet_mdg = (toilet==11 | toilet==12 | toilet==16 | ///
...
replace toilet_mdg = 0 if toilet==14 | toilet==15 | toilet==17 | ///
						  toilet==18 | toilet==23 | toilet==31 | ///
						  toilet==42 | toilet==96
...
clonevar ndwater = hv202  
	//Egypt DHS 2014: No observation for non-drinking water. 
...
replace water_mdg = 0 if water==32 | water==43 | water==61 | ///
						 water==62 | water==96 
...
Note: Egypt DHS 2004 has no information on walls */
lookfor wall
clonevar wall = hv214 
...
Note: Egypt DHS 2004 has no information on roof */
lookfor roof
clonevar roof = hv215
...
**************************************************************
gen housing_1 = 1
replace housing_1 = 0 if floor_imp==0 
replace housing_1 = . if floor_imp==. 
lab var housing_1 "Household has floor that it is not low quality material"
tab housing_1, miss
...
Note: Egypt has no data on cooking fuel*/
...
save "$path_out/egy_dhs14.dta", replace 
