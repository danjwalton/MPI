...
2019 Global Multidimensional Poverty Index - Algeria MICS 2012-13
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Algeria_MICS4_Datasets
...
/*Algeria MICS 2012-13: P35 indicates that anthropometric data was collected 
...
gen str30 datalab = "children_nutri_dza" 
...
*** Variable: AGE ***
	//We use age in months as it has fewer missing values 
tab cage, miss 
codebook cage 
clonevar age_months = cage
desc age_months
replace age_months = . if cage==.
summ age_months
...
*** Variable: BODY WEIGHT (KILOGRAMS) ***
	/*Note: Some 8.5 percent of children under 5 years have missing info on 
	weight. As a result, this affect the size of missing observations 
	for the final nutrition indicator. */
codebook an3, tab (9999)
clonevar weight = an3	
replace weight = . if an3>=90
tab an2 an3 if an3>=90 | an3==., miss 
...
*** Variable: HEIGHT (CENTIMETERS)
	/*Note: Some 9 percent of children under 5 years have missing info on 
	height. As a result, this affect the size of missing observations 
	for the final nutrition indicator. */
codebook an4, tab (9999)
clonevar height = an4
replace height = . if an4>=900 
tab an2 an4 if an4>=900 | an4==., miss
desc height 
summ height
...
use "$path_out/children_nutri_dza_z_rc.dta", clear 
...
/*Note in the context of Algeria MICS 2012-13, 93 children are replaced as '.' 
...
sort ind_id
save "$path_out/dza13_CH.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_dza_z_rc.xls"
erase "$path_out/children_nutri_dza_prev_rc.xls"
erase "$path_out/children_nutri_dza_z_rc.dta"


	/*Note that some 8.9% of the children under 5 who were sampled had missing 
...
sort ind_id
save "$path_out/dza13_BH.dta", replace	
...
tab cm1 cm8, miss
	...
	child mortality. Note that 15 women reported as never had a child but 
	have a child who died. THis mismatch will be addressed in section 2.4 */
...
rename marital marital_wom
	...
	//Retain relevant variables:	
keep wm7 cm1 cm8 cm9a cm9b ma5 ind_id women_WM *_wom
order wm7 cm1 cm8 cm9a cm9b ma5 ind_id women_WM *_wom 
sort ind_id
save "$path_out/dza13_WM.dta", replace
...
********************************************************************************
	...
	//Note: Algeria MICS 2012-13 does not have male recode information.
...
save "$path_out/dza13_HH.dta", replace
...
*** Merging BH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/dza13_BH.dta"
drop _merge
erase "$path_out/dza13_BH.dta" 
 
 
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/dza13_WM.dta"
tab hl7, miss 
...
drop temp
drop _merge
erase "$path_out/dza13_WM.dta"


*** Merging HH Recode 
*****************************************
merge m:1 hh_id using "$path_out/dza13_HH.dta"
tab hh9 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed 
drop _merge
erase "$path_out/dza13_HH.dta"


*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/dza13_CH.dta"
drop _merge
erase "$path_out/dza13_CH.dta"
...
*****************************************
	/*There is no male data, hence the eligibility variable is assigned as 
	missing observation */
...
***********************************************
	/*Since there is no male data, the adult eligibility variable is assigned as 
	missing observation */
...
//Subnational region
	/*There are 9 subnational territories in Algeria. However, in the MICS, the 
	southern territories were grouped into a single territory resulting in 
	7 subnational territories (p.32) */
lookfor province
codebook hh7, tab (99)
decode hh7, gen(temp)
...
tab hh7 region, miss 
drop temp
label define lab_reg 1"Nord-Centre" 2"Nord-Est" 3"Nord-Ouest" ///
4"Hauts Plateaux-Centre" 5"Hauts Plateaux-Est" 6"Hauts Plateaux-Ouest" ///
7"Sud"			
...
5 years of compulsory primary, 
4 years of compulsory secondary, and 
3 years of upper secondary. 

The admission age to compulsory education is 6 years. 
Primary education takes place from age 6-11 (grades 1-5). 
Lower secondary education takes place from age 11-15 (grades 6-9). 
Upper secondary education takes place from age 15-18(grades 10-12).

Change in education system in 2007-08
Reference: http://data.uis.unesco.org/ */

tab ed4b ed4a, miss
tab age ed6a if ed5==1, miss
	/*In the case of Algeria MICS 2012-13, there is inconsistency such as 
...
replace eduyears = eduhighyear + 6 if (edulevel==2)   
	/*low secondary education assumed to start after 5 years of general 
	education */
	
replace eduyears = eduhighyear + 9 if (edulevel==3) 
	/*high secondary education assumed to start after 9 years of general 
	education */
replace eduyears = eduhighyear + 12 if (edulevel==4) 
	/*University after 12 years of education, 5 years of primary + 7 years of 
	secondary */

replace eduyears = . if (edulevel==2 & eduhighyear > 12) 
	/* The report on Algeria's education, groups primary & secondary together 
...
gen child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Algeria, the official school entrance age is 6 years  
...
codebook cm9a cm9b
	/*In the context of Algeria MICS 2012-13, child mortality information was 
	not collected from men hence mcm9a mcm9b is missing. On the other hand, 
	women provided the information on child mortality:*/


egen temp_f = rowtotal(cm9a cm9b), missing
	//Total child mortality reported by eligible women
replace temp_f = 0 if cm1==1 & cm8==2 | cm1==2 
replace temp_f = 0 if ma5==3
	/*Assign a value of "0" for:
	- all eligible women who have ever gave birth but reported no child death 
	- all eligible women who never ever gave birth 
	- all women who were not currently married or ever married did not answer 
	the child mortality question, henced assumed to have 0 child mortality */
replace temp_f = 0 if no_fem_eligible==1 & temp_f == .
	/*Assign a value of "0" for:
...
replace childu18_died_per_wom_5y = 0 if ma5==3															   
	/*Assign a value of "0" for:
	- all eligible women who never ever gave birth 
	- all women who were not currently married or ever married did not answer 
	the child mortality question, henced assumed to have 0 child mortality*/
...
********************************************************************
	/*Note: Following the MDG definition, 'flush to unkown place' is considered 
	as non-improved. However, in Algeria, this particular category is identified 
	as improved sanitation facilities (p.117 country report). As such, we 
	follow the report */
...
replace toilet_mdg=0 if toilet==14
	...
	"flush to somewhere else" and "flush do not know where"*/
...
tab ws2 if water==91 	
	...
	cooking and hand washing. However, it is important to note that households 
	using bottled water for drinking are classified as unimproved source if this 
	is explicitly mentioned in the country report. */	
...
*****************************************************************
gen	cooking_mdg = 1
replace cooking_mdg = 0 if cookingfuel>5 & cookingfuel<10 
...
char _dta[cty] "Algeria"
char _dta[ccty] "DZA"
char _dta[year] "2012-2013" 	
...
save "$path_out/dza_mics12-13.dta", replace 
