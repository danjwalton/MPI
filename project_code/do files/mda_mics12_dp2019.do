...
2019 Global Multidimensional Poverty Index - Moldova, Republic of MICS 2012
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Moldova_2012_MICS_Datasets
...
/*Moldova MICS 2012: Anthropometric information was collected for all children
under 5 years of age. Height and weight measurements were not collected for
adults, either men or women. (p.20) */
...
gen str30 datalab = "children_nutri_mda" 
...
use "$path_out/children_nutri_mda_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Moldova, Republic of MICS 2012, 305 children were 
...
sort ind_id
save "$path_out/MDA12_CH.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_mda_z_rc.xls"
erase "$path_out/children_nutri_mda_prev_rc.xls"
erase "$path_out/children_nutri_mda_z_rc.dta"
...
lookfor birth
	/* bh4c does not exist. Instead there is bh4m (month of birth) and bh4y 
	(year of birth). This will be converted to cmc for comparison with wdoi. */
lookfor death
	/* bh9c does not exist. Instead there is bh9u (age at death, units) and bh9n 
	(age at death, number). This will be converted to months for comparison with 
	wdoi */
desc bh4m bh4y bh9u bh9n
gen bh4c = 12*(bh4y - 1900) + bh4m
	/* bh4c is generated using the cmc formula for comparison with wdoi */
gen bh9c = bh9n/30 if bh9u==1
	/*bh9c is generated as age at deaths months. For bh9u, 1=days, 2=months, and 
	3=years */
replace bh9c = bh9n if bh9u==2
...
sort ind_id
save "$path_out/MDA12_BH.dta", replace	
...
tab wb2, miss
	//The age profile of women in the sample, for Moldova MICS 2012, is 15-49.
...
sort ind_id
save "$path_out/MDA12_WM.dta", replace
...
sort ind_id
save "$path_out/MDA12_MN.dta", replace
...
save "$path_out/MDA12_HH.dta", replace
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MDA12_BH.dta"
drop _merge
erase "$path_out/MDA12_BH.dta" 
 
 
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MDA12_WM.dta"
tab hl7, miss 
gen temp = (hl7>0) 
...
drop temp _merge
erase "$path_out/MDA12_WM.dta"


*** Merging HH Recode 
*****************************************
merge m:1 hh_id using "$path_out/MDA12_HH.dta"
tab hh9 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed 
drop _merge
erase "$path_out/MDA12_HH.dta"


*** Merging MN Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MDA12_MN.dta"
drop _merge
erase "$path_out/MDA12_MN.dta"


*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MDA12_CH.dta"
drop _merge
erase "$path_out/MDA12_CH.dta"
...
//Subnational region
	/*Note: The sample for the 2012 Moldova MICS was designed to provide 
	estimates for a large number of indicators on the situation of children, 
	women and men at the national level, for urban and rural areas, and for the 
	following regions: North; Central; South; the Chișinău municipality */
codebook hh7, tab (99)
clonevar region = hh7
...
/* According to the survey report (p.86): "According to the Law on Education in 
the Republic of Moldova, there are nine years of compulsory education, including 
chidren of age 7-15 years, which is typical for general compulsory education. 
Children enter primary school at the age of 7 and secondary school at 11. There 
are 4 grades in primary school (I-IV) and 5 grades in lower secondary school - 
gymnasium (V-IX). Upper secondary school comprises 3 grades (for children aged 
16-18 years)." */
...
replace eduyears = 0 if edulevel<=2 & eduhighyear==.   
	...
	is unknown*/
replace eduyears=4 if edulevel==2 & eduhighyear==0 | eduhighyear==.	
replace eduyears=9 if edulevel==3 & eduhighyear==0 | eduhighyear==.
replace eduyears = eduhighyear + 12 if (edulevel==4 | edulevel==5 | edulevel==6)   
	/*Polyvalent, technical school, and university assumed to start after 12 
	years of education */
replace eduyears=12 if edulevel>=4 & edulevel<=6 & eduhighyear==.	
...
gen	child_schoolage = (age>=7 & age<=15)
	/*
	Note: In Moldova, the official school entrance age is 7 years.  
...
****************************************
	...
	not know where is considered as non-improved. However, in Moldova, this 
	particular category is identified as improved sanitation facilities (p.56 
...
gen	floor_imp = 1
replace floor_imp = 0 if floor==96 | floor<=12	
...
codebook hc8c hc8b hc8d hc9b hc8e hc9c hc8o hc11
...
save "$path_out/mda_mics12.dta", replace 
