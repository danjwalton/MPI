...
2019 Global Multidimensional Poverty Index - Kenya DHS 2014 [STATA do-file]. 
...
/*Kenya DHS 2014: Anthropometric data was gathered from all children under 5 
living  in the sampled households. Anthropometric data was collected from 
women aged 15-49 living in 1/2 of the subsample households that also 
received the men's questionnaire. However, anthropometric data was not collected 
from men (p.6-7 & p.399). */
...
********************************************************************************
	...
	in the context of Kenya DHS 2014, the quality of the KR file is not at par 
	with other KR files of other countries. There is some difference between 
	members in the KR file and the same members in the PR file. After going 
	through carefully the files, it was decided that we would construct the 
	child under 5 heatlh indicator from the PR file*/
	
use "$path_in/KEPR71FL.DTA", clear 
...
count if hv105<5 
	//The dataset has 21,970 children under 5
count if hv120==1	
	/* 21,896 children under 5 are eligible for anthropometric 
	measurement. This roughly corrosponds to 100% of the sample */	
count if hc1!=.
	//All 21,896 children under 5 have information on age in months
tab hv105 if hc1!=.
	/*A cross check with the age in years reveal that all are within the 5 year 
	age group */
tab hc13 if hc1!=., miss
	/*Of the 21,896 children under 5, 21,137 have been measured while 172 have 
	missing information */
tab hc13 if hc1!=. & hc3<=9990, miss
tab hc13 if hc1!=. & hc2<=9990, miss
	/*For example, height and weight data is available for all 21,137 children 
...
gen str30 datalab = "children_nutri_ken" 
...
use "$path_out/children_nutri_ken_z_rc.dta", clear  
...
count if _fwei==1 | _flen==1 
	/*Note: In Kenya DHS 2014, 152 children were replaced as missing because
...
save "$path_out/KEN14_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_ken_z_rc.xls"
erase "$path_out/children_nutri_ken_prev_rc.xls"
erase "$path_out/children_nutri_ken_z_rc.dta"
...
use "$path_in/KEBR71FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Kenya DHS 2014: these figures are identical
...
sort ind_id
save "$path_out/KEN14_BR.dta", replace	
...
use "$path_in/KEIR71FL.DTA", clear
...
sort ind_id
save "$path_out/KEN14_IR.dta", replace
...
count if v012>=15 & v012<=19
	//Total number of girls in the IR recode: 6,078	
...
gen str30 datalab = "girl_nutri_ken" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_ken_z.dta", clear 
...
sort ind_id
save "$path_out/KEN14_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_ken_z.xls"
erase "$path_out/girl_nutri_ken_prev.xls"
erase "$path_out/girl_nutri_ken_z.dta"
...
use "$path_in/KEMR70FL.DTA", clear 
...
sort ind_id
save "$path_out/KEN14_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Kenya 2014, anthropometric data was NOT collected 
for men */
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/KEN14_BR.dta"
drop _merge
erase "$path_out/KEN14_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/KEN14_IR.dta"
...
drop _merge
erase "$path_out/KEN14_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/KEN14_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13==0 & hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Kenya DHS 2014, 58 girls 15-19 were identified as measured 
...
drop _merge
erase "$path_out/KEN14_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/KEN14_MR.dta"
tab men_MR hv118, miss col
drop _merge
erase "$path_out/KEN14_MR.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/KEN14_KR.dta"
drop _merge
erase "$path_out/KEN14_KR.dta"
...
tab resident, miss
	...
	In the context of Kenya DHS 2014, 2,747 (1.79%) individuals who were 
...
/*
In the context of Kenya DHS 2014,anthropometric data was collected from women 
aged 15-49 living in 1/2 of the subsample households that also received the 
men's questionnaire, suggesting a presence of subsample selection. Interestingly, 
Kenya DHS 2014 also collected height and weight measurements from children under 
5 who live in the 100% sampled households. By keeping only the 50% subsample, 
this means we exclude information from children who were sampled in the other 
50% of the household. However, the child nutrition figures based on 100% and 
50% sample are very similar. Following the principles of the global MPI, we 
restrict the analysis of the global MPI for Kenya to the subsample. This is to 
avoid inflating the non-deprived category of adult nutrition, given that in our 
work, all households that don't have eligible population will be replaced as 
'non-deprived'. */
...
gen	no_male_eligible = (hh_n_male_eligible==0) 	
	...
	//NOTE: Not all surveys have male recode file. 
...
*****************************************
	//NOTE: Kenya DHS 2014 has no observation for hemoglobin data 
gen no_hem_eligible = . 	
...
tab hv115 marital, miss
	//NOTE: Kenya dhs 2014 has no observation for marital status
...
//Subnational region
	/* NOTE: According to the survey report for Kenya DHS 2014, the full sample 
	is representative for indicators at the national, rural/urban, regional, 
	and county levels. However, data collected only in the full questionnaires 
	(the 1/2 subsample) is representative at the national, rural/urban, and 
	regional levels and not recommended for estimation at the county level 
	(p. 6). There are 8 regions. */
lookfor region
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Kenya, the official school entrance age is 6 years.  
...
********************************************************************************
	//Note: Kenya DHS 2014 has no anthropometric data for men.
...
lab var m_bmi "Male's BMI"
	//Kenya DHS 2014 has no anthropometric data for men
...
clonevar ndwater = hv202  
	//Kenya DHS 2014: no observations for non-drinking water.
...
tab cookingfuel cooking_mdg, miss	
	...
	/*Note that in Kenya DHS 2014, the category 'other' cooking fuel is not 
...
save "$path_out/ken_dhs14.dta", replace 
