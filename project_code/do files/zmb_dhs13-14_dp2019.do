...
2019 Global Multidimensional Poverty Index - Zambia DHS 2013-14 [STATA do-file]. 
...
/*
In Zambia anthropometric measures were collected for all children and 
women (p.7). Anthropometric measures were not collected for men. 
*/
...
use "$path_in/ZMKR61FL.DTA", clear 
...
drop if b5==0 
	...
	//Zambia DHS 2013-14: 743 observations deleted

duplicates report ind_id
duplicates tag ind_id, gen(duplicates)
tab b16 if duplicates!=0 
tab hw13 if duplicates!=0
	/*For children not listed in the household, create a false 
...
gen str30 datalab = "children_nutri_zmb" 
...
standards */
use "$path_out/children_nutri_zmb_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Zambia DHS 2013-14, 200 children were replaced as
...
sort ind_id
duplicates report ind_id
save "$path_out/ZMB13-14_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_zmb_z_rc.xls"
erase "$path_out/children_nutri_zmb_prev_rc.xls"
erase "$path_out/children_nutri_zmb_z_rc.dta"
...
use "$path_in/ZMBR61FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Zambia DHS 2013-14: these figures are identical 
...
sort ind_id
save "$path_out/ZMB13-14_BR.dta", replace	
...
use "$path_in/ZMIR61FL.DTA", clear
...
sort  ind_id
save "$path_out/ZMB13-14_IR.dta", replace
...
count if v012>=15 & v012<=19
	//Total number of girls in the IR recode: 3,686	
...
gen str30 datalab = "girl_nutri_zmb" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_zmb_z.dta", clear 
...
sort ind_id
save "$path_out/ZMB13-14_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_zmb_z.xls"
erase "$path_out/girl_nutri_zmb_prev.xls"
erase "$path_out/girl_nutri_zmb_z.dta"
...
use "$path_in/ZMMR61FL.DTA", clear 
...
sort ind_id
save "$path_out/ZMB13-14_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Zambia 2013-14, anthropometric data was NOT collected 
...
sort ind_id
save "$path_out/ZMB13-14_MR_boys.dta", replace
...
use "$path_in/ZMPR61FL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ZMB13-14_BR.dta"

drop _merge

erase "$path_out/ZMB13-14_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ZMB13-14_IR.dta"
...
erase "$path_out/ZMB13-14_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/ZMB13-14_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Zambia DHS 2013-14, 95 girls 15-19 were identified as measured 
...
erase "$path_out/ZMB13-14_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ZMB13-14_MR.dta"
...
tab hb13 hb65 if hv118==1 & men_MR ==., miss
	//No anthropometric data for men in Zambia DHS 2013-14

drop _merge

erase "$path_out/ZMB13-14_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
	//No anthropometric data for boys in Zambia DHS 2013-14

merge 1:1 ind_id using "$path_out/ZMB13-14_MR_boys.dta"
	
drop _merge

erase "$path_out/ZMB13-14_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ZMB13-14_KR.dta" 
...
erase "$path_out/ZMB13-14_KR.dta"
...
tab resident, miss 
	...
	In the context of Zambia DHS 2013-14, 1,500 (1.80%) individuals who were 
...
In the context of Zambia DHS 2013-14, height and weight measurements were
collected from all children (0-5) and women (15-49) who were sampled. As such 
there is no presence of nutrition subsample for Zambia DHS 2013-14.
*/
...
in those relevant indicators.(2010)*/
...
***********************************************
	/*NOTE: In the DHS datasets, we use this variable as a control 
	variable for the nutrition indicator if nutrition data is 
	present for children, women and men. Since anthropometric data was not
	collected from men, this variable is created as an empty variable */
gen no_eligibles = .
...
*****************************************
	...
	So "hem_eligible" represents that subsample.*/		
...
//Subnational region
	/*NOTE: The sample for the Zambia 2013-14 was designed to provide estimates 
	at the national and provincial levels, as well as for rural and urban areas 
	within the provinces. Zambia is now administratively divided into 10 
	provinces (Central, Copperbelt, Eastern, Luapula, Lusaka, Muchinga, 
	Northern, North Western, Southern, and Western) (p.7). */  
...
gen	child_schoolage = (age>=7 & age<=15)
	/* 
	Note: In Zambia the official school entrance age is 7 years. 
...
********************************************************************************
	//Note: Zambia DHS 2013-14 has no anthropometric data for men, 15-59 years
...
lab var m_bmi "Male's BMI"
	//Zambia DHS 2013-14 has no anthropometric data for men, 15-59 years
...
/*In Zambia,the report considers the category other (96) as neither improve 
nor non-improved (p. 17). But we consider it as non-improved*/
...
save "$path_out/zmb_dhs13-14.dta", replace 
