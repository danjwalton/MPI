...
2019 Global Multidimensional Poverty Index - South Africa DHS 2016 
[STATA do-file]. Retrieved from: 
...
/*South Africa DHS 2016: Anthropometric testing was incorporated in the 
subsample of households selected for the male survey and adult health module 
(p.39 of PDF). Within this subsample, height and weight measurements were 
recorded for children age 0-59 months for whom consent was obtained and for men
and women age 15 years and older who consented to measurement (p.39 pf PDF).   

According to (page 332 of the PDF):
	"Anthropometric data on height and weight were collected for all women
	and men age 15 and older who consented to measurements (82% of women
	and 78% of men)."
	
Also, on page 39 of the PDF DHS Report of South Africa:	
	"Height and weight measurements were recorded for children age 0-59 
	months for whom consent was obtained from their parents/guardians 
	and for women and men age 15 and older who consented to measurement."
*/
...
use "$path_in/ZAPR71FL.DTA", clear 
...
tab hv120 hv027, miss
	//2,079 children under 5 are eligible for anthropometric measurement	
count if hc1!=.
	//All 2,079 children under 5 have information on age in months
tab hv105 if hc1!=.
	/*A cross check with the age in years reveal that all are within the 5 year 
	age group */
tab hc13 if hc1!=., miss
	/*Of the 2,079 children, 5,866 have been measured while 243 have 
	missing information */
tab hc13 if hc1!=. & hc3<=9990, miss
tab hc13 if hc1!=. & hc2<=9990, miss
	/*Of the 2,079 children, 1,517 (73%) have been measured. For example, 
	height and weight data is available for the 1,517 children. According to 
	the report (p.177), the low percentages of valid data were mainly the 
	result of missing or incomplete data (p.177). */
...
gen str30 datalab = "children_nutri_zaf" 
...
standards */
use "$path_out/children_nutri_zaf_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1 
	/*South Africa DHS 2016: 30 children were replaced as missing because
	they have extreme z-scores which are biologically implausible. */
 
count if stunting!=. & hc13==0 & hv102==1 & hv103==1 
	/*1,439 children under 5 who are usual residents and slept the night 
	before in the household was measured and have height-for-age (stunting) 
	indicator. Table 11.1 (sum 'number of children' by age in months, p.188) 
	indicate that height-for-age indicator covered 1,405 children. */  
...
sort ind_id
save "$path_out/ZAF16_PR_child.dta", replace
	

	//Erase files from folder:
erase "$path_out/children_nutri_zaf_z_rc.xls"
erase "$path_out/children_nutri_zaf_prev_rc.xls"
erase "$path_out/children_nutri_zaf_z_rc.dta"
...
use "$path_in/ZABR71FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//South Africa DHS 2016: These figures are identical
...
sort ind_id
save "$path_out/ZAF16_BR.dta", replace	
...
use "$path_in/ZAIR71FL.DTA", clear
...
sort ind_id
save "$path_out/ZAF16_IR.dta", replace
...
gen str30 datalab = "girl_nutri_zaf" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_zaf_z.dta", clear 
...
sort ind_id
save "$path_out/ZAF16_PR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_zaf_z.xls"
erase "$path_out/girl_nutri_zaf_prev.xls"
erase "$path_out/girl_nutri_zaf_z.dta"
...
use "$path_in/ZAMR71FL.DTA", clear 
...
codebook mv201 mv206 mv207,tab (999)
	/*If there is missing values, check why this is the case. Only retain the 
	age group who were eligible to provide information on child mortality. */
...
sort ind_id
save "$path_out/ZAF16_MR.dta", replace
...
/* We use datalab to specify the name that will prefix the output files that 
will be produced from using this ado file*/
gen str30 datalab = "boy_nutri_zaf" 
lab var datalab "Working file"
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores to compute BMI-for-age*/
use "$path_out/boy_nutri_zaf_z.dta", clear 

	
gen	z_bmi = _zbfa
replace z_bmi = . if _fbfa==1 
	/*South Africa DHS 2016: 1 boy 15-19 years were replaced as missing 
...
sort ind_id
save "$path_out/ZAF16_PR_boys.dta", replace


	//Erase files from folder:
erase "$path_out/boy_nutri_zaf_z.xls"
erase "$path_out/boy_nutri_zaf_prev.xls"
erase "$path_out/boy_nutri_zaf_z.dta"
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ZAF16_BR.dta"
drop _merge
erase "$path_out/ZAF16_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ZAF16_IR.dta"
tab women_IR hv117, miss col
tab ha65 if hv117==1 & women_IR==., miss 
	//Total number of eligible women not interviewed
drop _merge
erase "$path_out/ZAF16_IR.dta"
...
tab v201 ha65 if hv117==1, miss
	/*Note: Some 14% eligible women did not provide information on their birth 
...
*****************************************
merge 1:1 ind_id using "$path_out/ZAF16_PR_girls.dta"
tab ha13 girl_PR if hv105>=15 & hv105<=19 & hv104==2 & hv027==1, miss col
drop _merge
erase "$path_out/ZAF16_PR_girls.dta"		
	
	
*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ZAF16_MR.dta"
tab men_MR hv118 if hv027==1, miss col
tab hb65 if hv118==1 & men_MR ==. 
	//Total of eligible men not interviewed
drop _merge
erase "$path_out/ZAF16_MR.dta"		

	
*** Merging 15-19 years: boys 
*****************************************	
merge 1:1 ind_id using "$path_out/ZAF16_PR_boys.dta"
tab hb13 boy_PR if hv105>=15 & hv105<=19 & hv104==1 & hv027==1, miss col
drop _merge
erase "$path_out/ZAF16_PR_boys.dta"


*** Merging child under 5 
*****************************************
merge 1:1 ind_id using "$path_out/ZAF16_PR_child.dta"
tab hv120, miss  
tab hc13 if hv120==1, miss
drop _merge
erase "$path_out/ZAF16_PR_child.dta"
...
drop if resident!=1 
tab resident, miss
	/*South Africa DHS 2016: 925 (2.38%) individuals who were 
...
South Africa DHS 2016 report indicate in households selected for the male 
survey, all eligible women and men age 15-older and children under 5 were 
measured for their height and weight. We use the variable hv027 to identify 
households selected for male interview which then allows for the identification 
of the anthropometric subsample. Table 8.3 (p.157 of PDF), Table 11.1 
(p.223 of PDF) and Table 17.1.1-2 (p.338-9)summarises early childhood mortality 
rates, nutritional status of children and nutritional status of women 
respectively by the 9 provinces in South Africa, suggesting the 
anthropometric and child mortality estimates are representative at the 
subnational level. */
...
***********************************************
	/*South Africa DHS 2016: Nutrition data was collected from all adults 15 
	years and older living in 50% of the households selected as part of the 
	anthropometric subsample (p.411 of PDF). Following the global MPI criteria, 
	we take into account of the anthropometric data for women and men aged 15 
	years and up to the age of 70 years only.*/
tab ha13, miss
tab ha13 if hv105>=15 & hv105!=. & hv104==2, miss
	//women 15 years and older
tab ha13 if hv105>=15 & hv105<=70 & hv104==2, miss
	//women 15-70 years 
...
***********************************************
	...
	years and up to the age of 70 years only.*/
tab hb13, miss
tab hb13 if hv105>=15 & hv105!=. & hv104==1, miss
	//men 15 years and older
tab hb13 if hv105>=15 & hv105<=70 & hv104==1, miss
	//men 15-70 years 
...
tab hv103 hv118, miss
	/*Only men who slept the night before in the household were 
	eligible for male interview (men's questionnaire). */
...
//Subnational region
	...
	/* South Africa DHS 2016 report (page 411 of the PDF):
	"The survey was designed to provide representative estimates for main 
	 demographic and health indicators for the country as a whole, for urban 
	 and non-urban areas separately, and for each of the nine provinces in
	 South Africa: Western Cape, Eastern Cape, Northern Cape, Free State, 
	 KwaZulu-Natal, North West,Gauteng, Mpumalanga, and Limpopo." */
...
gen	child_schoolage = (age>=7 & age<=15)
	/* South Africa: The official school entrance age to primary school is 
	7 years. So, age range is 7-15 (=7+8) 
	Source: "http://data.uis.unesco.org/?ReportId=163"
	Also, in South Africa DHS Report 2016 (page 51 of the PDF), school 
	attendance is calculated with a sample of children age 7-13 for primary 
	school NAR and children age 14-18 for secondary school NAR*/
...
foreach var in ha40 hb40 {
			 ...
			 /*BMI data is available for women and men aged 15-95+ years. But 
			 for the purpose of the global MPI, we only use anthropometric 
			 information from adults up to the age of 70 years.*/ 
			 drop inf_`var'
			 }
***

*** BMI Indicator for Women 15-70 years ***
******************************************************************* 
gen	f_bmi = ha40/100
...
*** BMI Indicator for Men 15-70 years ***
******************************************************************* 
gen m_bmi = hb40/100
...
codebook toilet, tab(99)
	/* In South Africa DHS 2016, pit latrine with a slab without ventilation 
	pipe (code 23) and checmical toilet (code 44) is classified as improved. 
	See Table 2.3 in page 56 of the DHS report.*/
	
gen	toilet_mdg = ((toilet<=23 | toilet==41 | toilet==44) & shared_toilet!=1) 
	/*Household is assigned a value of '1' if it uses improved sanitation and 
	does not share toilet with other households  */	
	
replace toilet_mdg = 0 if (toilet<=23 | toilet==41 | toilet==44)  & shared_toilet==1   
...
tab cookingfuel cooking_mdg, miss	
	...
	/*South Africa DHS 2015-16: the category 'other' cooking fuel is not 
	identified as solid fuel by report (p.57 of PDF). Hence this particular 
...
tab N, miss
drop N
	//Final sample retained for estimation is 37,716 individuals (96.66%)

codebook region, tab (99)
tab region if d_nutr==., miss	 
tab region if d_cm==., miss	
tab region if d_nutr==. | d_cm==., miss	 
 
	/* Note: Nutrition has 16.5% missing value. This corrosponds with the report 
	where it is stated that the low percentages of valid data for children 
	under 5 were mainly the result of missing or incomplete data (p.177). In
	addition, only some 82% of women and 78% of men consented to anthropometric 
	measurements (p.332).
	
	In addition, child mortality has close to 5% missing observations. As 
	indicated under the merging section, some 14% eligible women did not provide 
	information on their birth history. So it is not surprising to observe
	a high missing value. */ 
 
 
*** Generate coutry and survey details for estimation ***
char _dta[cty] "South Africa"
char _dta[ccty] "ZAF"
char _dta[year] "2016" 	
...
save "$path_out/zaf_dhs16.dta", replace 
