...
2019 Global Multidimensional Poverty Index - Trinidad and Tobago MICS 2011
[STATA do-file]. Available from OPHI website: 
http://ophi.org.uk/  
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Trinidad_and_Tobago_MICS4_Datasets
...
/*Trinidad and Tobago MICS 2011: Anthropometric data was collected only 
for childen under 5 years. (p.11) */
...
gen str30 datalab = "children_nutri_tto" 
...
codebook an3, tab (999) 
	//Some 9.5% of children under 5 years have missing info on weight
clonevar weight = an3	
...
codebook an4b, tab (999) 
	//9.23% of children under 5 years have  missing info on height
clonevar height = an4b
replace height = . if an4b>=999 
	//All missing values or out of range are replaced as "."
tab	an2 an4b if an4b>=999 | an4b==., miss
desc height 
summ height

	
*** Variable: MEASURED STANDING/LYING DOWN	
	/* For children aged below 24 months (<731 days), it makes a difference 
	whether the child was measured lying down (recumbent), in which case his/her 
	length was measured, than whether he was measured standing, in which his/her 
	measure is height. The ado can adjust for this difference. When the kid was 
	measured standing, the ado converts the height to recumbent length by adding 
	0.7cm, and for children aged 24 months or above, who are measured in a 
	recumbent position, the ado converts the length to standing height by 
	subtracting 0.7 cm. The exported variable is _clenhei, which is the 
	converted length/height according to age. In MICS all children under 24 
	months are supposed to have been measured in recumbent position, and all 
	children above 24 months are supposed to have been measured in standing 
	position. The survey provides a variable that controls for this.*/
codebook an4a
...
standards */
use "$path_out/children_nutri_tto_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Trinidad and Tobago MICS 2011, 28 children were 
	replaced as '.' because they have extreme z-scores that are biologically 
	implausible.*/  
...
sort ind_id
save "$path_out/TTO11_CH.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_tto_z_rc.xls"
erase "$path_out/children_nutri_tto_prev_rc.xls"
erase "$path_out/children_nutri_tto_z_rc.dta" 
...
********************************************************************************
/* Note: There is no BH dta file for Trinidad and Tobago MICS 2011. Hence this 
...
tab wb2, miss 
	//301 women with missing info on age
...
replace marital = 4 if mstatus == 2 & ma6==2
	//4: Divorced	
replace marital = 5 if mstatus == 2 & (ma6==3 | ma6==4)
	//5: Separated/not living together/no longer in visiting
...
sort ind_id
save "$path_out/TTO11_WM.dta", replace
...
/* Note: There is no MN dta file for Trinidad and Tobago MICS 2011. Hence 
...
save "$path_out/TTO11_HH.dta", replace
...
*** Merging BR Recode 
*****************************************
	//No BH recode for Trinidad and Tobago MICS 2011
	
 
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TTO11_WM.dta"
...
drop temp
drop _merge
erase "$path_out/TTO11_WM.dta"



*** Merging HH Recode 
*****************************************
merge m:1 hh_id using "$path_out/TTO11_HH.dta"
tab hh9 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed 
drop _merge
erase "$path_out/TTO11_HH.dta"



*** Merging MN Recode 
*****************************************
	//No male recode for Trinidad and Tobago MICS 2011
...
*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TTO11_CH.dta"
drop _merge
erase "$path_out/TTO11_CH.dta"
...
//Subnational region 
	...
	and for 5 Regional Health Authorities (regions): North West, North Central, 
	South West, East, and Tobago." (p.11) */
...
********************************************************************************
/* Primary school starts at age 5, comprises 7 grades followed by a secondary 
entrance assessment. Secondary school is compulsory for the first 5 years during 
which time a traditional academic curriculum is followed. Following completion 
of their Caribbean secondary education certificate, students may stay on at 
secondary school for a further 2 years. This may lead to a Caribbean advanced 
proficiency certificate and entrance to tertiary education.*/
...
replace edulevel = . if ed4a==. | ed4a==9 
...
replace eduyears = 0 if edulevel<2 & eduhighyear==.   
	...
	is unknown*/
replace eduyears = eduhighyear + 7 if edulevel==2
    //Primary lasts 7 years according to the report and UNESCO website
replace eduyears = eduhighyear + 12 if (edulevel==3)   
	/*Technical formation requires basic secondary education (compulsory), and 
	the latter lasts 5 years*/
replace eduyears = eduhighyear + 14 if (edulevel==4) 
	/*Professional education requires basic secondary education (5 years) plus 
	2 additional years of advanced secondary*/
...
gen	child_schoolage = (age>=5 & age<=13)
	/*
	Note: In Trinidad and Tobago, the official school entrance age is 5 years  
	as stated in the country report and the UNESCO website. 
	So, age range is 5-13 (=5+8) 
...
********************************************************************************
	/*NOTE: Trinidad and Tobago MICS 2011: No information on child mortality 
	from men */
...
drop temp_f
	...
	/* In the case of Trinidad and Tobago, this variable is created as empty 
	variable because the survey did not collect information on child mortality 
	from men */	
...
************************************************************************
	/*In the case of Serbia, there is no birth history data. This means, there 
...
****************************************
	...
	Following the MDG definition, 'flush don't know where' and 
	'flush to somewhere else' are considered as non-improved. However, in 
	Trinidad and Toabago MICS 2011, 'flush to unknown place/not sure/ DK where'
	is identified as improved sanitation facilities (p.70 country report). 
	We have followed the report. 
	
	In Trinidad and Tobago, the report considers the category open defecation 
	as neither improve nor non-improved. But we consider it as non-improved to 
	be consistent with the indicator for destitution below.(p.70)*/
...
replace toilet_mdg = . if toilet==. | toilet==99
	/* Note: In Trinidad and Tobago "Missing" is classified as non-improved
	(p.70). For consistency with other countries, we kept the line above*/ 
...
gen	water_mdg = 1 if water==11 | water==12 | water==13 | water==14 |  ///
	...
	/*Non deprived if water is 11"piped into dwelling", 12"piped to yard/plot", 
	 13 "piped to neighbor", 14"public tap/standpipe", 41"protected spring", 
	 51"rainwater", 91"bottled water" */
	
replace water_mdg = 0 if water==42 | water==61 | ///
						 water==81 | water==71 | water==96 
	/*Deprived if it is 42 "unprotected spring", 61 "tanker truck"
	81 "surface water (river/lake, etc)", 71 "cart with small tank", 96 "other" */
	
replace water_mdg = 0 if water_mdg==1 & timetowater>=30 & ///
						 timetowater!=. & timetowater<=990
...
replace water_mdg = 0 if water==91 & ///
						(ndwater==42 | ndwater==61 | ///
						 ndwater==81 | ndwater==71 | ///
						 ndwater==96 | ndwater==82) 
	...
	non-drinking activities. Households using bottled water for both drinking 
	and not drinking are considered deprived. */
...
gen	floor_imp = 1
replace floor_imp = 0 if floor<=14 | floor==96 
	//Deprived if "mud/earth/clay", "sand", "dung", "other" "tapia"
...
lab var cooking_mdg "Household has cooking fuel according to MDG standards"
	/* Non deprived if: 1"electricity", 2"lpg", 3"natural gas", "biogas", 
						5"kerosene" , "no food cooked in household", 96"other"
...
codebook hc8c hc8b hc8d hc9b hc8e hc9c hc11 hc9e hc9j
...
clonevar radio = hc8b 
replace radio=1 if hc9e==1 
	/*Stereo or radio with cd player was included to form the radio variable. 
	However, we did not take into account of MP3 player or iPod, given that 
	both items do not provide communication or information*/
...
*** Rename key global MPI indicators for estimation ***
	/* Note: In the case of Trinidad and Tobago MICS 2011, there is no 
	birth history file. We are not able to identify whether child mortality 
	occured in the last 5 years preceeding the survey date. As such, for the 
	estimation, we use the indicator 'hh_mortality' that represent all child 
	mortality that was ever reported. */
...
char _dta[cty] "Trinidad and Tobago"
char _dta[ccty] "TTO"
char _dta[year] "2011" 	
...
save "$path_out/tto_mics11.dta", replace 
