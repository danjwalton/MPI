...
2019 Global Multidimensional Poverty Index - Sierra Leone MICS 2017 
[STATA do-file]. 
...
/* Sierra Leone MICS 2017: Seven questionnaires were used in the survey: 
...
and the dwelling; 
2) a water quality testing questionnaire administered in 3 households in each 
...
children under 5 living in the household; 
...
household; 
7) and a verbal autopsy questionnaire, administered to mothers (or caretakers) 
of all children under 5 who had died in the five years preceding the survey. 
...
desc cage caged
tab cage, miss
	//Age in months: information missing for 10 children
tab caged, miss
	/*Age in days: information missing for 10 children. We use age in days 
...
gen str30 datalab = "children_nutri_sle"
...
standards */
use "$path_out/children_nutri_sle_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1 
	/*Sierra Leone MICS 2017: 360 children were replaced as missing because 
	they have extreme z-scores which are biologically implausible. */

count   
	/*Sierra Leone MICS 2017: the number of eligible children is 11,774 as in 
	the country report (p.iii). No mismatch. */	
...
sort ind_id
save "$path_out/SLE17_CH.dta", replace



	//Erase files from folder:
erase "$path_out/children_nutri_sle_z_rc.xls"
erase "$path_out/children_nutri_sle_prev_rc.xls"
erase "$path_out/children_nutri_sle_z_rc.dta"
...
sort ind_id
save "$path_out/SLE17_BH.dta", replace	
...
tab wb4, miss 
	/*Sierra Leone MICS 2017: Fertility and mortality question was collected 
	from women 15-49 years. 133 women with missing age information.*/
	
tab cm1 cm8, miss
	/*Sierra Leone MICS 2017: 22 women report never having given birth but 
...
sort ind_id
save "$path_out/SLE17_WM.dta", replace
...
tab mwb4, miss 
	/*Sierra Leone MICS 2017: Fertility and mortality question was collected 
	from men 15-49 years. 119 men with missing age information.*/
	
tab mcm1 mcm8, miss
	/*Sierra Leone MICS 2017: 26 men report never fathering a child but who 
...
sort ind_id
save "$path_out/SLE17_MN.dta", replace
...
save "$path_out/SLE17_HH.dta", replace
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/SLE17_BH.dta"
drop _merge
erase "$path_out/SLE17_BH.dta" 
 
 
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/SLE17_WM.dta"
count if hl8>0
	/*18,006 women 15-49 years were eligible for interview. This matches the 
	country report (p.iii) */
drop _merge
erase "$path_out/SLE17_WM.dta"


*** Merging HH Recode 
*****************************************
merge m:1 hh_id using "$path_out/SLE17_HH.dta"
tab hh46 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed 
drop _merge
erase "$path_out/SLE17_HH.dta"


*** Merging MN Recode 
*****************************************
merge 1:1 ind_id using "$path_out/SLE17_MN.dta"
drop _merge
erase "$path_out/SLE17_MN.dta"


*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/SLE17_CH.dta"
drop _merge
erase "$path_out/SLE17_CH.dta"
...
//Subnational region
	/* The sample for the Sierra Leone MICS 2017 was designed to provide 
	estimates for a large number of indicators on the situation  of children 
	and women at the national level, for urban and rural areas, four regions 
	of the country (Eastern Province, Northern Province, Southern Province 
	and Western Area) and for the 14 districts of the country: (1) Kailahun, 
	(2) Kenema; (3) Kono; (4) Bombali; (5) Kambia; (6) Koinadugu; 
	(7) Port Loko; (8) Tonkolili; (9) Bo; (10) Bonthe; (11) Moyamba; 
	(12) Pujehun; (13) Western Rural; and (14) Western Urban.. pg2 */   
	
codebook hh7a, tab (99) 
...
/*In Sierra Leone, children enter primary school at age 6, junior secondary at 
age 12 and secondary school at age 15. There are 6 grades in primary school 
and 3 + 4 grades in secondary school. In primary school, grades are referred 
to as class 1 to class 6. For junior secondary school, grades are referred to 
as Junior Secondary School (JSS) 1 to 3 and in upper secondary to Senior 
Secondary School (SSS) 1 to 4 (p.215 of country report). */
...
replace eduyears = eduhighyear + 6 if edulevel==2   
	/*There are 6 grades in primary school; followed by 3 grades in junior 
	secondary school (JSS). As such we add 6 years to each of the grades 
	completed at the JSS level. */
replace eduyears = eduhighyear + 9 if edulevel==3   
	/*There are 6 grades in primary school; followed by 3 grades in junior 
	secondary school; and 4 grades in Senior Secondary School (SSS). This means, 
	individuals would have completed 9 years of schooling before reaching SSS.
	As such we add 9 years to each of the grades completed at the SSS level.*/		
replace eduyears = eduhighyear + 13 if edulevel==4 
replace eduyears = eduhighyear + 13 if edulevel==5 
	/*Individuals would have completed 13 years of schooling before reaching 
	post-secondary education, or university education or vocational/technical/
	nursing/teachers training. As such we add 13 years to each of the grades 
	completed at the higher education and vocation level.*/
replace eduyears = 13 if edulevel==5 & eduhighyear==.
	/*We assume that an individual who has vocational/technical/nursing/teachers 
	training but no information on grade, has completed high school */ 
...
replace toilet_mdg = 0 if toilet==14 
	/* The report indicate (p.284) that flush to open drain is considered as 
	improved as well as unimproved sanitation facility. We have coded it as
	unimproved facility following the SDG definition.*/	
...
replace toilet_mdg = 0 if shared_toilet==1	
	/*Sierra Leone MICS 2017: There are 6 individuals who did not response on 
	the type of toilet, but they indicated that they share their toilet 
	facilities. We replaced these individuals as deprived following the
...
Page 268 of the country report indicate that packaged water (bottled water and 
...
gen	water_mdg = 1 if water==11 | water==12 | water==13 | water==14 | ///
					 ...
					 water==71 | water==72 | water==91 | water==92 
	...
	/* Note: In Sierra Leone, packaged water (bottled water and sachet water) 
	and delivered water (tanker truck and cart with small drum/tank) are 
	treated as improved source in the report (p. 268-269). As such, we have 
	followed the report. */
...
gen	cooking_mdg = 1
replace cooking_mdg = 0 if cookingfuel>=4 & cookingfuel<=9
...
replace cooking_mdg = 0 if eu1==96 
	/*The cross tab between eu1 and eu4 indicate that of the 39 individuals
	reported using other type of cookstove, 36 of them have used solid fuel 
	on the cookstove, while 3 of them did not report the type of fuel. It is 
	very likely that these 3 individuals have also used some form of fuel that 
	is not clean. As such, we identify all individuals who reported using other 
	fuel on cookstove as deprived. */
replace cooking_mdg = 0 if cookingfuel==96 
	/*We have identified all 36 individuals who use three stone stove or open 
	fire with other forms of cooking fuel as deprived. This is because, the
	nature of the stove suggest that the fuel that is used is most likely not 
	clean.*/	
replace cooking_mdg = . if cookingfuel==99 	& eu1==99
	/*15 individuals who did not respond to type of cookstove and type of 
...
save "$path_out/sle_mics17.dta", replace 
