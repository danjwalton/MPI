...
2019 Global Multidimensional Poverty Index - Nepal DHS 2016 [STATA do-file]. 
...
/*Nepal DHS 2016: Men's questionnaire was administered in half of the households 
sampled (p.3). Among the households selected as subsample of the male survey, 
anthropometric data was collected from all chidren 0-59 months, and women 
and men age 15 and older living in these households (p. 4). More importantly, 
it should be noted that height and weight measurement was collected not only 
for women and men 15-49 years in this subsample, but also for women and men 
age 50 and above to provide background information for blood pressure 
assessments (p.4; p.298-99). 
The principles of the global MPI is to use all nutritional information made 
available by the survey for individuals up to the age of 70 years. As such, we 
have assessed the nutritional status of men and women aged 15-70 years for Nepal 
DHS 2016.*/
...
use "$path_in/NPKR7HFL.DTA", clear 
...
drop if b5==0 
	...
	//Nepal DHS 2016: 177 observations deleted

duplicates report ind_id
duplicates tag ind_id, gen(duplicates)
tab b16 if duplicates!=0 
tab hw13 if duplicates!=0
	/*For children not listed in the household a false household line is 
	created. We will check at merging stage */
...
count if v135==1
	//Check the number of children who are usual residents
...
*** Variable: AGE ***
tab hw1, miss 
	/*Note that in the case of Nepal DHS 2016, the missing values for hw1 
	represents children 0-5 who were not part of the subsample for 
...
desc weight 
summ weight
	/*Note that in the case of Nepal DHS 2016, the missing values for weight 
	represents children under 5 years who were not part of the subsample for 
	anthropometric measures */
...
desc height 
summ height
	/*Note that in the case of Nepal DHS 2016, the missing values for height
	represents children under 5 years who were not part of the subsample for 
	anthropometric measures */
...
standards */
use "$path_out/children_nutri_npl_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1 
	/*Note: In Nepal DHS 2016, 7 children were replaced as missing because
...
sort ind_id
duplicates report ind_id
save "$path_out/NPL16_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_npl_z_rc.xls"
erase "$path_out/children_nutri_npl_prev_rc.xls"
erase "$path_out/children_nutri_npl_z_rc.dta"
...
use "$path_in/NPBR7HFL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Nepal DHS 2016: these figures are identical
...
sort ind_id
save "$path_out/NPL16_BR.dta", replace	
...
use "$path_in/NPIR7HFL.DTA", clear
...
sort ind_id
save "$path_out/NPL16_IR.dta", replace
...
summ weight
	/*Note that in the case of Nepal DHS 2016, the missing values for weight 
	represents women 15-49 who were not part of the subsample for 
	anthropometric measures */


*** Variable: HEIGHT (CENTIMETERS)
codebook v438, tab (9999)
gen	height = v438/10 
	//We divide it by 10 in order to express it in centimeters
...
gen str30 datalab = "girl_nutri_npl" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_npl_z.dta", clear 
...
sort ind_id
save "$path_out/NPL16_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_npl_z.xls"
erase "$path_out/girl_nutri_npl_prev.xls"
erase "$path_out/girl_nutri_npl_z.dta"
...
use "$path_in/NPMR7HFL.DTA", clear 
...
sort ind_id
save "$path_out/NPL16_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Nepal 2016, anthropometric data was collected 
...
use "$path_in/NPPR7HFL.DTA", clear
...
count if hv104==1 & hv105>=15 & hv105<=59
	//Total number of men 15-59 years in the PR recode	
keep if hv104==1 & hv105>=15 & hv105<=59	
	//Keep only men between the age 15-59 years to merge with the MR recode	
...
keep ind_id teen_MR_temp hb2 hb3 hb13
	//Retain the height and weight variables

sort ind_id	

	//Save a temp file for merging with MR:
save "$path_out/temp.dta", replace


********************************************************************************
*** Step 1.6b  MR - MEN'S RECODE  
***(Boys 15-19 years in the household) 
********************************************************************************
...
merge 1:1 ind_id using "$path_out/temp.dta"
keep if _merge==3
	/*Note: The male individuals extracted from the PR file are many more than 
	the male individuals in the MR file. This is because the MR file only 
	contains individual men who were selected as part of the male subsample. On 
	the otherhand, the PR file contains all members of the household that was 
	interviewed. For the purpose of measuring BMI-for-age, our primary interest 
	is the men who were selected as part of the male subsample. As such, we only 
	keep the individuals who matched at _merge==3, that is, the 4,063 
	individual men from the male subsample. */  
drop _merge
...
*** Variable: SEX ***
gen gender=1 
	/*Assign all observations as "1" for male, as the MR file contains 
	only men*/
	
*** Variable: AGE IN MONTHS ***
codebook mv006, tab (12)
	//month of interview
codebook mv007, tab (10)
	//year of interview
codebook mv009, tab (100)
	//month of birth
codebook mv010, tab (100)
	//year of birth

gen imonth = mdy(mv006, 1, mv007)
	//month of interview (mv006)
	//year of interview (mv007)
gen bmonth = mdy(mv009, 1, mv010) 
	//month of birth (mv009)
	//year of birth (mv010)
gen age_month_b = (imonth-bmonth)/30.4375 
	//Calculate age in months 
...
*** Variable: BODY WEIGHT (KILOGRAMS) ***
	/*In the case of Nepal DHS 2016, the usual mv437 variable (body weight) 
...
*** Variable: HEIGHT (CENTIMETERS)
	/*In the case of Nepal DHS 2016, the usual mv438 variable (height) 
...
/* We use datalab to specify the name that will prefix the output files that 
will be produced from using this ado file*/
gen str30 datalab = "boy_nutri_npl" 
lab var datalab "Working file"	
...
use "$path_out/boy_nutri_npl_z.dta", clear 
...
gen teen_MR = 1
	...
	//Retain relevant variables:	
keep ind_id teen_MR age_month_b low_bmiage*
order ind_id teen_MR age_month_b low_bmiage*

sort ind_id

	//Erase files from folder:
erase "$path_out/boy_nutri_npl_z.xls"
erase "$path_out/boy_nutri_npl_prev.xls"
erase "$path_out/temp.dta"
erase "$path_out/boy_nutri_npl_z.dta"


	//Save a temp file for merging with PR:
save "$path_out/NPL16_MR_boys.dta", replace
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/NPL16_BR.dta"

drop _merge

erase "$path_out/NPL16_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/NPL16_IR.dta"
...
erase "$path_out/NPL16_IR.dta."


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/NPL16_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13==0 & hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Nepal DHS 2016,33 girls 15-19 were identified as measured 
...
erase "$path_out/NPL16_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/NPL16_MR.dta"
...
erase "$path_out/NPL16_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
merge 1:1 ind_id using "$path_out/NPL16_MR_boys.dta"

tab teen_MR hv118 if hv105>=15 & hv105<=19, miss col
tab hb65 if hv118==1 & teen_MR==. & (hv105>=15 & hv105<=19), miss 
	//Total number of eligible boys not interviewed
tab hb13 if teen_MR== . & hv118==1 & (hv105>=15 & hv105<=19), miss 
tab hb40 if hb65==1 & hb13==0 & hv118==1 & teen_MR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Nepal DHS 2016,24 boys 15-19 were identified as measured 
	and are present in the PR recode, but they were not present in the MR 
	recode. It should be noted that 23 of them have BMI information but not 
	BMI-for-age.*/
	
drop _merge

erase "$path_out/NPL16_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/NPL16_KR.dta"
...
tab child_KR underweight if b16==0, miss   

drop _merge

erase "$path_out/NPL16_KR.dta"
...
tab resident, miss
	...
	In the context of Nepal DHS 2016, 2,817 (5.73%) individuals who were 
...
/*
In the context of Nepal DHS 2016, height and weight measurements were
collected from children (0-5), women and men (15-49) living in 1/2 of the 
sampled households. As such there is subsample selection. hv042 identifies the 
subsample. 
*/
...
***********************************************
	/*NOTE: In the DHS datasets, we use this variable as a control 
	variable for the nutrition indicator if nutrition data is present for 
	children, women and men. Do note that in the case of Nepal nutrition data 
	was collected from all adults 15 years and older living in 50% of the 
	households selected as part of the anthropometric subsample (p.4; p.298-99). 	
	However following the global MPI criteria, we make use of the anthropometric 
	data for women and men aged 15 years and up to the age of 70 years only.*/
	
gen fem_nutri_eligible = (ha13!=.) & hv105>=15 & hv105<=70
bysort	hh_id: egen hh_n_fem_nutri_eligible = sum(fem_nutri_eligible) 	
gen	no_fem_nutri_eligible = (hh_n_fem_eligible==0) 	
drop fem_nutri_eligible hh_n_fem_nutri_eligible


gen	male_nutri_eligible = (hb13!=.) & hv105>=15 & hv105<=70
...
//Subnational region
	/*Note: The country report (page 45) indicate that collected data allowed 
	calculation of key demographic indicators, particularly fertility and 
	under-5 mortality rates, at the national level, for urban and rural areas,
	and for the country's seven provinces. This suggest subnational data is 
	representative at the province level. Provinces are labeled as Province 1-7 */
lookfor region
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Nepal, the official school entrance age is 6.  
	So, age range is 6-14 (=6+8)
	Source: Nepal DHS 2016 report (p.16)
	*/
...
********************************************************************************
	/*Note: Nepal DHS 2016 has anthropometric data for women and men aged 15 
	years and older (p.4; p.298-99). This departs from the usual DHS surveys 
	that tend to collect anthropometric data only from adults between the age 
	group of 15-49 or 15-59 years. For the purpose of the global MPI, we have 
	used nutrition data when the data is available for all individuals but up 
	to the age of 70 years. Hence, in the case of Nepal, we make use of the 
	anthropometric data for women and men aged 15 - 70 years. */
...
*** Standard MPI: BMI Indicator for Women 15 years and older ***
******************************************************************* 
...
replace f_low_bmi = . if age>70 & age<.
	/*Replace women who are 70 years and older with '.' even if they had 
...
replace hh_no_low_bmi = 1 if no_fem_nutri_eligible==1
	...
	between 15-70 years is identified as non-deprived in nutrition. */	
...
*** Standard MPI: BMI Indicator for Men 15 years and older***
******************************************************************* 
...
replace m_low_bmi = . if age>70 & age<.
	/*Replace men who are 70 years and older with '.' even if they had 
	provided nutrition information since the global MPI does not take into 
	account of nutrition information of those above 70 years. */ 
...
*** Standard MPI: BMI-for-age for individuals 15 years and older
*** 				  and BMI for individuals 20 years and older ***
******************************************************************* 
...
replace hh_no_low_bmiage = 1 if no_fem_nutri_eligible==1 & no_male_nutri_eligible==1 
...
replace hh_nutrition_uw_st = 1 if no_eligibles==1
	...
	women, men and children, as non-deprived in nutrition*/	 	
...
Note: The Nepal DHS 2016 report indicate that households that use bottled water 
for drinking are classified as using an improved source only if the water they 
use for cooking and hand washing comes from an improved source (p.9). */	
...
clonevar car = hv212  
replace car = 1 if 	sh122g==1	
	/*Nepal DHS 2016 has information on three wheeler tempo. We take into 
	account of car and tempo ownership. */
...
save "$path_out/npl_dhs16.dta", replace 
