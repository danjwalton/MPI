...
2019 Global Multidimensional Poverty Index 2.0 - Democratic Republic of the 
Congo DHS 2013-14 [STATA do-file]. Available from OPHI website: http://ophi.org.uk/  
...
********************************************************************************
*** Congo Democratic Republic DHS 2013-14 ***
********************************************************************************
...
/*In Congo DHS 2013-14, anthropometric measures are available for women (15-49) 
and children (<5) eligible in a subsample of 1/2 of households (p.7). 
Anthropometric data was not collected for men.  */
...
use "$path_in/CDPR61FL.DTA", clear 
...
tab hv120,miss	
	/*9,629 children under 5 are eligible for anthropometric 
	measurement. */	
count if hc1!=.
	//All 9,629 children under 5 have information on age in months
tab hv105 if hc1!=.
	/*A cross check with the age in years reveal that all are within the 5 year 
	age group */
tab hc13 if hv120==1, miss
	//9,388 (97.5%) of the 9,629 children have been measured
tab hc13 if hc3<=9990, miss
tab hc13 if hc2<=9990, miss
	/*For example, height and weight data is available for all 9,388 children 
...
gen str30 datalab = "children_nutri_cod" 
...
standards */
use "$path_out/children_nutri_cod_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1 	
	/* 240 children were replaced as missing because they have extreme 
...
sort ind_id
save "$path_out/COD13-14_PR_child.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_cod_z_rc.xls"
erase "$path_out/children_nutri_cod_prev_rc.xls"
erase "$path_out/children_nutri_cod_z_rc.dta"

		
********************************************************************************
*** Step 1.2  BR - BIRTH RECODE 
*** (All females 15-49 / 12-49 years who ever gave birth)  
********************************************************************************
/*The purpose of step 1.2 is to identify children of any age who died in 
the last 5 years prior to the survey date.*/

use "$path_in/CDBR61FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Congo DR 2013-14: these figures are identical
...
sort ind_id
save "$path_out/COD13-14_BR.dta", replace	
...
use "$path_in/CDIR61FL.DTA", clear
...
codebook v201 v206 v207,tab (999)
	/*Congo DR DHS 2013-14: Fertility and mortality question was only 
...
sort ind_id
save "$path_out/COD13-14_IR.dta", replace
...
tab ha13 hv103 if hv105>=15 & hv105<=19 & hv104==2 & hv042==1, miss
	/*62 of the 2,152 women 15-19 years are identified as non-eligible
...
gen str30 datalab = "girl_nutri_cod" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_cod_z.dta", clear 

	
gen	z_bmi = _zbfa
replace z_bmi = . if _fbfa==1 
	/*Congo DR DHS 2013-14: 1 girl 15-19 years were replaced as missing 
...
sort ind_id
save "$path_out/COD13-14_PR_girls.dta", replace

	
	//Erase files from folder:
erase "$path_out/girl_nutri_cod_z.xls"
erase "$path_out/girl_nutri_cod_prev.xls"
erase "$path_out/girl_nutri_cod_z.dta"
...
use "$path_in/CDMR61FL.DTA", clear 
...
codebook mv201 mv206 mv207,tab (999)
	/*Congo DR 2013-14: Fertility and mortality question was only 
...
sort ind_id
save "$path_out/COD13-14_MR.dta", replace
...
********************************************************************************
	/*Note: In the case of Congo DR DHS 2013-14, anthropometric data was not 
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/COD13-14_BR.dta"
drop _merge
erase "$path_out/COD13-14_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/COD13-14_IR.dta"
tab women_IR hv117, miss col
tab ha65 if hv117==1 & women_IR ==., miss 
	//Total number of eligible women not interviewed
drop _merge
erase "$path_out/COD13-14_IR.dta"
...
tab v201 ha65 if hv117==1, miss
	/*Note: Some 1.4% eligible women did not provide information on their birth 
...
*****************************************
merge 1:1 ind_id using "$path_out/COD13-14_PR_girls.dta"
tab girl_PR hv042 if hv105>=15 & hv105<=19 & hv104==2, miss col
drop _merge
erase "$path_out/COD13-14_PR_girls.dta"	
	
	
*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/COD13-14_MR.dta"
tab men_MR hv118 if hv027==1, miss col
tab hb65 if hv118==1 & men_MR ==. 
	//Total of eligible men not interviewed
drop _merge
erase "$path_out/COD13-14_MR.dta"	
...
*****************************************
merge 1:1 ind_id using "$path_out/COD13-14_PR_child.dta"
tab hv120, miss  
tab hc13 if hv120==1, miss
drop _merge
erase "$path_out/COD13-14_PR_child.dta"
...
tab resident, miss
	...
	In the context of Congo DR DHS 2013-14, 1,226 (1.27%) individuals who were 
...
/*
For Congo DR DHS 2013-14, height and weight measurements were
...
***********************************************
	//Note: There is no male anthropometric data for Congo DR DHS 2013-14
...
*** for adult nutrition indicator
***********************************************
		/*There is no male anthropometric data for Congo DR DHS 2013-14. So 
...
//Marital status of household member
	/*Note that in the case of Congo DR, the category divorce and not living 
	together are presented in the data as a single category. As such we are not 
	able to identify them separately */
...
//Subnational region
	/*NOTE: The sample for the Congo DHS 2013-14 was designed to provide 
	estimates of key indicators for the country as a whole, for urban and rural 
	areas separately, and for each of the 11 provinces (p.7). */ 
...
gen	child_schoolage = (age>=6 & age<=14)
	/*Note: In Congo DR, the official school entrance age is 6 years.  
...
********************************************************************************
	//Note: Congo DR DHS 2013-14 has no anthropometric data for adult men. 


lookfor body mass
codebook ha40 hb40 
	/* Note Congo DHS 2013-14: hb40 variable exists, but has all missing values 
	because anthropometric data was not collected for men */
...
******************************************************************* 
	//Note: Congo DR DHS 2013-14 has no anthropometric data for men. 
...
clonevar ndwater = hv202  
	//Congo DR DHS 2013-14 has no observation for non-drinking water 
...
char _dta[cty] "Congo, Democratic Republic of the"
char _dta[ccty] "COD"
...
save "$path_out/cod_dhs13-14.dta", replace 
