...
2019 Global Multidimensional Poverty Index - Bolivia DHS 2008 [STATA do-file]. 
...
/* Bolivia DHS 2008: Anthropometric data was collected for all women aged 15-49 
in the sampled households. This does not coincide with the subsample for 
hemoglobine tests, but it doesn't affect MPI computations, as there is no 
subsample for nutrition. */
...
use "$path_in/BOKR51FL.DTA", clear 
...
sort ind_id
save "$path_out/BOL08_KR.dta", replace
...
use "$path_in/BOBR51FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Bolivia DHS 2008: these figures are identical
...
sort ind_id
save "$path_out/BOL08_BR.dta", replace	
...
use "$path_in/BOIR51FL.DTA", clear
...
sort  ind_id
save "$path_out/BOL08_IR.dta", replace
...
count if v012>=15 & v012<=19
	*Total number of girls in the IR recode: 3,505
...
sort ind_id
save "$path_out/BOL08_IR_girls.dta", replace
...
use "$path_in/BOMR51FL.DTA", clear 
...
sort ind_id
save "$path_out/BOL08_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Bolivia 2008, anthropometric data was NOT collected 
for men*/
...
use "$path_in\BOPR51FL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/BOL08_BR.dta"

drop _merge

erase "$path_out/BOL08_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/BOL08_IR.dta"
...
erase "$path_out/BOL08_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/BOL08_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss 
	/*Note: In Bolivia DHS 2008, 50 girls 15-19 were identified as measured 
...
erase "$path_out/BOL08_IR_girls.dta"


*** Merging MR Recode 
*****************************************	
merge 1:1 ind_id using "$path_out/BOL08_MR.dta"
tab men_MR hv118, miss col
drop _merge
erase "$path_out/BOL08_MR.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/BOL08_KR.dta"
...
tab resident, miss
	...
	In the context of Bolivia DHS 2008, 608 (0.01%) individuals who were 
...
/*
In the context of Bolivia DHS 2008, height and weight measurements were
...
tab hv117 if hv104==2 & hv105>=15 & hv105<=49, miss
	/*Number of eligible women 17,655 in raw PR recode. This is same as in 
...
ta hv118 if hv104==1 & hv105>=15 & hv105<=64, miss
	/*Number of eligible men 6,684 in raw PR recode. This is same as in Final 
	Report, page 38 */
...
*****************************************
	...
	data. In the case of Bolivia DHS 2008, all women 15-49 & children under 5 
	were measured. As such there is no presence of nutrition subsample */
gen no_hem_eligible = . 	
...
//Subnational region
	/*We use the "hv024" variable that contains 9 districts */   
lookfor region
...
recode attendance (2=1)
	/*NOTE: In the case of Bolivia, "attended some time" indicate children who
	attended school in the 2007 school year. */	
...
gen child_schoolage = (age>=6 & age<=14)
	/*Note: In Bolivia, the official school entrance age is 6 years.  
...
********************************************************************************
/*Note: Bolivia DHS 2008 has no anthropometric data for men, 15-54 years */
...
********************************************************************
gen toilet_mdg = 0 if sh53==0 | sh56==4 | sh56==3
replace toilet_mdg = 1 if sh56==1 | sh56==2
replace toilet_mdg = . if sh53==9 | sh56==9
replace toilet_mdg = 0 if hv225==1 
...
clonevar ndwater = hv202  
	//No observation on non-drinking water
...
replace water_mdg = 0 if water==32 | water==42 | water==43 | water==44 | ///
...
replace water_mdg = 0 if water==71 & (ndwater==32 | ndwater==61 | ///
...
gen	roof_imp = 1 
replace roof_imp = 0 if roof==11 | roof==96  	
...
save "$path_out/bol_dhs08.dta", replace 
