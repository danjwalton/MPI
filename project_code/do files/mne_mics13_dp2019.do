...
2019 Global Multidimensional Poverty Index - Montenegro MICS 2013 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Montenegro_2013_MICS_Datasets
...
/*Montenegro MICS 2013: Anthropometric data was collected only 
...
gen str30 datalab = "children_nutri_mne" 
...
igrowup_restricted reflib datalib datalab gender age_days ageunit ///
weight height measure oedema sw


/*We now turn to using the dta file that was created and that contains 
the calculated z-scores */
use "$path_out/children_nutri_mne_z_rc.dta", clear 
...
sort ind_id
save "$path_out/MNE13_CH.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_mne_z_rc.xls"
erase "$path_out/children_nutri_mne_prev_rc.xls"
erase "$path_out/children_nutri_mne_z_rc.dta"
...
********************************************************************************
/* Note: There is no no birth history data file for Montenegro MICS 2013. 
Hence this section has been deactivated */
...
sort ind_id
save "$path_out/MNE13_WM.dta", replace
...
/* Note: Montenegro MICS 2013 did collect information from men, but not on child
mortality. Hence this section only contributes information on men's religion 
and marital status. */
...
rename marital marital_men
	...
	//Retain relevant variables:	
keep ind_id men_MN *_men 
order ind_id men_MN *_men 
sort ind_id
save "$path_out/MNE13_MN.dta", replace
...
save "$path_out/MNE13_HH.dta", replace
...
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MNE13_WM.dta"
tab hl7, miss 
gen temp = (hl7>0) 
...
drop temp _merge
erase "$path_out/MNE13_WM.dta"


*** Merging HH Recode 
*****************************************
merge m:1 hh_id using "$path_out/MNE13_HH.dta"
tab hh9 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed 
drop _merge
erase "$path_out/MNE13_HH.dta"


*** Merging MN Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MNE13_MN.dta"
drop _merge
erase "$path_out/MNE13_MN.dta"


*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MNE13_CH.dta"
drop _merge
erase "$path_out/MNE13_CH.dta"
...
//Subnational region
/*Note: The sample for the 2013 Montenegro MICS was designed to provide 
...
three regions of Montenegro: the South, Centre and North (p.3). */
codebook hh7, tab (99)
clonevar region = hh7
...
/*Montenegro MICS 2013: According to the survey report (p. 154): "children enter 
primary school when they are due to turn 6 in the calendar year which they start 
attending school, while they enter secondary school at age 15. There are nine 
grades in primary school and a maximum of four grades in secondary school." */
...
replace eduyears = eduhighyear + 9 if (edulevel==2)
	//Secondary school assumed to start after 9 years of primary education 
replace eduyears = eduhighyear + 13 if (edulevel==3)   
	//Higher education assumed to start after 13 years of education
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Montenegro, the official school entrance age is 6 years.
...
********************************************************************************
	//NOTE: Montenegro MICS 2013: No information on child mortality from men 
...
drop temp_f
	...
	/*In the case of Montenegro, this variable takes missing value because the 
...
************************************************************************
	/* In the case of Montenegro, there is no birth history data. This means, 
	there is no information on the date of death of children who have died. 
	Thus, we are not able to construct the indicator on child mortality that 
...
********************************************************************
	/*Note: In Montenegro, the report considers the category open defecation  
	as neither improve nor non-improved. But we consider it as non-improved as 
	this is clearly a deprived standard. In addition, the report, assumes 
	missing toilet information as non-improved. We follow the report. */
...
replace toilet_mdg = 0 if toilet==14 | toilet==15 | toilet==99
	/*Flush to somewhere else or don't know where or missing is considered 
	unimproved */
...
clonevar ndwater = ws2  
	/* Non-drinking water: Montenegro MICS 2013 observations only for 
	households with bottled water used for drinking */	
tab ws2 if water==91 	
...
*** Rename key global MPI indicators for estimation ***
	/* Note: In the case of Montenegro MICS 2013, there is no birth history file. 
...
save "$path_out/mne_mics13.dta", replace 
