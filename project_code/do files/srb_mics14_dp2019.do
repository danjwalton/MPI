...
2019 Global Multidimensional Poverty Index - Serbia MICS 2014 [STATA do-file].
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Serbia_2014_MICS_Datasets
...
/*Serbia MICS 2014: Anthropometric data was collected only for childen under 
5 years as indicated by the questionnaire for children under-5 that was 
administered to mothers (or caretakers) (p.5) */
...
gen str30 datalab = "children_nutri_srb" 
...
use "$path_out/children_nutri_srb_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Serbia MICS 2014, 21 children were replaced 
...
sort ind_id
save "$path_out/srb14_CH.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_srb_z_rc.xls"
erase "$path_out/children_nutri_srb_prev_rc.xls"
erase "$path_out/children_nutri_srb_z_rc.dta" 
	

	/*Note: Some 13 percent of children under 5 years have either missing info 
	on height or weight. The Serbia MICS 2014 report confirms the high missing 
	value by stating that overall only 87% of children had both their weights 
	and heights measured (p.34). As a result, this affect the size of missing 
	observations for the final nutrition indicator. Some 6 percent of the 
...
********************************************************************************
/* Note: There is no no birth history data file for Serbia MICS 2014. Hence 
...
sort ind_id
save "$path_out/srb14_WM.dta", replace
...
/* Note: There is no male data file for Serbia MICS 2014. Hence this section has 
been deactivated */
...
save "$path_out/srb14_HH.dta", replace
...
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/srb14_WM.dta"
tab hl7, miss 
gen temp = (hl7>0) 
...
drop temp _merge
erase "$path_out/srb14_WM.dta"



*** Merging HH Recode 
*****************************************
merge m:1 hh_id using "$path_out/srb14_HH.dta"
tab hh9 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed 
drop _merge
erase "$path_out/srb14_HH.dta"



*** Merging MN Recode 
*****************************************
	//There is no male data file for Serbia MICS 2014
...
*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/srb14_CH.dta"
drop _merge
erase "$path_out/srb14_CH.dta"
...
//Subnational region 
	/* Note: The national sample for the 2014 Serbia MICS was designed to 
	provide estimates for a large number of indicators on the situation of 
	children and women at the national level, for urban and other areas, and 
	for 4 regions: Belgrade; Vojvodina; Sumadija and Western Serbia; and 
...
/* According to the survey report (p. 180): "In Serbia, children enter primary 
school at age 6 and enter secondary school at age 14. There are 8 grades in 
primary school and 3 or 4 grades in secondary school. In primary school, grades 
are referred to as year 1 to year 8. For secondary school, grades are referred 
to as year 1 to year 3 or 4." */
...
replace eduyears = eduhighyear + 8 if (edulevel==3)   
	//Secondary education assumed to start after 8 years of primary education 
replace eduyears = eduhighyear + 12 if (edulevel==4) 
	/*Higher education assumed to start after 8 years of primary + 4 years of 
	secondary */
replace eduyears = 8 if edulevel==3 & eduhighyear==.  
	/*12 cases replaced in which we assume at least the years of education of 
	the previous level */
replace eduyears = 12 if edulevel==4 & eduhighyear==.  
	/*1 case replaced in which we assume at least the years of education of the 
	previous level */	
...
gen	child_schoolage = (age>=7 & age<=15)
	/*Note: In Serbia, the official primary school entrance age is 7 years.  
...
********************************************************************************
	//NOTE: Serbia MICS 2014: No information on child mortality from men 
...
drop temp_f
	...
	/*In the case of Serbia, this variable takes missing value because the 
...
****************************************
	...
	not know where is considered as non-improved. However, in Serbia, this 
	particular category is identified as improved sanitation facilities (p.87 
	...
	this is clearly a deprived standard. In addition, in Serbia, the report, 
...
replace toilet_mdg = 0 if toilet==14
 	//Household is assigned a value of '0' if it flushes to somewhere else		
...
replace television = 1 if hc8s == 1
...
*** Rename key global MPI indicators for estimation ***
	/* Note: In the case of Serbia MICS 2014, there is no birth history file. 
...
save "$path_out/srb_mics14.dta", replace 
