...
2019 Global Multidimensional Poverty Index - Peru DHS-Cont 2012 [STATA do-file]. 
...
/*Peru DHS 2012: Anthropometric information were recorded for all eligible women 
...
use "$path_in/PEPR6IFL.DTA", clear 


*** Generate a household unique key variable at the household level using: 
	***hv001=cluster number 
	***hv002=household number
gen g=substr(hhid, -2, 2)
destring g, replace
gen double hh_id = hv001*10000000 + hv002*1000 + g*100 
format hh_id %20.0g
...
gen double ind_id = hv001*10000000 + hv002*1000 + g*100 + hvidx 
format ind_id %20.0g
...
tab hv120,miss	
	/*10,231 children under 5 are eligible for anthropometric 
	measurement. */	
count if hc1!=.
	//All 10,231 children under 5 have information on age in months
tab hv105 if hc1!=.
	/*A cross check with the age in years reveal that all are within the 5 year 
	age group */
tab hc13 if hv120==1, miss
	//9,978 (97.53%) of the 10,231 children have been measured
tab hc13 if hc3<=9990, miss
tab hc13 if hc2<=9990, miss
	/*For example, height and weight data is available for all 9,978 children 
...
sum age	
tab age age_months if age>59.99
replace age = age_months if age>59.99
...
gen str30 datalab = "children_nutri_per" 
...
standards */
use "$path_out/children_nutri_per_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1 	
	/* 3 children were replaced as missing because they have extreme 
...
sort ind_id
save "$path_out/PER12_PR_child.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_per_z_rc.xls"
erase "$path_out/children_nutri_per_prev_rc.xls"
erase "$path_out/children_nutri_per_z_rc.dta"
...
use "$path_in/PEBR6IFL.DTA", clear
...
gen g=substr(caseid, -4, 2)
destring g, replace
gen double ind_id = v001*10000000 + v002*1000 + g*100 + v003 
format ind_id %20.0g
label var ind_id "Individual ID"
...
compare tot_child_died tot_child_died_2
	//Peru DHS-Cont 2012: these figures are identical
...
sort ind_id
save "$path_out/PER12_BR.dta", replace	
...
use "$path_in/PEIR6IFL.DTA", clear
...
codebook v201 v206 v207,tab (999)
	/*Peru DHS 2012: Fertility and mortality question was only 
...
sort ind_id
save "$path_out/PER12_IR.dta", replace
...
tab ha13 hv103 if hv105>=15 & hv105<=19 & hv104==2, miss
	/*250 of the 4,895 women 15-19 years are identified as non-eligible
...
gen str30 datalab = "girl_nutri_per" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_per_z.dta", clear 

	
gen	z_bmi = _zbfa
replace z_bmi = . if _fbfa==1 
	/*Peru DHS 2012: 1 girl 15-19 years were replaced as missing because 
...
sort ind_id
save "$path_out/PER12_PR_girls.dta", replace

	//Erase files from folder:
erase "$path_out/girl_nutri_per_z.xls"
erase "$path_out/girl_nutri_per_prev.xls"
erase "$path_out/girl_nutri_per_z.dta"
...
********************************************************************************
	...
	/*Note: There is no male recode file for Peru DHS-Cont 2012. Hence the 
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/PER12_BR.dta"
drop _merge
erase "$path_out/PER12_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/PER12_IR.dta"
tab women_IR hv117, miss col
tab ha65 if hv117==1 & women_IR ==., miss 
	//Total number of eligible women not interviewed
drop _merge
erase "$path_out/PER12_IR.dta"
...
tab v201 ha65 if hv117==1, miss
	/*Note: Some 2.71% eligible women did not provide information on their birth 
...
*****************************************
merge 1:1 ind_id using "$path_out/PER12_PR_girls.dta"
tab ha13 girl_PR if hv105>=15 & hv105<=19 & hv104==2, miss col
drop _merge
erase "$path_out/PER12_PR_girls.dta"


*** Merging MR Recode 
*****************************************
//Note: There is no male recode file for Peru DHS 2012.
...
*****************************************
merge 1:1 ind_id using "$path_out/PER12_PR_child.dta"
tab hv120, miss  
tab hc13 if hv120==1, miss
drop _merge
erase "$path_out/PER12_PR_child.dta"
...
tab resident, miss
	...
	In the context of Peru DHS-Cont 2012, 2,842 (2.65%) individuals who were 
...
/*
In the context of Peru DHS-Cont 2012, height and weight measurements were
collected from all children (0-5) and women (15-49). As such there is no 
presence of nutrition subsample. 
*/
...
*** for adult nutrition indicator
***********************************************
		/*There is no male recode file for Peru DHS 2012. So 
...
recode relationship (1=1)(2=2)(3=3)(11=3)(4/10=4)(15=6)(12=5)
...
//Subnational region
	/*NOTE: The total sample allows indicator estimates for households and for 
	women between 15-49 years of age for a number of important variables in 
	each of the 24 departments and natural region of the country (p383) */ 
lookfor region
...
gen child_schoolage = (age>=6 & age<=14)
	/* In Peru, the official school entrance age is 6 years. 
...
********************************************************************************
	//Peru DHS 2012 has no anthropometric data for adult men 
...
******************************************************************* 
	//Note: Peru DHS 2012 has no anthropometric data for men. 
...
drop temp_f
	...
	/*Since there is no male data file for Peru, this variable is created as 
	an emplty variable */
...
home, roundtrip */
********************************************************************
	/*Note: The country report of Peru DHS 2012 does not specify whether 
	drinking water that is sourced from a well inside the dwelling is improved 
	or non-improved. Since there is lack of information on the quality of water 
	drawn from the well in the dwelling, we have identified this particular 
	category as non-imporved.*/
...
replace water_mdg = 0 if water==22 | water==41 | water==43 | ///
						 water==21 | water==61 | water==96 
...
replace water_mdg = 0 if water==71 & ///
						(ndwater==22 | ndwater==41 | ndwater==43 | /// 
						 ndwater==61 | ndwater==96)
...
gen	wall_imp = 1 
replace wall_imp = 0 if wall<=24 | wall==41 | wall==96  
...
gen	roof_imp = 1 
replace roof_imp = 0 if roof<=22 | roof==41 | roof==96  
...
tab cookingfuel cooking_mdg, miss	
	...
	/*Note that in Peru DHS-Cont 2012, the category 'other' cooking fuel is not 
...
codebook hv208 sh61j
clonevar television = hv208 
replace television=1 if television==0 & sh61j==1
...
save "$path_out/per_dhs12.dta", replace 
