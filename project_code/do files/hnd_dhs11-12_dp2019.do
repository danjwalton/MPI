...
2019 Global Multidimensional Poverty Index - Honduras DHS 2011-12 [STATA do-file]. 
...
/*Honduras DHS 2011 2012: Anthropometric information were recorded for all 
eligible children born since 2006 (age 0-59 months) on 21,362 hh (100% of the 
sample) and women aged 15-49 (p.5). Anthropometric information was not collected
from men 15-54*/
...
use "$path_in/HNKR62FL.DTA", clear 
...
gen str30 datalab = "children_nutri_hnd" 
...
tab hw2 if hw2>990,m nol   
	//Missing values are 994 to 999
replace weight = . if hw2>=990 
	//All missing values or out of range are replaced as "."
tab	hw13 hw2 if hw2>=990 | hw2==., miss 
...
standards */
use "$path_out/children_nutri_hnd_z_rc.dta", clear 
...
sort ind_id
duplicates report ind_id
save "$path_out/hnd11-12_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_hnd_z_rc.xls"
erase "$path_out/children_nutri_hnd_prev_rc.xls"
erase "$path_out/children_nutri_hnd_z_rc.dta"
...
use "$path_in/HNBR62FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Honduras DHS 2011-12: these figures are identical
...
sort ind_id
save "$path_out/hnd11-12_BR.dta", replace	
...
use "$path_in/HNIR62FL.DTA", clear
...
sort  ind_id
save "$path_out/hnd11-12_IR.dta", replace
...
gen str30 datalab = "girl_nutri_hnd" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_hnd_z.dta", clear 
...
sort ind_id
save "$path_out/hnd11-12_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_hnd_z.xls"
erase "$path_out/girl_nutri_hnd_prev.xls"
erase "$path_out/girl_nutri_hnd_z.dta"
...
use "$path_in/HNMR62FL.DTA", clear 
...
sort ind_id
save "$path_out/hnd11-12_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Honduras 2011-2012, anthropometric data was NOT collected 
for men, hence variables in this section carry missing values.*/
...
sort ind_id
save "$path_out/hnd11-12_MR_boys.dta", replace
...
use "$path_in/HNPR62FL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/hnd11-12_BR.dta"

drop _merge

erase "$path_out/hnd11-12_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/hnd11-12_IR.dta"
...
erase "$path_out/hnd11-12_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/hnd11-12_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss 
	/*Note: In Honduras DHS 2011-2012, 97 girls 15-19 were identified as measured 
...
erase "$path_out/hnd11-12_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/hnd11-12_MR.dta"

tab men_MR hv118, miss col

drop _merge

erase "$path_out/hnd11-12_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
merge 1:1 ind_id using "$path_out/hnd11-12_MR_boys.dta"

	//No anthropometric data for boys in Honduras DHS 2011-12
	
drop _merge

erase "$path_out/hnd11-12_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/hnd11-12_KR.dta"
...
erase "$path_out/hnd11-12_KR.dta"
...
tab resident, miss
	...
	In the context of Honduras DHS 2011-12, 3,025 (3%) individuals who were 
...
/*
In the context of Honduras DHS 2011-2012, height and weight measurements were
collected from all children (0-5) and women (15-49) for the full sample of 
households. Men were not measured. As such there is no subsample selection. 
*/
...
*****************************************
tab hv117 if hv104==2 & hv105>=15 & hv105<=49, miss
	/*number of eligible women: 24,414 in raw PR recode 
	(same as in Final Report, page 51)*/
...
*****************************************
ta hv118 if hv104==1 & hv105>=15 & hv105<=64, miss
	/*number of eligible men 8,179 in raw PR recode 
	(same as in Final Report, page 51)*/
...
tab hv115 marital, miss
	//Note: For Honduras DHS 2011-2012 marital status is not available
...
//Subnational region
	//For Honduras DHS 2011-2012 hv024 variable contains 18 regions  
lookfor region
codebook hv024, tab (100)	
clonevar region = hv024
lab var region "Region for subnational decomposition"
label define reg 1"Atlántida" 2"Colón" ///
				 3"Comayagua" 4"Copán" 5"Cortés" 6"Choluteca" ///
				 7"El Paraíso" 8"Francisco Morazán" 9"Gracias a Dios" ///
...
codebook attendance, tab (10)
	/*NOTE: In the case of Honduras, "attended some time" indicate children who
	attended school in the 2010 school year. */
...
gen child_schoolage = (age>=6 & age<=14)
	/*The Final Report of the DHS indicates that primary educ starts at 6 years 
	of age and the National Law of Education states that primary school goes 
...
********************************************************************************
//Note: Honduras DHS 2011-2012 has no anthropometric data for men, 15-54 years 
...
codebook electricity, tab (10)
	//HND DHS 2011-12 has no data on electricity.
...
recode hv225 (9=.), gen(shared_toilet)  


	
*** Standard MPI ***
****************************************
	/*NOTE: Table 10.8 in the report indicate that any type of flush toilet 
	are considered improved. This would imply that categories=13 
	(Flush toilet does not know connection) & =14 (Flush toilet with connection 
	to open water) are considered as improved sanitation. Table on page 38 
	showing progress the MDGs indicators indicate that while all types of flush 
	toilet (including don't know where and to open water) are considered as 
	improved sanitation, pit latrine is considered as non-improved. */

gen toilet_mdg = ((toilet<23 | toilet==41) & shared_toilet==0) 
...
*** Standard MPI ***
***************************************
	/*NOTE: There are two categories referring to wells: 
	=21 "Well water with pump", 
	=31 "Well water with winch". 
	Table 10.8 (page 265) of the report considers these categories as improved 
	sources of water: piped water, water from wells, public tap, rainwater and 
	bottled water, and they consider as unimproved sources: surface water, 
	tanker trunk, and spring/stream/waterhole. Thus, =21 and =31 are 
	considered here as improved.
	
	The category on  "Public fountain" is equivalent to public tap. Following 
	this, it is considered unimproved.*/

gen water_mdg = 1 if water==11 | water==12 | water==13 | water==14 | ///
					 water==21 | water==31 | water==41 | water==44 | ///
					 water==51 | water==71  
...
replace water_mdg = 0 if water==32 | water==42 | water==43 | water==45 | ///
...
replace water_mdg = 0 if water==71 & (ndwater==32 | ndwater==42 | ///
						 ndwater==43 | ndwater==44 | ndwater==45 | ///
...
tab cookingfuel cooking_mdg, miss	
	...
	/*Note that in Honduras, the category 'other' cooking fuel is not 
...
save "$path_out/hnd_dhs11-12.dta", replace 
