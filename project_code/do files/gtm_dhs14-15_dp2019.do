...
2019 Global Multidimensional Poverty Index 2.0 - Guatemala DHS 2014-15 
...
/*Guatemala DHS 2014-15: Anthropometric information were recorded for all 
eligible children age 0-59 months and eligible women aged 15-49 (p.4). 
...
use "$path_in/GUKR71FL.DTA", clear 
...
duplicates report ind_id 
	//We should not have any duplicates at this stage
...
gen str30 datalab = "children_nutri_gtm" 
...
standards */
use "$path_out/children_nutri_gtm_z_rc.dta", clear 
...
sort ind_id
duplicates report ind_id
save "$path_out/GTM14-15_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_gtm_z_rc.xls"
erase "$path_out/children_nutri_gtm_prev_rc.xls"
erase "$path_out/children_nutri_gtm_z_rc.dta"
...
use "$path_in/GUBR71FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Guatemala DHS 2014-15: these figures are identical
...
sort ind_id
save "$path_out/GTM14-15_BR.dta", replace	
...
use "$path_in/GUIR71FL.DTA", clear
...
sort ind_id
save "$path_out/GTM14-15_IR.dta", replace
...
gen str30 datalab = "girl_nutri_gtm" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_gtm_z.dta", clear 
...
sort ind_id
save "$path_out/GTM14-15_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_gtm_z.xls"
erase "$path_out/girl_nutri_gtm_prev.xls"
erase "$path_out/girl_nutri_gtm_z.dta"
...
use "$path_in/GUMR71FL.DTA", clear 
...
********************************************************************************
/*Note: In the case of Guatemala 2014-15, anthropometric data was NOT collected 
for men, hence this step is irrelevant. */
...
sort ind_id
save "$path_out/GTM14-15_MR_boys.dta", replace
...
use "$path_in/GUPR71FL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GTM14-15_BR.dta"

drop _merge

erase "$path_out/GTM14-15_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GTM14-15_IR.dta"
...
erase "$path_out/GTM14-15_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/GTM14-15_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss 
	/*Note: In Guatemala DHS 2014-15, 39 girls 15-19 were identified as measured 
...
erase "$path_out/GTM14-15_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GTM14-15_MR.dta"

tab men_MR hv118, miss col

drop _merge

erase "$path_out/GTM14-15_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
merge 1:1 ind_id using "$path_out/GTM14-15_MR_boys.dta"

drop _merge

erase "$path_out/GTM14-15_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GTM14-15_KR.dta"
...
erase "$path_out/GTM14-15_KR.dta"
...
tab resident, miss
	...
	In the context of Guatemala DHS 2014-15, 1,197 (1.2%) individuals who were 
...
/*
In the context of Guatemala DHS 2014-15, height and weight measurements were
collected from children (0-5) and women (15-49) for the full sample of 
households. 
*/
...
********************************************************************************
*** 1.13 RENAMING DEMOGRAPHIC VARIABLES ***
********************************************************************************
...
//Subnational region
	/* NOTE: According to the report the unit of study and analysis is the 
	department (p.6). Also, to be in line with the decompositions published in 
	the past, we use the "shdepto2" variable that contains 22 districts + 
	Guatemala municipio. */
lookfor region
codebook shdepto2, tab (100)	
clonevar region = shdepto2
lab var region "Region for subnational decomposition"
label define region_lab 1"Guatemala Municipio" 2"Guatemala Resto" ///
3"El Progreso" 4"Sacatepéquez" 5"Chimaltenango" ///
6"Escuintla" 7"Santa Rosa" 8"Sololá" 9"Totonicapán" ///
10"Quetzaltenango" 11"Suchitepéquez" 12"Retalhuleu" ///
13"San Marcos" 14"Huehuetenango" 15"Quiché" 16"Baja Verapaz" ///
17"Alta Verapaz" 18"Petén" 19"Izabal" 20"Zacapa" 21"Chiquimula" ///
22"Jalapa" 23"Jutiapa" 
label values region region_lab
tab shdepto2 region, miss 
...
bys hh_id: egen hh_years_edu6_1 = max(years_edu6)
...
gen child_schoolage = (age>= 7 & age<= 15)
	/*Note: In Guatemala, the official school entrance age is 7 years.  
	So, age range is 7-15 (=7+8)
	Source: The tables in DHS Report report the Primary Net Attendance Rate for 
	the age range of 7-12. Thus, age range 7-14. */
...
********************************************************************************
/*Note: Guatemala DHS 2014-15 has no anthropometric data for men, 15-59 years */
...
***************************************
	...
	/*For Guatemala, we followed the DHS report in which public fountain/tank, 
	mechanical or manual well, and bottled water are considered not improved */

gen water_mdg = 1 if water==11 | water==12 | water==13 | water==14 | ///
                     water==21 | water==43 | water==51 
...
replace water_mdg = 0 if water==31 | water==32 | water==41 | water==42 | ///
						 water==44 | water==61 | water==71 | water==96 
...
save "$path_out/gtm_dhs14-15.dta", replace 
