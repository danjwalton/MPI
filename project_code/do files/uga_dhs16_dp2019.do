...
2019 Global Multidimensional Poverty Index - Uganda DHS 2016 [STATA do-file]. 
...
/*Uganda DHS 2016: In the subsample of one-third households selected for the 
male survey, height and weight information was collected from eligible women 
15-49 years and men 15-54 years, as well as children age 0-59 months (p.5)*/
...
drop if b5==0 
	//Exclude children who are not alive 
	//Uganda DHS 2016: 812 observations deleted MP
...
standards */
use "$path_out/children_nutri_uga_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Uganda DHS 2016, 29 children were replaced as
...
erase "$path_out/children_nutri_uga_z_rc.xls"
erase "$path_out/children_nutri_uga_prev_rc.xls"
erase "$path_out/children_nutri_uga_z_rc.dta"

	
	//Save a temp file for merging with PR:
save "$path_out/UGA16_KR.dta", replace
...
compare tot_child_died tot_child_died_2
	//Uganda DHS 2016: these figures are identical
...
sort ind_id
save "$path_out/UGA16_BR.dta", replace	
...
sort ind_id
save "$path_out/UGA16_IR.dta", replace
...
count if v012>=15 & v012<=19
	//Total number of girls in the IR recode: 4,276	
...
gen str30 datalab = "girl_nutri_uga" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_uga_z.dta", clear 
...
sort ind_id
save "$path_out/UGA16_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_uga_z.xls"
erase "$path_out/girl_nutri_uga_prev.xls"
erase "$path_out/girl_nutri_uga_z.dta"
...
sort ind_id
save "$path_out/UGA16_MR.dta", replace
...
/*Note: In the case of Uganda DHS 2016, anthropometric data was collected 
...
count if hv104==1 & hv105>=15 & hv105<=59
	//Total number of men 15-59 years in the PR recode: 19,817	
...
tab hb2 if hb2<=999 & _merge==2, miss
	//check

keep if _merge==3
	/*Note: we only keep the individuals who matched at _merge==3, that is, the 
	5,336 individual men from the PR file. The large majority of the mismatch 
	have missing weight or height information. */  
drop _merge
...
*** Variable: BODY WEIGHT (KILOGRAMS) ***
	/*In the case of Uganda DHS 2016, the usual mv437 variable (body weight) 
...
*** Variable: HEIGHT (CENTIMETERS)
	/*In the case of Uganda DHS 2016, the usual mv438 variable (height) 
...
count if mv012>=15 & mv012<=19
	//Total number of boys in the MR recode: 1,270	
...
/* We use datalab to specify the name that will prefix the output files that 
will be produced from using this ado file*/
gen str30 datalab = "boy_nutri_uga" 
lab var datalab "Working file"	
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores to compute BMI-for-age*/
use "$path_out/boy_nutri_uga_z.dta", clear 
...
sort ind_id
save "$path_out/UGA16_MR_boys.dta", replace


	//Erase files from folder:
erase "$path_out/boy_nutri_uga_z.xls"
erase "$path_out/boy_nutri_uga_prev.xls"
erase "$path_out/temp.dta"
erase "$path_out/boy_nutri_uga_z.dta"
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/UGA16_BR.dta"

drop _merge

erase "$path_out/UGA16_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/UGA16_IR.dta"
...
erase "$path_out/UGA16_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/UGA16_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13==0 & hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Uganda DHS 2016, 72 girls 15-19 were identified as measured 
...
erase "$path_out/UGA16_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/UGA16_MR.dta"
...
erase "$path_out/UGA16_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
merge 1:1 ind_id using "$path_out/UGA16_MR_boys.dta"
...
tab hb40 if hb65==1 & hb13==0 & hv118==1 & teen_MR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Uganda DHS 2016, 57 boys 15-19 were identified as measured 
	and are present in the PR recode, but they were not present in the IR recode. 
	It should be noted that they have BMI information but not BMI-for-age.*/
	
drop _merge

erase "$path_out/UGA16_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/UGA16_KR.dta"
...
tab resident, miss
	...
	In the context of Uganda DHS 2016, 2,115 (2.29%) individuals who were 
...
Uganda DHS 2016: In the subsample of one-third households selected for the 
male survey, height and weight information was collected from eligible women 
and men, as well as children age 0-59 months (p.3)
*/

tab  hv027 hv042, miss
...
//Subnational region
	/* NOTE: The sample for the Uganda DHS 2016 was designed to provide 
	estimates of key indicators for the country as a whole, for urban and rural 
	areas separately, and for each of the 15 regions. */  	
lookfor region
...
gen	child_schoolage = (age>=6 & age<=14)
	/*Note: In Uganda, the official school entrance age to primary education is 
...
tab hv202 if water==72 	
/*Because the quality of bottled/sachet water is not known, households using 
bottled or sachet water for drinking are classified as using an improved or 
unimproved source according to their water source for non-drinking activities 
such as cooking and hand washing. However, it is important to note that 
households using bottled water for drinking are classified as unimproved source 
if this is explicitly mentioned in the country report. */	
...
replace water_mdg = 0 if water==71 & ///
						(ndwater==32 | ndwater==42 | ndwater==43 | /// 
						 ndwater==61 | ndwater==63 | ndwater==96)
replace water_mdg = 0 if water==72 & ///
						(ndwater==32 | ndwater==42 | ndwater==43 | /// 
						 ndwater==61 | ndwater==63 | ndwater==96)						 
	/*Households using bottled water/sachet water for drinking are classified as using an 
...
tab cookingfuel cooking_mdg, miss	
	...
	/*Note that in Uganda DHS 2016, the category 'other' cooking fuel is not 
...
save "$path_out/uga_dhs16.dta", replace 
