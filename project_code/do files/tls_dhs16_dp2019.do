...
2019 Global Multidimensional Poverty Index - Timor-Leste DHS 2016 
...
/*Timor-Leste DHS 206: All women age 15-49 and children age 0-59 months were 
eligible for anthropometric measurements. In one-third of the sampled households, 
all men age 15-59 were eligible for individual interview. In this subsample of 
households men age 15-49 were eligible for anthropometric measurements. Also in 
this subsample of households anemia testing was performed among consenting women 
age 15-49 and consenting men age 15-59, and among children age 6-59 months whose 
parents or guardians consented. (Report, p.2) */

********************************************************************************
*** Step 1.1 KR - CHILDREN's RECODE (under 5)
********************************************************************************

use "$path_in/TLKR71FL.DTA", clear 
...
gen double ind_id = v001*10000 + v002*100 + b16 
format ind_id %20.0g
...
drop if b5==0 
	...
	//Timor-Leste DHS 2015-16: 265 observations deleted
...
gen str30 datalab = "children_nutri_tls" 
...
standards */
use "$path_out/children_nutri_tls_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Timor-Leste DHS 2016, 332 children were replaced 
...
erase "$path_out/children_nutri_tls_z_rc.xls"
erase "$path_out/children_nutri_tls_prev_rc.xls"
erase "$path_out/children_nutri_tls_z_rc.dta"

	
	//Save a temp file for merging with PR:
save "$path_out/TLS16_KR.dta", replace
...
use "$path_in/TLBR71FL.DTA", clear
...
gen double ind_id = v001*10000 + v002*100 + v003 
format ind_id %20.0g
label var ind_id "Individual ID"
...
compare tot_child_died tot_child_died_2
	//Timor-Leste DHS 2016: these figures are identical
...
sort ind_id
save "$path_out/TLS16_BR.dta", replace	
...
use "$path_in/TLIR71FL.DTA", clear
...
sort ind_id
save "$path_out/TLS16_IR.dta", replace
...
count if v012>=15 & v012<=19
	//Total number of girls in the IR recode: 3,126	
...
gen str30 datalab = "girl_nutri_tls" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_tls_z.dta", clear 
...
sort ind_id
save "$path_out/TLS16_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_tls_z.xls"
erase "$path_out/girl_nutri_tls_prev.xls"
erase "$path_out/girl_nutri_tls_z.dta"
...
use "$path_in/TLMR71FL.DTA", clear 
...
gen double ind_id = mv001*10000 + mv002*100 + mv003 	
format ind_id %20.0g
label var ind_id "Individual ID"
...
sort ind_id
save "$path_out/TLS16_MR.dta", replace
...
********************************************************************************
	/*Note: In the case of Timor-Leste 2016, anthropometric data was collected 
	for men living in one third of the households selected. This information, 
	however, was not recorded in the male recode, but only on the household 
	members recode.*/
...
use "$path_in/TLPR71FL.DTA", clear 

gen double ind_id = hv001*10000 + hv002*100 + hvidx 
format ind_id %20.0g
label var ind_id "Individual ID"
...
*** Merging info from men' s recode 
*****************************************
...
count if mv012>=15 & mv012<=19
	//Total number of boys in the MR recode: 1,053	
...
/* We use datalab to specify the name that will prefix the output files that 
will be produced from using this ado file*/
gen str30 datalab = "boy_nutri_tls" 
lab var datalab "Working file"	
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores to compute BMI-for-age*/
use "$path_out/boy_nutri_tls_z.dta", clear 
...
save "$path_out/TLS16_MR_boys.dta", replace
...
erase "$path_out/boy_nutri_tls_z.xls"
erase "$path_out/boy_nutri_tls_prev.xls"
erase "$path_out/boy_nutri_tls_z.dta"
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TLS16_BR.dta"

drop _merge

erase "$path_out/TLS16_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TLS16_IR.dta"
...
erase "$path_out/TLS16_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/TLS16_IR_girls.dta"
...
tab ha2 if hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19) & ha65==1, miss
tab ha3 if hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19) & ha65==1, miss
	/*Note: There are 36 girls for which there is anthropometric information
	on the PR recode but there were no info on the IR recode.*/

drop _merge

erase "$path_out/TLS16_IR_girls.dta"


*** Merging MR Recode 15-59 years
*****************************************
merge 1:1 ind_id using "$path_out/TLS16_MR.dta"
...
erase "$path_out/TLS16_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
merge 1:1 ind_id using "$path_out/TLS16_MR_boys.dta"
...
tab hb65 hb13 if hv118==1 & teen_MR==. & (hv105>=15 & hv105<=19), miss   
tab hb2 if hb65==1 & hb13==0 & teen_MR== . & hv118==1 & (hv105>=15 & hv105<=19), miss   
tab hb3 if hb65==1 & hb13==0 & teen_MR== . & hv118==1 & (hv105>=15 & hv105<=19), miss   
	/*Note: There seems to be 20 boys for which there is anthropometric 
	information on the PR recode but for which there was probably not age month 
	info on the IR recode.*/
	
drop _merge

erase "$path_out/TLS16_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TLS16_KR.dta"
count if b16==0 
tab b16 child_KR, miss
tab b16 women_BR, miss
	//The children without household line are unique to the KR and BR recode
...
codebook hc3 if hc13==0 & hv120==1 & child_KR==.   
sum hc5 if hc13==0 & hv120==1 & child_KR==.   
codebook hc3 if hc13==0 & hv120==1 & child_KR==.
codebook hc2 if hc13==0 & hv120==1 & child_KR==.
	/* There seems to be 815 eligible children who were measured but whose 
	anthropometric information is not included in the child recode (probably 
	because the criteria mentioned above). */
...
*****************************************
	/* The hv042 (household selected for hemoglobin) is a variable that 
	indicates whether there is selection of subsample for anthropometric 
	measures. In a significant number of country surveys, only half of the 
	household or one third or two third of the household is assessed for 
...
********************************************************************************
*** Step 1.11 SUBSAMPLE VARIABLE ***
********************************************************************************

/*
In the context of Timor-Leste DHS 2016, height and weight measurements were
collected from children (0-5) and women (15-49) in all households; height and 
weight measurements were collected from men (15-59) living in 1/3 of the sampled 
households. Thus we will estimate the GMPI using the full sample. There is no
presence of subsample.
*/
...
/*Subnational representation: check report at which subnational level estimates 
are representative */
	/*NOTE: "There are five geographic regions in Timor-Leste, and these are 
	subdivided into 12 municipalities and special administrative region (SAR) of 
	Oecussi. The 2016 TLDHS sample was designed to produce reliable estimates of 
	indicators for the country as a whole, for urban and rural areas, and for 
	each of the 13 municipalities." (Report, p.2) These municipalities are 
	identified by the variable hv024.*/   
clonevar region = hv024
label define lab_regions 1"Aileu" 2"Ainaro" 3"Baucau" 4"Bobonaro" ///
5"Cova Lima" 6"Dili" 7"Ermera" 8"Lautem" 9"Liquica" 10"Manatuto" ///
11"Manufahi" 12"Oecussi" 13"Viqueque"
label values region lab_regions
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Timor-Leste, the official school entrance age for primary school 
	is 6 years. So, age range is 6-14 (=6+8)
...
********************************************************************************
	/*Note: Timor-Leste DHS 2016 has anthropometric data for men (15-59 years) 
	only for 1/3 of the households. */
...
*** Standard MPI: BMI Indicator for Men 15-59 years ***
******************************************************************* 
...
foreach var in low_bmiage low_bmiage_b {
			 gen inf_`var' = 1 if `var'!=.
			 bysort sex: tab age inf_`var' 
			 drop inf_`var'
			 }
...
save "$path_out/tls_dhs16.dta", replace 
