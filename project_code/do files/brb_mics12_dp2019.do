...
2019 Global Multidimensional Poverty Index -  Barbados MICS 2012 [STATA do-file]. 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Barbados_2012_MICS_Datasets
...
gen str30 datalab = "children_nutri_brb" 
...
use "$path_out/children_nutri_brb_z_rc.dta", clear 
...
/* Create the child nutrition variables following WHO standards */
...
erase "$path_out/children_nutri_brb_z_rc.xls"
erase "$path_out/children_nutri_brb_prev_rc.xls"
erase "$path_out/children_nutri_brb_z_rc.dta"
...
********************************************************************************
	...
	/* Note: There is no birth history data file for Barbados MICS 2012. Hence 
...
********************************************************************************
	...
	//Note: Barbados MICS 2012 has no male recode file. 
...
tab hh7 region, miss 
drop temp
label define lab_reg 1"Christ Church & St. Philip" 2"St Michael" ///
3"St. James, St. George, & St.Thomas" ///
4"St. Lucy, St. Peter, St. Andrew, St. Joseph, & St. John" 
...
/*  
According to the country report (p.89), the admission age to compulsory primary 
education in Barbados is 5 years. There are one grade for pre-primary plus six 
grades at the primary level. At the secondary level there are five grades. 
In addition, secondary schools offer 6th form education (post-secondary, 
non-tertiary) for two additional years. */
...
replace eduhighyear = 0 if edulevel< 3
	/*The variable "eduhighyear" was replaced with a '0' if edulevel is 
	preschool */ 
...
replace eduyears = eduhighyear + 7 if (edulevel==4) 
	//Secondary comes after 7 years of primary education
replace eduyears = eduhighyear + 12 if (edulevel==5)   
	/*Post secondary education is assumed to start after 12 years of general 
	education (7 years primary + 5 years secondary)*/
replace eduyears = eduhighyear + 14 if (edulevel==6) 
	/*University after 14 years of education: 7 years of primary + 5 years of 
	secondary + 2 years of post-secondary */
...
gen	child_schoolage = (age>=5 & age<=13)
	/*Note: In Barbados, the official school entrance age is 5 years  
	So, age range is 5-13(=5+8)
	Source: country report, p.89 & http://data.uis.unesco.org/?ReportId=163*/  
...
********************************************************************************
	/*NOTE: Barbados MICS 2012 has no data on child mortality. */
...
********************************************************************
	/*Note: According to the Barbados country report, all sources are considered
	as improved, except other facilities. In addition, the report assumes 
	missing toilet information as non-improved. We follow the report(p.48).
	Furthermore, the report considers the category open defecation as neither 
	improved nor non-improved. But we consider it as non-improved as this is 
	clearly a deprived standard. */

gen	toilet_mdg = 1 if toilet<=22 & shared_toilet!=1 
replace toilet_mdg = 0 if toilet<=22 & shared_toilet==1 
replace toilet_mdg = 0 if toilet==95 | toilet==96
...
replace water_mdg = 0 if water==96 
...
codebook hc8c hc8b hc8d hc9b hc8e hc9e hc9c hc9d hc8o hc8k hc11
...
save "$path_out/brb_mics12.dta", replace 
