...
2019 Global Multidimensional Poverty Index - Tanzania DHS 2015-16 [STATA do-file]. 
...
/*Tanzania DHS 2015-2016: All eligible women in all sampled household were 
weighed and measured for anthropometric indicators. In addition, all children 
under 5 were weighed and measured for anthropometric indicators (p.14-15). 
A male survey was conducted at the same time in a subsample consisting of one 
household from every three selected for the female survey. However, 
anthropometric information was not collected from men 15-49 */
...
gen str30 datalab = "children_nutri_tza" 
...
standards */
use "$path_out/children_nutri_tza_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Tanzania DHS 2015-16, 61 children were replaced as
...
erase "$path_out/children_nutri_tza_z_rc.xls"
erase "$path_out/children_nutri_tza_prev_rc.xls"
erase "$path_out/children_nutri_tza_z_rc.dta"

	
	//Save a temp file for merging with PR:
save "$path_out/TZA15-16_KR.dta", replace
...
compare tot_child_died tot_child_died_2
	//Tanzania DHS 2015-16: these figures are identical
...
sort ind_id
save "$path_out/TZA15-16_BR.dta", replace	
...
sort ind_id
save "$path_out/TZA15-16_IR.dta", replace
...
count if v012>=15 & v012<=19
	//Total number of girls in the IR recode: 2,932	
...
gen str30 datalab = "girl_nutri_tza" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_tza_z.dta", clear 
...
sort ind_id
save "$path_out/TZA15-16_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_tza_z.xls"
erase "$path_out/girl_nutri_tza_prev.xls"
erase "$path_out/girl_nutri_tza_z.dta"
...
sort ind_id
save "$path_out/TZA15-16_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Tanzania 2015-16, anthropometric data was NOT collected 
...
sort ind_id
save "$path_out/TZA15-16_MR_boys.dta", replace
...
*** Merging BR Recode 
*****************************************
merge m:m ind_id using "$path_out/TZA15-16_BR.dta"

drop _merge

erase "$path_out/TZA15-16_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TZA15-16_IR.dta"
...
erase "$path_out/TZA15-16_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/TZA15-16_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13==0 & hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Tanzania DHS 2015-16, 53 girls 15-19 were identified as measured
	and are presentin the PR recode, but they were not present in the IR recode.*/
	
drop _merge

erase "$path_out/TZA15-16_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TZA15-16_MR.dta"

tab men_MR hv118, miss col 

drop _merge

erase "$path_out/TZA15-16_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
	/*Note: Anthropometric data was not collected for men as part of the 
	Tanzania DHS 2015-16 dataset. */	
merge 1:1 ind_id using "$path_out/TZA15-16_MR_boys.dta"

	
drop _merge

erase "$path_out/TZA15-16_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TZA15-16_KR.dta"
...
erase "$path_out/TZA15-16_KR.dta"
...
tab resident, miss
	...
	In the context of Tanzania DHS 2015-16, 2,449 (3.75%) individuals who were 
...
/*
The hv042 (household selected for hemoglobin) is a variable that indicates 
whether there is selection of subsample for anthropometric measures. In a 
significant number of country surveys, only half of the household or one third 
or two third of the household is assessed for anthropometric measures.

In the context of Tanzania DHS 2015-16, height and weight measurements were
collected from all children inder 5 and women (15-49). As such there is no 
...
//Subnational region
	/* NOTE: To estimate geographic differentials for certain demographic 
	indicators, Tanzania was divided into nine geographic zones. Although these 
	zones are not official administrative areas, this classification system is 
	also used by the Reproductive and Child Health Section. Grouping the regions 
	into zones allowed a relatively large number of people in the denominator 
	and a reduced sampling error (p.13).*/
codebook shreg1, tab (100)	
clonevar region = shreg1
...
gen	child_schoolage = (age>=7 & age<=15)
	/*
	Note: In Tanzania, the official school entrance age is 7 years.  
...
********************************************************************************
	//Note: Tanzania DHS 2015-16 has no anthropometric data for men, 15-49 years
...
clonevar ndwater = hv202  
	//Tanzania DHS 2015-16: No observation for non-drinking water
...
label define lab_floor ///
	...
	32  "vinyl/asphaltstrips" ///
	33  "ceramic tiles/terrazzo" ///
	34  "ceramic/concrete" /// 
	35  "carpet" ///
	96  "other"
...
****************************************
gen	cooking_mdg = 1
replace cooking_mdg = 0 if cookingfuel>=7 & cookingfuel<12 
	//12=bottled gas is identified as clean fuel
...
tab cookingfuel cooking_mdg, miss	
	...
	/*Tanzania DHS 2015-16: kerosene is identified in the report as solid 
	fuel (p.33). However, DHS team has confirmed that this is an error in their
	survey report.
	
	The category 'other' cooking fuel is not identified as solid fuel. Hence 
	this particular category is identified as non-deprived. */
...
save "$path_out/tza_dhs15-16.dta", replace 
