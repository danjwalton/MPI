...
2019 Global Multidimensional Poverty Index - Myanmar DHS 2015-16 [STATA do-file]. 
...
/*Mayanmar DHS 2015-16: Anthropometric information were recorded for all 
children age 0-59 months and women age 15-49 (p.3). */
...
use "$path_in/MMKR71FL.DTA", clear 
...
drop if b5==0 
	...
	//Myanmar DHS 2015-16: 218 observations deleted
...
gen str30 datalab = "children_nutri_mmr" 
...
use "$path_out/children_nutri_mmr_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1 
	/*Note: In Myanmar DHS 2015-16, 24 children were replaced as missing because
...
sort ind_id
save "$path_out/MMR15-16_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_mmr_z_rc.xls"
erase "$path_out/children_nutri_mmr_prev_rc.xls"
erase "$path_out/children_nutri_mmr_z_rc.dta"
...
use "$path_in/MMBR71FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Myanmar DHS 2015-16: these figures are identical
...
sort ind_id
save "$path_out/MMR15-16_BR.dta", replace	
...
use "$path_in/MMIR71FL.DTA", clear
...
sort ind_id
save "$path_out/MMR15-16_IR.dta", replace
...
gen str30 datalab = "girl_nutri_mmr" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_mmr_z.dta", clear 
...
sort ind_id
save "$path_out/MMR15-16_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_mmr_z.xls"
erase "$path_out/girl_nutri_mmr_prev.xls"
erase "$path_out/girl_nutri_mmr_z.dta"
...
use "$path_in/MMMR71FL.DTA", clear 
...
sort ind_id
save "$path_out/MMR15-16_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Myanmar 2015-16, anthropometric data was NOT collected 
...
sort ind_id
save "$path_out/MMR15-16_MR_boys.dta", replace
...
use "$path_in/MMPR71FL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MMR15-16_BR.dta"

drop _merge

erase "$path_out/MMR15-16_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MMR15-16_IR.dta"
...
erase "$path_out/MMR15-16_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/MMR15-16_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13==0 & hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Myanmar DHS 2015-16, 79 girls 15-19 were identified as measured 
...
erase "$path_out/MMR15-16_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MMR15-16_MR.dta"

tab men_MR hv118, miss col
	//No anthropometric data for men in Myanmar DHS 2015-16

drop _merge

erase "$path_out/MMR15-16_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
merge 1:1 ind_id using "$path_out/MMR15-16_MR_boys.dta"

tab teen_MR hv118 if hv105>=15 & hv105<=19, miss col 
	//No anthropometric data for boys in Myanmar DHS 2015-16
	
drop _merge

erase "$path_out/MMR15-16_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MMR15-16_KR.dta"
...
erase "$path_out/MMR15-16_KR.dta"
...
tab resident, miss
	...
	In the context of Myanmar DHS 2015-16, 2,076 (3.73%) individuals who were 
...
/*
In the context of Myanmar DHS 2015-16, height and weight measurements were
collected from all children (0-5) and women (15-49) in the sampled 
households (p.3). So there is no subsample selection.
*/
...
//Subnational region
	/*NOTE: The sample for the Myanmar DHS 2015-16 was designed to provide 
	estimates of key indicators for the country as a whole, for urban and rural 
	areas separately, and for each of the seven States and eight Regions of 
	Myanmar (p.1).*/   
lookfor region
...
gen	child_schoolage = (age>=5 & age<=13)
	/*
	Note: In Myanmar, the official school entrance age is 5 years.  
...
********************************************************************************
	//Note: Myanmar DHS 2015-16 has no anthropometric data for men.
...
lab var m_bmi "Male's BMI"
	//Myanmar DHS 2015-16 has no anthropometric data for men.
...
clonevar ndwater = hv202  
	//Myanmar DHS 2015-16 has no observation for non-drinking water
...
****************************************
gen	cooking_mdg = 1
replace cooking_mdg = 0 if cookingfuel>=6 & cookingfuel<95 
...
save "$path_out/mmr_dhs15-16.dta", replace 
