...
2019 Global Multidimensional Poverty Index - Turkmensitan MICS 2015-16 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Turkmenistan_MICS5_Datasets
...
/* Turkmenistan MICS 2015-16: P32 indicates that anthropometric data was 
collected only for childen under 5 years. */
...
gen str30 datalab = "children_nutri_tkm" 
...
use "$path_out/children_nutri_tkm_z_rc.dta", clear 
...
sort ind_id
save "$path_out/TKM15-16_CH.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_tkm_z_rc.xls"
erase "$path_out/children_nutri_tkm_prev_rc.xls"
erase "$path_out/children_nutri_tkm_z_rc.dta"
...
codebook ln bhln, tab (100)
	/* Note: Two variables exist for line number: ln ("Line number of women")
	and bhln ("Line number"). We have chosen to go with ln, because it refers to
	women in the label. They do not carry identical information. */
gen double ind_id = hh1*100000 + hh2*100 + ln 
format ind_id %20.0g
label var ind_id "Individual ID"
...
sort ind_id
save "$path_out/TKM15-16_BH.dta", replace	
...
tab wb2, miss
	//75 women with missing age information

tab cm1 cm8, miss
	/* Turkimenistan MICS 2015-16: one observation of a woman who had never 
	given birth but had a child who later died. This will be recoded as missing 
	later */
...
sort ind_id
save "$path_out/TKM15-16_WM.dta", replace
...
/* Note: There is no MN dta file for Turkmenistan MICS 2015-16. Hence this 
...
save "$path_out/TKM15-16_HH.dta", replace
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TKM15-16_BH.dta"
drop _merge
erase "$path_out/TKM15-16_BH.dta" 
 
 
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TKM15-16_WM.dta"
tab hl7, miss 
gen temp = (hl7>0) 
...
drop temp
drop _merge
erase "$path_out/TKM15-16_WM.dta"

 	
*** Merging HH Recode 
*****************************************
merge m:1 hh_id using "$path_out/TKM15-16_HH.dta"
tab hh9 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed 
drop _merge
erase "$path_out/TKM15-16_HH.dta"


*** Merging MN Recode 
*****************************************
	//No male recode for Turkmenistan MICS 2015-16
...
*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TKM15-16_CH.dta"
count if ln==0 
...
drop _merge
erase "$path_out/TKM15-16_CH.dta"
...
//Subnational region
	//Split into the six regions (p.4).
codebook hh7, tab (99)	
decode hh7, gen(temp)
...
/* According to the Turkmenistan MICS 2015-16 survey report (p.133): "According 
to the Law on Education from 2012 (2013/2014 school year), children enter 
primary school at age 6 and enter secondary school at age 10 in Turkmenistan. 
There are 4 grades in primary school and 8 grades in secondary school (in total, 
12 grades). According to the previous Law on Education, children entered primary 
school at age 7 and there were 3 grades in primary school and 8 grades in 
secondary school (in total, 11 grades)." */

tab ed4b ed4a, miss
tab age ed6a if ed5==1, miss
	//Check: For those currently in school, check their level of schooling
	/*In the case of Turkmenistan MICS 2015-16, there is inconsistency such as 
...
replace eduyears = 0 if edulevel<=0 & eduhighyear==.   
	/*Assuming 0 year if they only attend preschool or primary but the last year 
	is unknown. Turkmenistan MICS 2015-16: primary not separated from secondary, 
	so this command makes 0 changes */
replace eduyears = eduhighyear + 11 if (edulevel==2 | edulevel==3)   
	/*Professional education assumed to start after 11 years of general 
	education based on prior law */
replace eduyears = eduhighyear + 11 if (edulevel==4) 
	//University after 11 years of education, based on prior law 
replace eduyears = . if (edulevel==1 & eduhighyear > 12) 
...
lookfor school
	/*Note that MICS may have more than one variable for school attendance 
	for current year. Please check */
codebook ed5, tab (10)
gen	attendance = .
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Turkmenistan, the official school entrance age is 6 years. 
...
********************************************************************************
	//NOTE: Turkmenistan MICS 2015-16: No information on child mortality from men 
...
drop temp_f
	...
	/* In the case of Turkmenistan, this variable takes missing value because 
...
tab ws9 shared_toilet, miss nol
	//0=no;1=yes;.=missing
label define lab_shared_toi 0"no" 1"yes"
label values shared_toilet lab_shared_toi
codebook shared_toilet, tab (10)
...
gen	toilet_mdg = ((toilet<23) & shared_toilet!=1) 
	//Deprived if: no facility, bush, field 
...
clonevar ndwater = ws2  
	/* Turkmenistan MICS 2015-16: observations only if main source of drinking 
	water is bottled water */
...
gen	water_mdg = 1 if water==11 | water==12 | water==13 | water==21 | ///
	...
	/*Non deprived if water is "piped into dwelling", "piped to yard/plot", 
	 ...
	 Piped into neighbour's yard is considered an improved source (p.71) */
...
replace water_mdg = 0 if water_mdg==1 & timetowater>=30 & timetowater!=. & timetowater!=998 
	//Deprived if water is at more than 30 minutes' walk (roundtrip) 

replace water_mdg = . if water==. | water==99
codebook ndwater, tab (100)
replace water_mdg = 0 if water==91 & (ndwater==61) 
...
gen	floor_imp = 1 
	//Note: There's no observation with natural floor in Turkmenistan.
...
replace cooking_mdg = 0 if cookingfuel>5 & cookingfuel<95 
	/*In the case of Turkmenistan MICS 2015-16, there is no observation in the 
	sample that uses solid fuels for cooking. */
...
codebook hc8c hc8f hc8b hc8d hc9b hc8e hc9c hc11 hc9d hc9h


clonevar television = hc8c
replace television = 1 if hc8f==1
	/* Turkmenistan MICS 2015-16: there are two variables for television - hcf8c 
	(television neither plasma nor mesomorphic) and hc8f (television plasma or 
	mesomorphic/LCD) */	
gen bw_television   = .
clonevar radio = hc8b 
...
clonevar car =  hc9h
replace car = 1 if hc9i==1
	/* Turkmenistan MICS 2015-16: hc9h (Passenger Car) and hc9i (Truck). Here, 
	either one will be considered as having this asset. */	
clonevar bicycle = hc9c
clonevar motorbike = hc9d
clonevar computer = hc8j
replace computer = 1 if hc9k==1
	//Turkmenistan MICS 2015-16: hc9k: ownership of a tablet
gen animal_cart = .
...
save "$path_out/tkm_mics15-16.dta", replace 
