...
2019 Global Multidimensional Poverty Index - Guinea MICS 2016 [STATA do-file]. 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Guinea_MICS5_Datasets
...
/*Guinea MICS 2016:
Four sets of questionnaires were used in the survey (p.4):
(i) a household questionnaire that was used to collect basic demographic 
information on all de jure household members (usual residents), the household 
and the dwelling;
(ii) a women's individual questionnaire administered in each household to all 
women aged 15-49;
(iii) an individual questionnaire for children under 5 years of age 
administered to mothers (caretakers) for all children under 5 living in the 
household. This questionnaire includes a biomarker module (test for anemia and 
malaria) administered to children aged 6-59 months, and;
(iv) a questionnaire on verbal autopsy administered to mothers (caretakers) for 
all children under 5 years old who died during the last 3 years. 

Anthropometric data was collected only from children under 5 (p.7).
*/
...
gen str30 datalab = "children_nutri_gin" 
...
use "$path_out/children_nutri_gin_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Guinea MICS 2016, 96 children were replaced 
...
sort ind_id
save "$path_out/GIN16_CH.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_gin_z_rc.xls"
erase "$path_out/children_nutri_gin_prev_rc.xls"
erase "$path_out/children_nutri_gin_z_rc.dta"
...
gen date_death = bh4c + bh9c	
	//Date of death = date of birth (bh4c) + age at death in months (bh9c)	
...
sort ind_id
save "$path_out/GIN16_BH.dta", replace
...
sort ind_id
save "$path_out/GIN16_WM.dta", replace
...
********************************************************************************
	/*Note: There is no male recode data for Guinea MICS 2016. As such this 
...
save "$path_out/GIN16_HH.dta", replace
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GIN16_BH.dta"
drop _merge
erase "$path_out/GIN16_BH.dta" 
 
 
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GIN16_WM.dta"
tab hl7, miss 
gen temp = (hl7>0) 
...
drop temp
drop _merge
erase "$path_out/GIN16_WM.dta"


*** Merging HH Recode 
*****************************************
merge m:1 hh_id using "$path_out/GIN16_HH.dta"
tab hh9 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed 
drop _merge
erase "$path_out/GIN16_HH.dta"


*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GIN16_CH.dta"
count if ln==0 
	//The children without household line are unique to the CH recode 
replace hh_id = hh1*100 + hh2 if ln==0 
	//Created hd_id for children without household line
drop _merge
erase "$path_out/GIN16_CH.dta"
...
//Subnational region 
	/*Note: The Guinea MICS 2016 sample was designed to provide estimates for a 
	large number of indicators on the situation of children and women at the 
	national, urban and rural levels, and each of the eight administrative 
	regions namely Conakry, Boké, Kindia, Labé, Mamou, Faranah, Kankan and
	N 'Zérékoré (p.4) */
codebook hh7, tab (99)
clonevar region = hh7
tab hh7 region, miss 
label define region_lab 1 "Boké" 2 "Conakry" 3 "Faranah" 4 "Kankan" ///
						5 "Kindia" 6 "Labé" 7 "Mamou" 8 "N'Zérékoré" 
...
/*The education model in Guinea consists of 13 years of basic education
with 6 years of compulsory primary education, 4 years of lower secondary, and
3 years of upper secondary. The admission age to compulsory education is 7 years. 
Preschool education takes place from age 4. Primary education takes place from 
age 6-12. Lower secondary education takes place from age 13-16. Upper secondary
education takes place from age 17-19.

References: 
http://uis.unesco.org/en/country/gn
https://www.epdc.org/sites/default/files/documents/EPDC%20NEP_Guinea.pdf
*/
...
replace edulevel = . if ed4a==. | ed4a==8 |ed4a==9  
	//ed4a=8/9/98/99 are missing values 
...
replace eduyears = 6 if (edulevel==2 & eduhighyear==0)
	//Individuals in college with zero grades have at least completed primaire
replace eduyears = 10 if (edulevel==3 & eduhighyear==0)
	//Individuals in college with zero grades have at least completed primaire
replace eduyears = eduhighyear + 13 if (edulevel>=4 & edulevel<.)   
...
gen	child_schoolage = (age>=7 & age<=15)
	/*Note: According to the information in the UIS website (and other sites), 
	7 is the official entrance age to compulsory education in Guinea. But the 
	survey report defines primary education 6 to 11. We used the starting age of 
	7, given that the wider literature seem to attest to 7 years.
	So in Guinea, the official school entrance age is 7 years. 
	Age range is 7-15 (=7+8). 
...
********************************************************************************
	//NOTE: Guinea MICS 2016: No information on child mortality from men 
...
drop temp_f
	...
	/* In the case of Guinea, this variable takes missing value because the 
...
*** Standard MPI ***
****************************************
	/*Note: Following the MDG definition, 'flush to somewhere else' is 
	considered as non-improved, as it is here. However, in Guinea, 'flush to 
	unknown place' is identified as improved sanitation facilities in the 
	country report (p. 136). So it is included in "improved" here too. */
...
*** Standard MPI ***
****************************************
	/*Note: According to the MICS report bottled water and water in sachets 
	is improved or unimproved depending on its source; and rainwater is 
	unimproved.*/ 
...
replace water_mdg = 0 if water==32 | water==42 | water==51 | water==71 | ///
						 water==81 | water==96 
...
replace water_mdg = 0 if (water==91 | water==92) & ///
						 (ndwater==32 | ndwater==51 | ndwater==71 | ///
						  ndwater==81 | ndwater==96 | ndwater==61) 
	/*Households using bottled water or sachets for drinking are classified as 
	using an improved or unimproved source according to their water source for 
...
save "$path_out/gin_mics16.dta", replace 
