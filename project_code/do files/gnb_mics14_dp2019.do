...
2019 Global Multidimensional Poverty Index - Guinea-Bissau MICS 2014 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Guinea_Bissau_MICS5_Datasets
...
/*Guinea-Bissau MICS 2014: P20 indicate that anthropometric data was collected 
...
gen str30 datalab = "children_nutri_gnb" 
...
use "$path_out/children_nutri_gnb_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Guinea-Bissau MICS 2014, 30 children were 
...
erase "$path_out/children_nutri_gnb_z_rc.xls"
erase "$path_out/children_nutri_gnb_prev_rc.xls"
erase "$path_out/children_nutri_gnb_z_rc.dta"
...
tab wb2, miss 
	//510 women 15-49 years with missing age info

tab cm1 cm8, miss
	/*Women who has never ever given birth will not have information on 
	child mortality. There are 8 women who indicated not having given birth 
	ever, but who report having had a child that died. This will be corrected
	later in the dofile */
...
//Subnational region
	/*For Guinea Bissau, the sample is representative at 
	national level for urban and rural environments and for 9 regions */
codebook hh7, tab (99)
clonevar region = hh7
label define reg 1"Tombali" 2"Quinara" 3"Oio" 4"Biombo" 5"Bolama" ///
6"Bafata" 7"Gabu" 8"Cacheu" 10"SAB"    
...
/* Note: The education system in Guinea-Bissau:
This is based on the UNESCO information (confirmed on page 202 of report):
http://uis.unesco.org/country/GW
Currently the education model holds in Guinea-Bissau, consisting of 9 years of 
basic education with 6 years of compulsory primary and 3 years of compulsory 
secondary. The admission age to compulsory education is 6 years. Preschool 
education takes place from age 3. Primary education takes place from age 6-11 
(grades 1-6). Lower secondary education takes place from age 12-14 (grades 7-9). 
Upper secondary education takes place from age 15-16 (grades 10-11). A total of 
11 years of education complete primary and secondary levels */

tab ed4b ed4a, miss
tab age ed6a if ed5==1, miss
	/*In the case of Guinea-Biassau MICS 2014, there are a few inconsistencies 
...
replace eduyears = eduhighyear + 6 if (edulevel==2)
	/*People who completed secondary first had to complete 6 years of primary */
replace eduyears = eduhighyear + 11 if (edulevel==3)   
	/*Professional education assumed to start after 11 years of general 
	education, https://www.epdc.org/country/guineabissau */
replace eduyears = eduhighyear + 11 if (edulevel==4) 
	/*University after 12 years of education, 6 years of primary + 5 years of 
	secondary */

replace eduyears = . if (edulevel==2 & eduhighyear>12) 
	/* Guinea-Bissau report groups primary & secondary together under General 
	Education. */
...
gen	child_schoolage = (age>=6 & age<=14)
	/*Note: In Guinea Bissau, the official school entrance age is 6 years  
...
gen	water_mdg = 1 if water==11 | water==12 | water==13 | water==14 | ///
	 ...
	 /*Non-deprived: piped water (into dwelling, compound, yard or plot, to 
	  neighbour, public tap/standpipe), tube well/borehole, protected well, 
	  protected spring and rainwater collection. Rainwater collection, however, 
	  was discarded in the calculations as described in the end of p 125*/
...
gen	roof_imp = 1 
replace roof_imp = 0 if roof<=13 | roof==96
...
replace television = 1 if hc8j==1
	//hc8j: tv plasma
gen bw_television   = .
clonevar radio = hc8b 
...
clonevar car = hc9g 	
clonevar bicycle = hc9d
clonevar motorbike = hc9e
clonevar computer = hc8f
replace computer = 1 if hc9c==1
	//hc9c: laptop/notebook 
...
save "$path_out/gnb_mics14.dta", replace 
