...
2019 Global Multidimensional Poverty Index - Nigeria MICS 2016-17 [STATA do-file]. 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Nigeria_MICS5_Datasets
...
/* Nigeria MICS 2016-17: P4 indicates that anthropometric data was collected 
for all childen under 5 years. */
...
count if _fwei==1 | _flen==1 
	/*Note: In Nigeria MICS 2016-17, 159 children were replaced as missing 
...
erase "$path_out/children_nutri_nga_z_rc.xls"
...
codebook ln bhln, tab (100)
	/* Two non-identical variables exist instead of wm4 for women's line number,
	ln and bhln; We used ln, because it alligns with the HL dta file. */
gen double ind_id = hh1*100000 + hh2*100 + ln
format ind_id %20.0g
label var ind_id "Individual ID"
...
tab cm1 cm8, miss
	...
	child mortality
	Note: In Nigeria MICS 2016-17, 39 women reported as not ever given birth 
	but had had child who died. */
...
//Subnational region
	//Representative at the 37 subnational states
lookfor region
lookfor state
...
/*	Basic education in Nigeria consists of 9 years in total: 6 years of
	compulsory primary and 3 years of compulsory junior secondary. An additional 
	3 years of senior secondary education is not compulsory. The admission age 
	to compulsory education is 6 years. Preschool education takes place from 
	age 3-5 years. Primary education takes place from age 6-11+ (grades 1-3, 
	lower basic education; grades 4-6, middle basic education). Junior secondary 
	education takes place from age 12-14 (grades 7-9). Senior secondary 
	education takes place from age 15-18 (grades 10-12). Reference: 
http://www.ibe.unesco.org/fileadmin/user_upload/Publications/WDE/2010/pdf-versions/Nigeria.pdf 
*/

tab ed4b ed4a, miss
codebook ed4b ed4a ed3, tab (30)
tab age ed6a if ed5==1, miss
	//Check: For those currently in school, check their level of schooling
	/*In the case of Nigeria MICS 2016-17, there is inconsistency such as 
...
replace edulevel = 0 if ed3==1 & ed4a==4 
	//Those who never attended non-formal school are replaced as '0'
...
clonevar newedu = ed4b
codebook newedu, tab (100)
recode newedu (0/10=0)(11=1)(12=2)(13=3)(14=4)(15=5)(16/20=6)(21=7)(22=8) ///
				(23=9)(24=10)(25=11)(26/30=12)(31/35=16)(36=18)
codebook newedu, tab (199)
label define lab_newed 1 "1" 0 "0" 2 "2" 3 "3"
label values newedu lab_newed
lab var newedu "Highest grade completed at that level"
	/* Recoded above to correspond to actual years in school. Note that all
	university degrees coded together as "16" years of education (12+4). We do 
	not know how many years each individual took to gain their degrees, only 
	that they have the accreditation itself. Therefore we chose the common 4 
	years degree as a benchmark. Additionally, all postgraduate diplomas
	are coded as 18 years (16+2, e.g. a common length for a master's).*/
codebook newedu, tab (30)
	/* Recoding done to ease the coding below. See survey report questionnaire
	(p.422) for more information on coding categories. */

clonevar eduhighyear = newedu
	//Highest grade of education completed
replace eduhighyear = .  if newedu==. 
	//Check that ed4b=97/98/99 are missing values
replace eduhighyear = 0  if ed3==2
replace eduhighyear = 0  if ed3==1 & ed4a==4
	//Those who never attended school are replaced as '0'
...
tab age if eduhighyear==0 & edulevel==2
replace eduyears = . if eduhighyear==0 & edulevel==2

tab age if eduhighyear==0 & edulevel==3
replace eduyears = . if eduhighyear==0 & edulevel==3
...
replace attendance = 0  if ed3==1 & ed4a==4 
	//Replace attendance with '0' if attended school but informal schooling
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Nigeria, the official school entrance age is 6 years. 
...
replace toilet_mdg = 0 if toilet == 14 
	...
	"flush to somewhere else" and "flush don't know where"  */
  	/*Note: 
	However, in Nigeria, "flush to unknown place" is identified as improved 
	sanitation facilities (p.118 report), while "flush to somewhere else" is 
	identified as non-improved. As such, we make adjustments following report */
...
gen	water_mdg = 1 if water==11 | water==12 | water==13 |  ///
					 water==14 | water==21 | water==31 |  ///
					 water==41 | water==51 | water==91 | water==92 
...
replace water_mdg = 0 if water==92 & ///
						(water==32 | water==42 | water==61 | ///
						 water==71 | water==81 | water==96)
	/*Households using sachet (pure) water for drinking are classified as using 
	an improved or unimproved source according to their water source for 
	non-drinking activities (p.107)	*/
