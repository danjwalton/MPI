...
2019 Global Multidimensional Poverty Index - Rwanda DHS 2014-2015 
...
********************************************************************************
	...
	/* In Rwanda DHS 2014-15: anthropometric information was recorded for all 
	eligible children age 0-59 months and eligible women aged 15-49 living in 
	in a subsample of 50 percent of the households not selected for the men’s 
	survey (p.1, p.165). In addition, height and weight information was also 
	collected from eligible men aged 15-59 years in half of households selected 
	for male survey (p.9). This suggest, that we can retain 100% of the sample 
	because in half of the households sampled information on nutrition will 
	come from eligible women and children, while the other half of the 
	households, the information is exclusively from eligible adult men. */
	
	
********************************************************************************
*** Step 1.1 PR - CHILDREN's RECODE (under 5)
********************************************************************************
	/*For the purpose of the Global MPI, in order to compute the child related 
	health indicators, we have consistently done it using the KR file. However,
	in the context of Rwanda DHS 2014-15, the quality of the KR file is not
	at par with other KR files of other countries. There is some difference 
	between members in the KR file and the same members in the PR file. After
	going through carefully the files, it was decided that we would construct 
	the child under 5 heatlh indicator from the PR file*/

use "$path_in/RWPR70FL.DTA", clear 
...
count if hv105<=5
	//The dataset has 9,506 children under 5
count if hv120==1	
	/*However, only 3,928 children under 5 are eligible for anthropometric 
	measurement.*/	
count if hc1!=.
	//All 3,928 children under 5 have information on age in months
tab hv105 if hc1!=.
	/*A cross check with the age in years reveal that all are within the 5 year 
	age group */
tab hc13 if hc1!=., miss
	/*Of the 3,928 children under 5, 3910 have been measured while 18 have 
	missing information */
tab hc13 if hc1!=. & hc3<=9990, miss
tab hc13 if hc1!=. & hc2<=9990, miss
	/*For example, height and weight data is available for all 3910 children 
...
count if hv102==1 
	//Check the number of children who are usual residents: 3,880
...
gen str30 datalab = "children_nutri_rwa" 
...
use "$path_out/children_nutri_rwa_z_rc.dta", clear  
...
erase "$path_out/children_nutri_rwa_z_rc.xls"
erase "$path_out/children_nutri_rwa_prev_rc.xls"
erase "$path_out/children_nutri_rwa_z_rc.dta"
save "$path_out/RWA14-15_KR.dta", replace
...
use "$path_in/RWBR70FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Rwanda DHS 2014-15: these figures are identical
...
sort ind_id
save "$path_out/RWA14-15_BR.dta", replace	
...
use "$path_in/RWIR70FL.DTA", clear
...
sort ind_id
save "$path_out/RWA14-15_IR.dta", replace
...
gen str30 datalab = "girl_nutri_rwa" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_rwa_z.dta", clear 
...
sort ind_id
save "$path_out/RWA14-15_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_rwa_z.xls"
erase "$path_out/girl_nutri_rwa_prev.xls"
erase "$path_out/girl_nutri_rwa_z.dta"
...
use "$path_in/RWMR70FL.DTA", clear 
...
sort ind_id
save "$path_out/RWA14-15_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Rwanda DHS 2014-15, anthropometric data was collected 
...
keep if _merge==3
	...
	keep the individuals who matched at _merge==3, that is, the 6,217 
...
*** Variable: SEX ***
gen gender=1 
	/*Assign all observations as "1" for male, as the MR file contains all 
...
*** Variable: BODY WEIGHT (KILOGRAMS) ***
	/*In the case of Rwanda DHS 2014-15, the usual mv437 variable (body weight) 
...
*** Variable: HEIGHT (CENTIMETERS)
	/*In the case of Rwanda DHS 2014-15, the usual mv438 variable (height) 
...
/* We use datalab to specify the name that will prefix the output files that 
will be produced from using this ado file*/
gen str30 datalab = "boy_nutri_rwa" 
lab var datalab "Working file"	
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores to compute BMI-for-age*/
use "$path_out/boy_nutri_rwa_z.dta", clear 
...
sort ind_id
save "$path_out/RWA14-15_MR_boys.dta", replace


	//Erase files from folder:
erase "$path_out/boy_nutri_rwa_z.xls"
erase "$path_out/boy_nutri_rwa_prev.xls"
erase "$path_out/temp.dta"
erase "$path_out/boy_nutri_rwa_z.dta"
...
*** Merging BR Recode 
*****************************************
merge m:m ind_id using "$path_out/RWA14-15_BR.dta"

drop _merge

erase "$path_out/RWA14-15_BR.dta"



*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/RWA14-15_IR.dta"
...
erase "$path_out/RWA14-15_IR.dta"



*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/RWA14-15_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13==0 & hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Rwanda DHS 2014-15, 1 girl 15-19 were identified as measured 
	and are present in the PR recode, but she is not present in the IR recode. 
	It should be noted that she has BMI information but not BMI-for-age.*/
drop _merge

erase "$path_out/RWA14-15_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/RWA14-15_MR.dta"
...
erase "$path_out/RWA14-15_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
merge 1:1 ind_id using "$path_out/RWA14-15_MR_boys.dta"
...
tab hb40 if hb65==1 & hb13==0 & hv118==1 & teen_MR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Rwanda DHS 2014-15, 11 BOYS 15-19 were identified as measured 
	and are present in the PR recode, but THEY ARE not present in the IR recode. 
	It should be noted that they have BMI information but not BMI-for-age.*/
drop _merge

erase "$path_out/RWA14-15_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/RWA14-15_KR.dta"

drop _merge

erase "$path_out/RWA14-15_KR.dta"
...
tab resident, miss 
	...
	In the context of Rwanda DHS 2014-15, 888 (1.6%) individuals who were 
...
/*
In the context of Rwanda DHS 2014-15, height and weight measurements were
collected from all women 15-49 years and all children under five living in 
1/2 of the sampled households (p.1) which was not selected for the male 
survey (p.165). In addition, height and weight information was also collected 
from eligible men in the other half of households selected for male survey (p.9). 
This suggest, that we can retain 100% of the sample because in half of the 
households sampled information on nutrition will come from eligible women and 
children, while the other half of the households, the information is exclusively 
from eligible adult men. As such for the purpose of the global MPI, we make use 
of the full sample.*/
...
//Subnational region
	/* NOTE: In Rwanda DHS 2014-15: The survey was designed to produce 
	representative estimates for the main demographic and health indictors for 
	the country as a whole, for the urban and rural areas, and for each of the 
	five provinces. See Appendix A: Sample implementation (p.313).*/
lookfor region
...
gen	child_schoolage = (age>=7 & age<=15)
	/*
	Note: In Rwanda, the official school entrance age is 7 years.  
...
*** Standard MPI: BMI Indicator for Adult Men 15-59 years ***
******************************************************************* 
...
replace hh_nutrition_uw_st = 1 if no_eligibles==1
	/*If country have collected anthropometric data from women 15-49, 
...
clonevar ndwater = hv202  
	//Rwanda DHS 2014-15: No observation for non-drinking water. 
...
tab cookingfuel cooking_mdg, miss	
	...
	/*Note that in Rwanda DHS 2014-15, the category 'other' cooking fuel is not 
...
save "$path_out/rwa_dhs14-15.dta", replace 
