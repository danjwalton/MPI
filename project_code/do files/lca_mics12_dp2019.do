...
2019 Global Multidimensional Poverty Index - Saint Lucia MICS 2012
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Saint_Lucia_2012_MICS_Datasets
...
all children under 5 living in the household (page 20). */
...
gen str30 datalab = "children_nutri_lca" 
...
use "$path_out/children_nutri_lca_z_rc.dta", clear 
...
sort ind_id
save "$path_out/lca12_CH.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_lca_z_rc.xls"
erase "$path_out/children_nutri_lca_prev_rc.xls"
erase "$path_out/children_nutri_lca_z_rc.dta"
...
********************************************************************************
	...
	/* Note: There is no birth history data file for St Lucia MICS 2012. Hence 
...
codebook mstatus ma6, tab (10)
	/*Note that in Saint Lucia MICS 2012 there is an extra label for "no longer 
	in a visiting relationship" which is classified as separated or not living 
	together in the global MPI work */
...
replace marital = 5 if mstatus == 2 & ma6==4
...
sort ind_id
save "$path_out/lca12_WM.dta", replace
...
********************************************************************************
	...
	//Note: St Lucia MICS 2012 has no male recode file.
...
save "$path_out/lca12_HH.dta", replace
...
*****************************************
	...
	//There is no birth history file for St Lucia MICS 2012


*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/lca12_WM.dta"
tab hl7, miss 
gen temp = (hl7>0) 
...
drop temp
drop _merge
erase "$path_out/lca12_WM.dta"


*** Merging HH Recode 
*****************************************

merge m:1 hh_id using "$path_out/lca12_HH.dta"
tab hh9 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed
drop _merge
erase "$path_out/lca12_HH.dta"


*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/lca12_CH.dta"
drop _merge
erase "$path_out/lca12_CH.dta"
...
//Subnational region
	/*Note: The sample for the Saint Lucia Multiple Indicator Cluster Survey 
	(MICS) was designed to provide estimates for a large number of indicators on 
	the situation of children and women at the national level and for urban and 
	rural areas (p.20). Due to the small size of so many districts it is not 
	realistic to provide estimates at the district level. There is no obvious 
	grouping of districts into a smaller sub-set of three or four regions, which 
	would have made sampling more manageable. Thus urban and rural population 
	were selected as the sampling strata for the purpose of the MICS (p122). 
	As such fr the purpose of the global MPI, we will not be able to decompose 
	the results at the subnational level */
gen region=.
...
/* The admission age to compulsory education in Saint Lucia is 5 years (p.82). 
Primary education takes place from age 5-11 and it lasts 7 years. 
Secondary education takes place from age 12-16 and it lasts 5 years. 
Compulsory education lasts 13 years.

Entrance age of primary: 5 years
Duration of primary: 7 years
Entrance age of lower secondary: 12 years
Duration lower secondary: 3 years 
Entrance age upper secondary: 15 years
...
http://www.ibe.unesco.org/fileadmin/user_upload/Publications/WDE/2010/pdf-versions/Saint_Lucia.pdf
*/
...
replace edulevel = . if ed4a==. | ed4a==8 |ed4a==98 | ed4a==9 
...
replace eduyears = eduhighyear + 7 if edulevel == 2 
	...
	//Level 3: secondary	
replace eduyears = eduhighyear + 7 if edulevel == 3 & age < 20
	/*Note: For people aged 19 or younger, we add 7 years to level 3, as they 
	are under the current system dated from 1997/1998 */
replace eduyears = eduhighyear + 10 if edulevel == 3 & age >= 20
	/*Note: For people aged 20 or older, we add 7+3 years to level 3, as they 
	are under the old system that lasted until 1996/1997 */

	//Level 4: Post Sec/Non-Tertiary	
replace eduyears = eduhighyear + 7 + 5 if edulevel == 4 & age < 20
	/*Note: For people aged 19 or younger, we add 7+5 years to level 4, as 
	they are under the current system dated from 1997/1998 */
replace eduyears = eduhighyear + 10 + 5 if edulevel == 4 & age >= 20
	/*Note: For people aged 20 or older, we add 7+3+5 years to level 4, as they 
	are under the old system that lasted until 1996/1997 */
	
	//Level 5: Tertiary/University
replace eduyears = eduhighyear + 7 + 5 if edulevel == 5 & age < 20
	/*Note: For people aged 19 or younger, we add 7+5 years to level 5, as they 
	are under the current system dated from 1997/1998 */
replace eduyears = eduhighyear + 10 + 5 if edulevel == 5 & age >= 20
	/*Note: For people aged 20 or older, we add 7+3+5 years to level 5, as they 
	are under the old system that lasted until 1996/1997 */
...
gen	child_schoolage = (age>=5 & age<=13)
	/*
	Note: In Saint Lucia, the official school entrance age is 5 years  
...
********************************************************************************
	/*NOTE: St Lucia MICS 2012 has no data on child mortality. As such the 
...
********************************************************************
	/*Note: According to Saint Lucia MICS 2012 country report (p.53), the 
	category 'flush to unknown place/not sure/do not know where' is an improved 
	sanitation facility.*/
...
replace water_mdg = 0 if water==32 | water==42 | water==61 | water==96 
...
codebook hc8c hc8b hc8d hc9b hc8e hc9f hc11
...
save "$path_out/lca_mics12.dta", replace 
