...
2019 Global Multidimensional Poverty Index - Mozambique DHS 2011 
...
********************************************************************************
	...
	/* In Mozambique anthropometric measures were collected for all children 
	and women. "[...] demographic and health data was collected for all the 
	women of 15-49 years and children of less than 5 years old found in the 
	selected family agregates (pp. 11 and Appendix A pp. 279)" No information 
	of height and weight for men was found. */
...
use "$path_in/MZKR62FL.DTA", clear 
...
drop if b5==0 
	...
	//Mozambique DHS 2011: 811 observations deleted
...
gen str30 datalab = "children_nutri_moz" 
...
use "$path_out/children_nutri_moz_z_rc.dta", clear 
...
erase "$path_out/children_nutri_moz_z_rc.xls"
erase "$path_out/children_nutri_moz_prev_rc.xls"
erase "$path_out/children_nutri_moz_z_rc.dta"


	//Save a temp file for merging with PR:
save "$path_out/MOZ11_KR.dta", replace
...
use "$path_in/MZBR62FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Mozambique DHS 2011: these figures are identical
...
sort ind_id
save "$path_out/MOZ11_BR.dta", replace	
...
use "$path_in/MZIR62FL.DTA", clear
...
sort ind_id
save "$path_out/MOZ11_IR.dta", replace
...
gen str30 datalab = "girl_nutri_moz" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_moz_z.dta", clear 
...
sort ind_id
save "$path_out/MOZ11_IR_girls.dta", replace

	//Erase files from folder:
erase "$path_out/girl_nutri_moz_z.xls"
erase "$path_out/girl_nutri_moz_prev.xls"
erase "$path_out/girl_nutri_moz_z.dta"
...
use "$path_in/MZMR62FL.DTA", clear 
...
sort ind_id
save "$path_out/MOZ11_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Mozambique DHS 2011, no anthropometric data was collected 
for men. Thus, this step is irrelevant. */
...
use "$path_in/MZPR62FL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MOZ11_BR.dta"

drop _merge

erase "$path_out/MOZ11_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MOZ11_IR.dta"
...
erase "$path_out/MOZ11_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/MOZ11_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss 
	/*Note: In Mozambique DHS 2011, 66 girls 15-19 were identified as measured 
...
erase "$path_out/MOZ11_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MOZ11_MR.dta"
tab men_MR hv118, miss col
drop _merge 
erase "$path_out/MOZ11_MR.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MOZ11_KR.dta"
...
tab resident, miss
	...
	In the context of Mozambique DHS 2011, 940 (1.49%) individuals who were 
...
/*In the context of Mozambique DHS 2011, height and weight measurements were
collected from children (0-5) and women (15-49) living in all of the sampled 
households. Hence there is no presence of subsample selection for the nutrition
indicators. */
gen subsample=.
...
*****************************************
	...
	data. However, there is no presence of subsample for Mozambique. */ 
gen	no_hem_eligible = .
...
//Subnational region
	/*In the case of Mozambique, the sample provides accurate estimates 
	at national, urban and rural levels, regional and provincial levels 
	(Appendix A, pp. 279). The regional variable at 11 provincial levels is 
	hv024, so this variable was used to sub-national estimates. */
...
replace eduyears = . if eduyears>20
...
gen	child_schoolage = (age>=6 & age<=14)
	/* 
	Note: In Mozambique, the official school entrance age is 6 years.  
...
********************************************************************************
	//Note: Mozambique DHS 2011 has no anthropometric data for men, 15-64 years.
...
foreach var in ha40 hb40 {
			 ...
			 //Check if it is restricted to only an age group or full sample. This is restricted only to females aged 15-49
			 drop inf_`var'
			 }
***
...
lab var m_bmi "Male's BMI"
	//Mozambique DHS 2011 has no anthropometric data for men, 15-64 years.
...
********************************************************************
	/*Note: In Mozambique,the report considers all toilets with flush do not 
	know where and toilets without flush do not know where as improved 
	facilities. For the purpose of the global MPI, we follow the 
	categorisation stated in the country report. */
...
*** Standard MPI ***
****************************************
gen	water_mdg = 1 if water==11 | /// Piped into dwelling 
					 water==12 | /// Piped to yard/plot
					 water==13 | /// Public tap/standpipe
					 water==14 | /// Neighbor's house
					 water==31 | /// Protected well
					 water==33 | /// Borehole with pump
					 water==51 | /// Rainwater
					 water==71   /* 71 Bottled water */
		
replace water_mdg = 0 if water==32 | water==43 | ///
						 water==61 | water==96 
	/*Deprived if it is 32"unprotected well", 61"tanker truck" 
	43"surface water (river/lake, etc)",96"other" */
...
tab cookingfuel cooking_mdg, miss	
	...
	/*Note that in Mozambique DHS 2011, the category 'other' cooking fuel is not 
...
save "$path_out/moz_dhs11.dta", replace 
