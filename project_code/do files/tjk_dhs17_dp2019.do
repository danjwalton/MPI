...
2019 Global Multidimensional Poverty Index - Tajikistan DHS 2017 [STATA do-file]. 
...
/*Tajikistan DHS 2017: In all households, height and weight measurements 
were recorded for children age 0-59 months and women age 15-49 (p.3). */
...
tab hv120, miss
	// 6,243 children under 5 are eligible for anthropometric measurement	
count if hc1!=.
	//All 6,243 children under 5 have information on age in months
tab hv105 if hc1!=.
	/*A cross check with the age in years reveal that all are within the 5 year 
	age group */
tab hc13 if hc1!=., miss
	/*Of the 6,243 children, 6,117 (98%) have been measured while 126 have 
	missing information */
tab hc13 if hc1!=. & hc3<=9990, miss
tab hc13 if hc1!=. & hc2<=9990, miss
	/*For example, height and weight data is available for all 6,117 children 
...
gen str30 datalab = "children_nutri_tjk" 
...
standards */
use "$path_out/children_nutri_tjk_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1 
	/*Note: Tajikistan DHS 2017: 38 children were replaced as missing because
...
save "$path_out/TJK17_PR_child.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_tjk_z_rc.xls"
erase "$path_out/children_nutri_tjk_prev_rc.xls"
erase "$path_out/children_nutri_tjk_z_rc.dta"
...
compare tot_child_died tot_child_died_2
	//Tajikistan DHS 2017: these figures are identical
...
sort ind_id
save "$path_out/TJK17_BR.dta", replace	
...
sort ind_id
save "$path_out/TJK17_IR.dta", replace
...
tab ha13 if hv105>=15 & hv105<=19 & hv104==2, miss 
	...
	male survey and have anthropometric data: 1,959 */
tab ha13 hv117 if hv105>=15 & hv105<=19 & hv104==2, miss
tab ha13 hv103 if hv105>=15 & hv105<=19 & hv104==2, miss
	/*41 of the 1,959 women 15-19 years are identified as non-eligible
...
gen str30 datalab = "girl_nutri_tjk" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_tjk_z.dta", clear 

	
gen	z_bmi = _zbfa
replace z_bmi = . if _fbfa==1 
	/*Tajikistan DHS 2017: 1 girl 15-19 years were replaced as missing 
...
sort ind_id
save "$path_out/TJK17_PR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_tjk_z.xls"
erase "$path_out/girl_nutri_tjk_prev.xls"
erase "$path_out/girl_nutri_tjk_z.dta"
...
********************************************************************************
	//Tajikistan DHS 2017: No male recode. 

********************************************************************************
*** Step 1.6  PR - INDIVIDUAL RECODE  
*** (Boys 15-19 years in the household)
********************************************************************************
	//Tajikistan DHS 2017: Anthropometric data was not collected from men. 
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TJK17_BR.dta"
drop _merge
erase "$path_out/TJK17_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TJK17_IR.dta"
tab women_IR hv117, miss col
tab ha65 if hv117==1 & women_IR ==., miss 
	//Total number of eligible women not interviewed
drop _merge
erase "$path_out/TJK17_IR.dta"
...
tab v201 ha65 if hv117==1, miss
	/*Note: Some 0.8% eligible women did not provide information on their birth 
...
*****************************************
merge 1:1 ind_id using "$path_out/TJK17_PR_girls.dta"
tab ha13 girl_PR if hv105>=15 & hv105<=19 & hv104==2, miss col
drop _merge
erase "$path_out/TJK17_PR_girls.dta"		
...
*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TJK17_PR_child.dta"
tab hv120, miss  
tab hc13 if hv120==1, miss
drop _merge
erase "$path_out/TJK17_PR_child.dta"
...
drop if resident!=1 
	/*Tajikistan DHS 2017: 575 (1.28%) individuals who were non-usual residents 
...
/* Tajikistan DHS 2017: height and weight measurements were collected from all 
children under 5 and women 15-49 years. There is no presence of subsample.   */	
...
applicable population, that is, children 0-5 years and adult women 15-49 years. 
...
***********************************************
//Tajikistan DHS 2017: No anthropometric data for adults.
...
//Subnational region
	/* The sample for Tajikistan DHS 2017 was designed to yield representative 
	results for the urban and rural areas separately, and for each of the four 
	administrative regions (Districts of Republican Subordination (DRS), Sughd, 
	Khatlon, and Gorno-Badakhshan Autonomous Oblast (GBAO)) and Dushanbe (p.1). */   
...
gen	child_schoolage = (age>=7 & age<=15)
	/* Note: In Tajikistan, the official school entrance age to primary school 
...
foreach var in ha40 {
			 ...
			 //Tajikistan DHS 2017: women 15-49 years
			 drop inf_`var'
			 }
***
...
******************************************************************* 
	//Note: Tajikistan DHS 2017 has no anthropometric data for men. 
...
gen	floor_imp = 1
replace floor_imp = 0 if floor<=12
...
replace wall_imp = 0 if wall<=26 | wall==96  
	/*Deprived if no wall, cane/palms/trunk, mud/dirt, uncovered adobe, plywood,
	stone with mud, cardboard, reused wood, other */	
...
gen	roof_imp = 1 
replace roof_imp = 0 if roof<=24 
	/*Deprived if no roof, thatch/palm leaf, sod, cardboard, 
	wood planks */	
...
lookfor car voiture truck van
	//Note: car (sh122f) and truck (sh122h) has been aggregated into hv212  
codebook hv212 
clonevar car = hv212  
...
save "$path_out/tjk_dhs17.dta", replace 
