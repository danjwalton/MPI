...
2019 Global Multidimensional Poverty Index - Zimbabwe DHS 2015 [STATA do-file]. 
...
/*Zimbabwe DHS 2015: In all households, height and weight measurements 
were recorded for children age 0-59 months, women age 15-49, and men age 15-54 
(p.3)*/
...
use "$path_in/ZWKR71FL.DTA", clear 
...
gen str30 datalab = "children_nutri_zwe"  
...
standards */
use "$path_out/children_nutri_zwe_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Zimbabwe DHS 2015, 51 children were replaced as
...
sort ind_id
duplicates report ind_id
save "$path_out/ZWE15_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_zwe_z_rc.xls"
erase "$path_out/children_nutri_zwe_prev_rc.xls"
erase "$path_out/children_nutri_zwe_z_rc.dta"
...
use "$path_in/ZWBR71FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Zimbabwe DHS 2015: these figures are identical
...
sort ind_id
save "$path_out/ZWE15_BR.dta", replace	
...
use "$path_in/ZWIR71FL.DTA", clear
...
sort ind_id
save "$path_out/ZWE15_IR.dta", replace
...
count if v012>=15 & v012<=19
	//Total number of girls in the IR recode: 2,156	
...
gen str30 datalab = "girl_nutri_zwe" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_zwe_z.dta", clear 
...
sort ind_id
save "$path_out/ZWE15_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_zwe_z.xls"
erase "$path_out/girl_nutri_zwe_prev.xls"
erase "$path_out/girl_nutri_zwe_z.dta"
...
use "$path_in/ZWMR71FL.DTA", clear 
...
sort ind_id
save "$path_out/ZWE15_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Zimbabwe DHS 2015, anthropometric data was collected 
...
use "$path_in/ZWPR71FL.DTA", clear
...
keep if _merge==3
	...
	keep the individuals who matched at _merge==3, that is, the 8,396 
...
*** Variable: BODY WEIGHT (KILOGRAMS) ***
	/*In the case of Zimbabwe DHS 2015, the usual mv437 variable (body weight) 
...
*** Variable: HEIGHT (CENTIMETERS)
	/*In the case of Zimbabwe DHS 2015, the usual mv438 variable (height) 
...
count if mv012>=15 & mv012<=19
	//Total number of boys in the MR recode: 2,065
...
/* We use datalab to specify the name that will prefix the output files that 
will be produced from using this ado file*/
gen str30 datalab = "boy_nutri_zwe" 
lab var datalab "Working file"	
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores to compute BMI-for-age*/
use "$path_out/boy_nutri_zwe_z.dta", clear 
...
sort ind_id
save "$path_out/ZWE15_MR_boys.dta", replace


	//Erase files from folder:
erase "$path_out/boy_nutri_zwe_z.xls"
erase "$path_out/boy_nutri_zwe_prev.xls"
erase "$path_out/boy_nutri_zwe_z.dta"
...
*** Merging BR Recode 
*****************************************
merge m:m ind_id using "$path_out/ZWE15_BR.dta"

drop _merge

erase "$path_out/ZWE15_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ZWE15_IR.dta"
...
erase "$path_out/ZWE15_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/ZWE15_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13==0 & hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Zimbabwe DHS 2015, 33 girls 15-19 were identified as measured 
...
erase "$path_out/ZWE15_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ZWE15_MR.dta"
...
erase "$path_out/ZWE15_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
merge 1:1 ind_id using "$path_out/ZWE15_MR_boys.dta"
...
tab hb40 if hb65==1 & hb13==0 & hv118==1 & teen_MR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Zimbabwe DHS 2015, 48 boys 15-19 were identified as measured 
	and are present in the PR recode, but they were not present in the MR recode. 
	It should be noted that they have BMI information but not BMI-for-age.*/
	
drop _merge

erase "$path_out/ZWE15_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/ZWE15_KR.dta"
...
tab resident, miss
	...
	In the context of Zimbabwe DHS 2015, 1,697 (3.3%) individuals who were 
...
Zimbabwe DHS 2015: In all households, height and weight measurements 
were recorded for children age 0-59 months, women age 15-49, and men age 15-54 
(p.3). This suggest there is no subsample selection for this dataset.
*/
...
*****************************************
	...
	third or two third of the households is assessed for nutrition related data. 
	Variable "hem_eligible" identifies that subsample. However, in Zimbabwe 
...
//Subnational region
	/*NOTE: The sample for the Zimbabwe DHS 2015 was designed to provide 
	estimates of key indicators for the country as a whole, for urban and rural 
	areas, and for each of Zimbabwe's ten provinces.*/  
lookfor region
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Zimbabwe, the official school entrance age is 6 years.  
...
*** Standard MPI: BMI Indicator for Men 15-54 years ***
******************************************************************* 
...
*** Standard MPI ***
****************************************
	/*Note: In Zimbabwe,the report  (p.11) considers the category other (96) as 
	neither improve nor non-improved. But we consider it as non-improved to be 
...
tab cookingfuel cooking_mdg, miss	
	...
	/*Note that in Zimbabwe DHS 2015, the category 'other' cooking fuel is not 
...
save "$path_out/zwe_dhs15.dta", replace 
