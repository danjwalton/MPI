...
2019 Global Multidimensional Poverty Index - Guyana MICS 2014 [STATA do-file]. 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Guyana_MICS_2014_Datasets
...
********************************************************************************
	...
	/*Guyana MICS 2014: Anthropometric data was collected from all children 
	under 5 (p.50)  */
...
gen str30 datalab = "children_nutri_guy" 
...
standards */
use "$path_out/children_nutri_guy_z_rc.dta", clear
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Guyana MICS 2014, 90 children were replaced 
...
sort ind_id
save "$path_out/guy14_CH.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_guy_z_rc.xls"
erase "$path_out/children_nutri_guy_prev_rc.xls"
erase "$path_out/children_nutri_guy_z_rc.dta"
...
sort ind_id
save "$path_out/guy14_BH.dta", replace	
...
tab cm1 cm8, miss  
	...
	child mortality. There are 4 women who never ever gave birth but reported 
	child mortality. This inconsistency will be addressed in the later part of 
	the do-file*/
...
tab mstatus ma6, miss 
	/*Note: For Guyana 2014 there is an extra label for "no longer in a 
	visiting relationship" which is classified as separated or not living 
	together*/
...
sort ind_id
save "$path_out/guy14_WM.dta", replace
...
replace marital = 5 if mmstatus == 2 & mma6==4
	//5: Separated/not living together	
label define lab_mar 1"never married" 2"currently married" 3"widowed" ///
					 4"divorced" 5"not living together"
label values marital lab_mar	
...
sort ind_id
save "$path_out/guy14_MN.dta", replace
...
save "$path_out/guy14_HH.dta", replace
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/guy14_BH.dta"

drop _merge

erase "$path_out/guy14_BH.dta" 
  
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/guy14_WM.dta"
...
erase "$path_out/guy14_WM.dta"

*** Merging HH Recode 
*****************************************
merge m:1 hh_id using "$path_out/guy14_HH.dta"
...
erase "$path_out/guy14_HH.dta"

*** Merging MN Recode 
*****************************************
merge 1:1 ind_id using "$path_out/guy14_MN.dta"

drop _merge

erase "$path_out/guy14_MN.dta

*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/guy14_CH.dta"

drop _merge

erase "$path_out/guy14_CH.dta"
...
//Subnational region
	/*Note: The Guyana MICS 2014 sample was designed to provide estimates for a 
	large number of indicators on the situation of children and women at the 
	national level, for urban and rural areas separately and for the two 
	geographic sub-areas defined as interior and coastal areas. */
codebook hh7a1, tab (10)	
decode hh7a1, gen(temp)
replace temp =  proper(temp)
encode temp, gen(region)
lab var region "Region for subnational decomposition"
tab hh7a1 region, miss 
drop temp
...
/* The admission age to compulsory education in Guyana is 6 years. 
Preschool education takes place from age 4 and lasts 2 years. Primary education 
takes place from age 6-12 (grades 1-6)and it lasts 6 years. Lower secondary 
education takes place from age 12-14 (grades 7-9). Upper secondary education 
takes place from age 15-16(grades 10-11). Compulsory education lasts 6 years.
...
Duration of compulsory education: 6 years (primary) 

References: 
http://data.uis.unesco.org/
http://www.ibe.unesco.org/fileadmin/user_upload/Publications/WDE/2010/pdf-versions/Guyana.pdf
*/

tab ed4b ed4a, miss
tab age ed6a if ed5==1, miss
	/*In the case of Guyana MICS 2014, there is inconsistency such 
...
replace eduyears = 0 if edulevel<=1 & eduhighyear==.   
	/*Assuming 0 year if they only attend preschool (0) or primary (1) but 
	the last year is unknown*/
replace eduyears = eduhighyear + 6 if (edulevel==2) 
	//Secondary (lower and higher) after 6 years of primary
replace eduyears = eduhighyear + 11 if (edulevel==3)  
	/*Higher education and university assumed to start after 11 years of 
	education (6 from primary and 5 from secondary) */
...
gen	child_schoolage = (age>=6 & age<=14)
	/*Note: In Guyana, the official school entrance age is 6 years  
...
*** Standard MPI: Child Under 2 Stunted & 2-5 Underweight Indicator ***
************************************************************************

gen st_uw_byage = 0
replace st_uw_byage = 1 if stunting==1 & age<2
replace st_uw_byage = . if stunting==. & age<2
replace st_uw_byage = 1 if underweight==1 & (age>=2 & age <=5)
replace st_uw_byage = . if underweight==. & (age>=2 & age <=5)
replace st_uw_byage = . if stunting==. & underweight==.

bysort hh_id: egen temp = max(st_uw_byage)
gen	hh_no_uw_st_byage = (temp==0) 
	//Takes value 1 if no child in the hh is underweight or stunted
replace hh_no_uw_st_byage = . if temp==.
replace hh_no_uw_st_byage = 1 if no_child_eligible==1 
	//Households with no eligible children will receive a value of 1 
lab var hh_no_uw_st_byage "Household has no child underweight or stunted by age"
drop temp
...
********************************************************************
	/*Note: The report considers the category open defecation as neither 
	improve nor non-improved. We follow the standard set in the MDGs, where 
	this is clearly a deprived standard (p.157). Furthermore, the report 
	considers missing information on sanitation as non-improved, which 
	we have followed. */
...
replace toilet_mdg = 0 if toilet==14 | toilet==99
	/*Household is assigned a value of '0' if it uses non-improved sanitation 
	that is flush to somewhere else or is mising (following report, p.157) */
...
*** Standard MPI ***
****************************************
	/*Note: Households with missing information on source of drinking water is 
	identified as deprived (Table WS.1, p.148). We have followed the country 
	report.*/
...
replace water_mdg = 0 if (water==91) & ///
						 (ndwater==32 | ndwater==42 | ndwater==71 | ///
						  ndwater==81 | ndwater==96 | ndwater==99) 
...
save "$path_out/guy_mics14.dta", replace 
