...
2019 Global Multidimensional Poverty Index - Kyrgyzstan MICS 2014 [STATA do-file]. 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Kyrgyzstan_2014_MICS_Datasets
...
/*Kyrgyztsan MICS 2014: The country report indicates that 
anthropometric data was collected only for childen under 5 years (p.iv ). */
...
gen str30 datalab = "children_nutri_kgz" 
...
use "$path_out/children_nutri_kgz_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1 
	/*Note: In Kyrgyzstan MICS 2014, 29 children were replaced as missing because
...
erase "$path_out/children_nutri_kgz_z_rc.xls"
erase "$path_out/children_nutri_kgz_prev_rc.xls"
erase "$path_out/children_nutri_kgz_z_rc.dta"
...
/* Note: There is no male data file for Kyrgyzstan MICS 2014. Hence this 
...
//Subnational region 
	/*Note: The sample for the Kyrgyzstan MICS 2014 was designed to provide 
	estimates for a large number of indicators on the situation of children and 
	women at the national level, for urban and rural areas, and for seven 
	oblasts: Batken, Djalal-Abad, Issyk-Kul, Naryn, Osh, Talas and Chui as well 
	as Bishkek and Osh cities (p.5). */
codebook hh7, tab (99)
decode hh7, gen(temp)
...
/*Kyrgyzstan MICS 2014 Survey Report states: "according to the Law on Education, 
the school education consists of three levels. The first level, primary 
education, consists of grades 1-4. The second level, lower secondary education, 
consists of grades 5-9. The first two levels together constitute what is 
referred to as basic secondary education according to the law. The third level, 
upper secondary education, consists of grades 10-11. The two levels, lower and 
upper secondary education, together constitute secondary education according to 
the law... Children enter primary school at age 6 or 7." (p.144-145) */
...
codebook ed4a, tab (100)
replace edulevel = . if ed4a==. | ed4a==8 | ed4a==9 
	//ed4a=8/98/99 are missing values
codebook ed3, tab (100)
	//Missing values coded as 9, recoded into missing first.
replace ed3 =. if ed3==9
...
replace eduyears = 0 if edulevel<=1 & eduhighyear==. | edulevel==0   
	/*Assuming 0 year if they only attend preschool or primary but the last year 
	is unknown*/
replace eduyears = eduhighyear + 9 if (edulevel==4 | edulevel==5)   
	/*Professional education assumed to start after 9 years of general 
	education, based on communication with data issuers 20 May 2016 */
replace eduyears = eduhighyear + 11 if (edulevel==6) 
	/*University after 11 years of education, 4 years of primary + 7 years of 
	secondary */
replace eduyears = 9 if (edulevel == 4 | edulevel==5 ) & (eduhighyear ==.) 
	/* Professional primary education assumed to be higher than secondary 
	education */
replace eduyears = 11 if (edulevel == 6 & eduhighyear ==.) 
	/* University assumed to be higher than secondary 
	education */
...
gen	child_schoolage = (age>=7 & age<=15)
	/* 
	Note: In Kyrgyzstan, the official school entrance age is 7 years.
...
********************************************************************************
	//NOTE: Kyrgyzstan MICS 2014: No information on child mortality from men 
...
drop temp_f
	...
	/* In the case of Kyrgyzstan, this variable takes missing value because the 
...
label define lab_shared_toi 1"Yes" 0"No"
...
********************************************************************
	/*Note: In Kyrgyzstan, the report assumes missing toilet information as 
	non-improved. We follow the report (p.84). */
...
gen	water_mdg = 1 if water==11 | water==12 | water==13 | water==14 | ///
	...
	/*Non deprived if water is "piped into dwelling", "piped to yard/plot", 
	 ...
	 Piped to neighbour, 14, included here (p.78). */
...
save "$path_out/kgz_mics14.dta", replace 
