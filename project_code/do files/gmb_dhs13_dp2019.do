...
2019 Global Multidimensional Poverty Index - Gambia DHS 2013 [STATA do-file]. 
...
/*Gambia DHS 2013: Anthropometric information were recorded for eligible 
children age 0-59 months (p. 6) and eligible women aged 15-49 (p.5) living in 
1/2 of the sampled households. Anthropometric information was not collected 
from men aged 15-54. */
...
use "$path_in/GMKR60FL.DTA", clear 
...
drop if b5==0 
	...
	//Gambia DHS 2013: 300 observations deleted
...
gen str30 datalab = "children_nutri_gmb" 
...
standards */
use "$path_out/children_nutri_gmb_z_rc.dta", clear 
...
sort ind_id
duplicates report ind_id
save "$path_out/GMB13_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_gmb_z_rc.xls"
erase "$path_out/children_nutri_gmb_prev_rc.xls"
erase "$path_out/children_nutri_gmb_z_rc.dta"
...
use "$path_in/GMBR60FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Gambia DHS 2013: these figures are identical
...
sort ind_id
save "$path_out/GMB13_BR.dta", replace	
...
use "$path_in/GMIR60FL.DTA", clear
...
sort ind_id
save "$path_out/GMB13_IR.dta", replace
...
gen str30 datalab = "girl_nutri_gmb" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_gmb_z.dta", clear 
...
sort ind_id
save "$path_out/GMB13_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_gmb_z.xls"
erase "$path_out/girl_nutri_gmb_prev.xls"
erase "$path_out/girl_nutri_gmb_z.dta"
...
use "$path_in/GMMR60FL.DTA", clear 
...
sort ind_id
save "$path_out/GMB13_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Gambia 2013, anthropometric data was NOT collected 
...
use "$path_in/GMPR60FL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GMB13_BR.dta"

drop _merge

erase "$path_out/GMB13_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GMB13_IR.dta"
...
erase "$path_out/GMB13_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/GMB13_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13==0 & hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Gambia DHS 2013, 31 girls 15-19 were identified as measured 
...
erase "$path_out/GMB13_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GMB13_MR.dta"
...
tab hb13 hb65 if hv118==1 & men_MR==., miss 
	//No anthropometric data for men in Gambia DHS 2013

drop _merge

erase "$path_out/GMB13_MR.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GMB13_KR.dta"
...
tab resident, miss
	...
	In the context of Gambia DHS 2013, 627 individuals who were 
...
/*
In the context of Gambia DHS 2013, height and weight measurements were
collected from children (0-5) and women (15-49) living in  the 1/2 of the 
sampled households. 
*/
...
//Subnational region
	/*NOTE: The sample for the Gambia DHS 2013 was designed to provide 
	estimates of key indicators for the country as a whole, for urban and rural 
	areas separately, and for each of the 8 LGS(p.5). */
...
gen	child_schoolage = (age>=7 & age<=15)
	/*Note: In Gambia, the official school entrance age is 6 years.  
...
********************************************************************************
/*Note: Gambia DHS 2013 has no anthropometric data for men 15-59 years */
...
lab var cooking_mdg "Household has cooking fuel by MDG standards"
	/* Non deprived if: "electricity", "lpg", "natural gas", "biogas", 
	   ...
	   Deprived if: "coal/lignite", "charcoal", "wood", "straw/shrubs/grass" 
					...
					//What about other coded as 96. Rizwan
				replace cooking_mdg = 0 if cookingfuel==96 // Rizwan
...
save "$path_out/gmb_dhs13.dta", replace 
