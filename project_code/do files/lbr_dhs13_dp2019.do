...
2019 Global Multidimensional Poverty Index - Liberia DHS 2013 
...
/*Liberia DHS 2013: Anthropometric information was recorded for all eligible
children age 0-59 months and all eligible men and women age 15-49 in a subsample 
of 1/2 of sampled households. (p.3) */


********************************************************************************
*** Step 1.1 KR - CHILDREN's RECODE (under 5)
********************************************************************************
use "$path_in/LBKR6AFL.DTA", clear 
...
drop if b5==0 
	//Exclude children who are not alive. 548 observations deleted.
...
gen str30 datalab = "children_nutri_lbr" 
...
use "$path_out/children_nutri_lbr_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1 
	/* Note: In the context of Liberia DHS 2013, 57 children are replaced as '.'
...
sort ind_id
duplicates report ind_id
save "$path_out/LBR13_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_lbr_z_rc.xls"
erase "$path_out/children_nutri_lbr_prev_rc.xls"
erase "$path_out/children_nutri_lbr_z_rc.dta"
...
use "$path_in/LBBR6AFL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Liberia DHS 2013: these figures are identical
...
sort ind_id
save "$path_out/LBR13_BR.dta", replace	
...
use "$path_in/LBIR6AFL.DTA", clear
...
sort  ind_id
save "$path_out/LBR13_IR.dta", replace
...
gen str30 datalab = "girl_nutri_lbr" 
lab var datalab "Working file"
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores */
use "$path_out/girl_nutri_lbr_z.dta", clear 
...
sort ind_id
save "$path_out/LBR13_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_lbr_z.xls"
erase "$path_out/girl_nutri_lbr_prev.xls"
erase "$path_out/girl_nutri_lbr_z.dta"
...
use "$path_in/LBMR6AFL.DTA", clear 
...
sort ind_id
save "$path_out/LBR13_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Liberia DHS 2013, anthropometric information for men was 
collected from all eligible men in a subsample of 1/2 of sampled households. 
This data is recorded in the household (PR) file, rather than the men's (MR) 
file.*/

use "$path_in/LBPR6AFL.DTA", clear 	
...
count if hv104==1 & hv105>=15 & hv105<=59
	//Total number of boys 15-19 years in the PR recode	
...
keep if _merge==3
	...
	keep the individuals who matched at _merge==3. */  
drop _merge
...
*** Variable: BODY WEIGHT (KILOGRAMS) ***
	/*In the case of Liberia DHS 2013, the usual mv437 variable (body weight) 
...
*** Variable: HEIGHT (CENTIMETERS)
	/*In the case of Liberia DHS 2013, the usual mv438 variable (height) 
...
/* We use datalab to specify the name that will prefix the output files that 
will be produced from using this ado file*/
gen str30 datalab = "boy_nutri_lbr" 
lab var datalab "Working file"	
...
use "$path_out/boy_nutri_lbr_z.dta", clear 
...
sort ind_id
	...
	//Erase files from folder:
erase "$path_out/boy_nutri_lbr_z.xls"
erase "$path_out/boy_nutri_lbr_prev.xls"
erase "$path_out/boy_nutri_lbr_z.dta"
erase "$path_out/temp.dta"

	
	//Save a temp file for merging with PR:
save "$path_out/LBR13_MR_boys.dta", replace
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/LBR13_BR.dta"
drop _merge
erase "$path_out/LBR13_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/LBR13_IR.dta"
...
drop _merge
erase "$path_out/LBR13_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/LBR13_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss 
	/*Note: In Liberia DHS 2013, 39 girls 15-19 were identified as measured 
...
drop _merge
erase "$path_out/LBR13_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/LBR13_MR.dta"
...
drop _merge
erase "$path_out/LBR13_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
merge 1:1 ind_id using "$path_out/LBR13_MR_boys.dta"
...
tab hb13 if teen_MR == . & hv118==1 & (hv105>=15 & hv105<=19), miss   
	/*Note: In Liberia DHS 2013, 48 boys 15-19 were identified as measured 
	and are present in the PR recode, but they were not present in the IR recode. 
	It should be noted that they have BMI information but not BMI-for-age.*/	
drop _merge
erase "$path_out/LBR13_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/LBR13_KR.dta"
...
tab resident, miss
	...
	In the context of Liberia DHS 2013, 769 (1.58%) individuals who were 
...
/*
The hv042 (household selected for hemoglobin) is essentially a variable that 
indicates whether there is selection of subsample for anthropometric measures. 
Liberia DHS 2013 survey report indicates no measurement taken for hemoglobin. 
In this case, the variable for subsample was hv027. */
...
*****************************************
	/* Because the hv042 (household selected for hemoglobin) variable does not 
	exist in the Liberia DHS 2013 data, we use hv027 (household selected for 
	male interview) instead, since the same subsample is used for male interview 
	and anthropometric data */
codebook hv042, tab (999)
	//Variable has all (48,806) missing values.
codebook hv027, tab(999)
gen hem_eligible =(hv027==1)
...
drop member
recode hhsize (53=.)
/* One household listed as having 53 members. However, it is irrelevant as it is 
not part of the subsample that we will retain for estimation purpose.*/


//Subnational region
	/*NOTE: The sample for the Liberia DHS 2013 was designed to provide 
	estimates of key indicators for the country as a whole, for urban and rural 
	areas separately, and for each of the 15 counties (p.291). Indicators will 
	also be shown for the Northern, Central, and Southern regions of the country. 
	So instead of "hv024" (that contains 5 regions), we use the "shcounty" 
	variable that contains 15 counties.*/  
lookfor region
codebook hv024, tab (100)
lookfor county counties
codebook shcounty, tab (100)
clonevar region = shcounty
lab var region "Region for subnational decomposition"
tab shcounty region, miss 
...
replace attendance = 0 if (attendance==9 | attendance==.) & hv109==0 
	/*In some countries, they don't assess attendance for those with no 
	 educational attainment. Please check and ensure they are replaced 
	 with a '0' */
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Liberia, the official school entrance age is 6 years.  
...
********************************************************************************
/*NOTE: In the subsample of households selected for the male survey, height and 
weight information was collected from eligible women and men aged 15-49 years, 
as well as children 0-59 months (p.3). */
...
*** Standard MPI: BMI Indicator for Men 15-49 years ***
******************************************************************* 
...
clonevar shared_toilet = hv225 
	...
	/*Note: In Liberia,the report considers the category other (96) as neither 
	improve nor non-improved. But we consider it as non-improved to be 
	consistent with the indicator for destitution below */
...
gen	wall_imp = 1 
replace wall_imp = 0 if wall<=24 | wall==96  
...
clonevar computer = sh110m
clonevar animal_cart = hv243c
	//All missing values for animal cart
...
replace telephone=0 if mobiletelephone==0
replace telephone=. if mobiletelephone==.
...
save "$path_out/lbr_dhs13.dta", replace 
