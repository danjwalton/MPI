...
2019 Global Multidimensional Poverty Index - Belize MICS 2015-16 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Belize_MICS5_Datasets
...
/*Belize MICS 2015-16: Anthropometric data was collected only for childen 
under 5 years as indicated by the questionnaire for children under-5 that was 
administered to mothers (or caretakers) (p.4) */
...
gen str30 datalab = "children_nutri_blz" 
...
use "$path_out/children_nutri_blz_z_rc.dta", clear 
...
sort ind_id
save "$path_out/blz15-16_CH.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_blz_z_rc.xls"
erase "$path_out/children_nutri_blz_prev_rc.xls"
erase "$path_out/children_nutri_blz_z_rc.dta"
...
sort ind_id
save "$path_out/blz15-16_BH.dta", replace
...
duplicates report ind_id 
	//5095 obs, no duplicates

gen women_WM =1 
	//dentification variable for observations in WM recode
...
tab mstatus ma6, miss 
	/*Please ensure recoding follows these 5 broad categories: 1"never married" 
	2"currently married" 3"widowed" 4"divorced" 5"not living together" */
...
label define lab_mar 1"never married" 2"currently married" 3"widowed" 4"divorced" 5"not living together"
...
sort ind_id
save "$path_out/blz15-16_WM.dta", replace
...
sort ind_id
save "$path_out/blz15-16_MN.dta", replace
...
save "$path_out/blz15-16_HH.dta", replace
...
merge 1:1 ind_id using "$path_out/blz15-16_BH.dta"

drop _merge

erase "$path_out/blz15-16_BH.dta" 
 
 
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/blz15-16_WM.dta"
...
erase "$path_out/blz15-16_WM.dta"


*** Merging HH Recode 
*****************************************

merge m:1 hh_id using "$path_out/blz15-16_HH.dta"
tab hh9 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed
drop _merge

erase "$path_out/blz15-16_HH.dta"


*** Merging MN Recode 
*****************************************

merge 1:1 ind_id using "$path_out/blz15-16_MN.dta"
drop _merge

erase "$path_out/blz15-16_MN.dta"


*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/blz15-16_CH.dta"

drop _merge

erase "$path_out/blz15-16_CH.dta"
...
//Subnational region
	/*According to MICS report (page 3, sample design) the survey is 
	representative for the regions Corozal, Orange Walk, Belize City South Side, 
	Belize (excluding Belize South Side), Cayo, Stann Creek and Toledo. */
lookfor region
codebook hh7, tab (99)
decode hh7, gen(temp)
...
tab hh7 region, miss 
drop temp
label define lab_reg 1"Belize (ex. Belize City South Side)" ///
2"Belize City South Side" 3"Cayo" 4"Corozal" 5"Orange Walk" ///
6"Stann Creek" 7"Toledo"
...
/* According to the survey report (page 134): In Belize, children enter primary 
school at age 6 and enter secondary school at age 12. There are 8 grades in 
primary school and 6 grades in secondary school. In primary school, grades are 
referred to as year 1 to year 6 (Standard 1 to Standard 6). In secondary school, 
grades are referred to as Form 1 to Form 6. The school year typically runs from 
September to June the following year.

Note: infant refres to the first two years of primary school. */

tab ed4b ed4a, miss
tab age ed6a if ed5==1, miss
clonevar edulevel = ed4a 
	//Highest educational level attended
replace edulevel = . if ed4a==. | ed4a==96 | ed4a==98 | ed4a==99 
...
replace eduyears = 0 if edulevel<=2 & eduhighyear==.   
	...
	is unknown. Since 1=infant, 2=primary, the command line should be 
	edulevel<=2 */

replace eduyears = eduhighyear + 2 if edulevel==2  
	/*After 2 years of compulsory infant, they start primary. Infant first for 
	two years (with infant, edulevel = 1, and then 6 years of the rest of 
	primary, with edulevel = 2. So, the command line should be edulevel=2 */

replace eduyears = eduhighyear + 8 if edulevel==3
	/*Secondary starts after 8 years of compulsory primary. After 8 years of 
	primary (infant plus primary), they go to secondary, which is edulevel==3. */
	
replace eduyears = eduhighyear + 12 if edulevel==4 
	//Associate is a tertiary degree that requires completion of secondary 
	
replace eduyears = eduhighyear + 12 if edulevel==5 
	//Bachelors is a tertiary degree that requires completion of secondary 
...
lookfor school
	//ed5: Attended school during current school year (2014-2015)
codebook ed5, tab (10)
gen	attendance = .
...
gen	child_schoolage = (age>=6 & age<=14)
	/*Note: In Belize, the official school entrance age is 6 years (page 134)
...
********************************************************************
gen	toilet_mdg = 1 if ((toilet<23 | toilet==31) & shared_toilet!=1) 
replace toilet_mdg = 0 if toilet>=23 & toilet!=31 & toilet!=.
...
tab toilet toilet_mdg, miss row
...
replace water_mdg = 0 if water_mdg==1 & timetowater>=30 & ///
						 timetowater!=. & timetowater!=998 & timetowater!=999
...
replace water_mdg = 0 if water==91 & ///
					     (ndwater==32 | ndwater==42 | water==61 | ///
...
*/			 
tab cookingfuel cooking_mdg, miss row
...
save "$path_out/blz_mics15-16.dta", replace 
