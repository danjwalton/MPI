...
2019 Global Multidimensional Poverty Index - Lesotho DHS 2014 [STATA do-file]. 
...
/*Lesotho DHS 2014: Anthropometric information was recorded for a subsample of 
1/2 of all eligible children age 0-59 months, eligible women aged 15-49, and 
eligible men 15-59 (p.2) */
...
use "$path_in/LSKR71FL.DTA", clear 
...
gen str30 datalab = "children_nutri_lso" 
...
use "$path_out/children_nutri_lso_z_rc.dta", clear 
...
sort ind_id
duplicates report ind_id
save "$path_out/LSO14_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_lso_z_rc.xls"
erase "$path_out/children_nutri_lso_prev_rc.xls"
erase "$path_out/children_nutri_lso_z_rc.dta"
...
use "$path_in/LSBR71FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Lesotho DHS 2014: these figures are identical
...
sort ind_id
save "$path_out/LSO14_BR.dta", replace	
...
use "$path_in/LSIR71FL.DTA", clear
...
sort ind_id
save "$path_out/LSO14_IR.dta", replace
...
gen str30 datalab = "girl_nutri_lso" 
lab var datalab "Working file"
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores */
use "$path_out/girl_nutri_lso_z.dta", clear 
...
sort ind_id
save "$path_out/LSO14_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_lso_z.xls"
erase "$path_out/girl_nutri_lso_prev.xls"
erase "$path_out/girl_nutri_lso_z.dta"
...
use "$path_in/LSMR71FL.DTA", clear 
...
sort ind_id
save "$path_out/LSO14_MR.dta", replace
...
********************************************************************************
	/*Note: In the case of Lesotho DHS 2014, anthropometric data for men is in 
	the PR file rather than the MR file, so this will be extracted to compute 
	BMI-for-age*/

use "$path_in/LSPR71FL.DTA", clear 
...
keep if _merge==3
	...
	keep the individuals who matched at _merge==3, that is, the 112,122 
...
*** Variable: BODY WEIGHT (KILOGRAMS) ***
	/*In the case of Lesotho DHS 2014, the usual mv437 variable (body weight) 
...
*** Variable: HEIGHT (CENTIMETERS)
	/*In the case of Lesotho DHS 2014, the usual mv438 variable (height) 
...
/* We use datalab to specify the name that will prefix the output files that 
will be produced from using this ado file*/
gen str30 datalab = "boy_nutri_lso" 
lab var datalab "Working file"	
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores */
use "$path_out/boy_nutri_lso_z.dta", clear 
...
sort ind_id
save "$path_out/LSO14_MR_boys.dta", replace


	//Erase files from folder:
erase "$path_out/boy_nutri_lso_z.xls"
erase "$path_out/boy_nutri_lso_prev.xls"
erase "$path_out/temp.dta"
erase "$path_out/boy_nutri_lso_z.dta"
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/LSO14_BR.dta"

drop _merge

erase "$path_out/LSO14_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/LSO14_IR.dta"
...
erase "$path_out/LSO14_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/LSO14_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13==0 & hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Lesotho, 10 girls 15-19 were identified as measured 
	and are present in the PR recode, but they were not present in the IR recode.  
	They have BMI information, but I cannot tell if they have BMI-for-age*/ 
	
drop _merge

erase "$path_out/LSO14_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/LSO14_MR.dta"
...
tab hb13 hb65 if hv118==1 & men_MR==., miss 
	/*Note: In Lesotho, 7 men were identified as measured 
	and are present in the PR recode, but they were not present in the IR recode.*/ 
drop _merge

erase "$path_out/LSO14_MR.dta"


*** Merging MR Recode: 15-19 years boys 
*****************************************
merge 1:1 ind_id using "$path_out/LSO14_MR_boys.dta"
...
tab hb40 if hb65==1 & hb13==0 & hv118==1 & teen_MR==. & (hv105>=15 & hv105<=19), miss  
	/*Note: In Lesotho, 23 boys 15-19 were identified as measured 
	and are present in the PR recode, but they were not present in the IR recode.*/ 	
	
drop _merge

erase "$path_out/LSO14_MR_boys.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/LSO14_KR.dta"
...
drop if resident!=1 
	...
	In the context of Lesotho DHS 2014, 8,544 (21.13%) individuals who were 
...
/*
In the context of Lesotho DHS 14, in 1/2 the households, blood pressure 
measurements and anaemia testing were performed among eligible women and men who 
consented to being tested. In the same subsample of households, height and weight 
were measured for eligible women, men, and children age 0-59 months.
As such we are able to identify households selected for nutrition subsample 
using the variable that identifies those who consented for hemoglobin 
measurement.
*/
...
/* Note for Lesotho DHS 2014: We find high numbers of households with no 
eligible members, which is consistent with the findings of the survey report. 
The survey report says the reason for these numbers is not known. (p.6) */
...
//Subnational region
	/*NOTE: The sample for Lesotho DHS 2014  was intended to allow estimates of 
	key indicators at the national level as well as in urban and rural areas, 
	four ecological zones, and each of Lesotho’s 10 districts (p.1). "hv024" 
...
gen	child_schoolage = (age>=6 & age<=14)
	/*Note: In Lesotho, the official school entrance age is 6 years, as 
	explained in survey report (p.13). So, age range is 6-14 (=6+8)*/
...
********************************************************************************
	/*Note: In the context of Lesotho DHS 2014, in 1/2 of the sampled 
	households, height and weight were measured for eligible women, men, and 
...
gen	water_mdg = 1 if water==11 | water==12 | water==13 | water==14 | ///
	...
	/*Non deprived if water is "piped into dwelling", "piped to yard/plot", 
	  ...
	  "protected spring", "rainwater", "bottled water" "neighbour's tap" */
...
save "$path_out/lso_dhs14.dta", replace 
