...
2019 Global Multidimensional Poverty Index - Mali MICS 2015 [STATA do-file]. 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Mali_MICS5_Datasets
...
/*Mali MICS 2015: Anthropometric data was collected only for children under 
5 years (p. 79-80). */ 
...
gen str30 datalab = "children_nutri_mli" 
...
use "$path_out/children_nutri_mli_z_rc.dta", clear 
...
count if _zlen==1 | _zwfl==1
	/*Note: In Mali MICS 2015, 17 children were replaced as missing because
...
sort ind_id
save "$path_out/MLI15_CH.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_mli_z_rc.xls"
erase "$path_out/children_nutri_mli_prev_rc.xls"
erase "$path_out/children_nutri_mli_z_rc.dta"
...
sort ind_id
save "$path_out/MLI15_BH.dta", replace	
...
tab wb2, miss
	//1,029 women 15-49 years have missing age
...
sort ind_id
save "$path_out/MLI15_WM.dta", replace
...
sort ind_id
save "$path_out/MLI15_MN.dta", replace
...
save "$path_out/MLI15_HH.dta", replace
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MLI15_BH.dta"
drop _merge
erase "$path_out/MLI15_BH.dta" 
 
 
*** Merging WM Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MLI15_WM.dta"
tab hl7, miss 
gen temp = (hl7>0) 
...
drop temp
drop _merge
erase "$path_out/MLI15_WM.dta"


*** Merging HH Recode 
*****************************************
merge m:1 hh_id using "$path_out/MLI15_HH.dta"
tab hh9 if _m==2
drop  if _merge==2
	//Drop households that were not interviewed 
drop _merge
erase "$path_out/MLI15_HH.dta"


*** Merging MN Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MLI15_MN.dta"
drop _merge
erase "$path_out/MLI15_MN.dta"


*** Merging CH Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MLI15_CH.dta"
drop _merge
erase "$path_out/MLI15_CH.dta"
sort ind_id
...
//Subnational region
	/*Mali MICS 2015: Data is representative at the national level, urban and 
	rural and 8 regions (including the capital Bamako) (p. 5) 
	
	Kidal was not enumerated because of security reasons (p.336)*/
codebook hh7, tab (99)
decode hh7, gen(temp)
...
/* MALI: the structure of the education system in Mali is the following: 
preschool is addressed to children between 0 and 6 years of age; 
primary education since 2010 has a unique cycle of 9 years but before 2010 is 
was composed of 2 cycles (the first lasting 6 years and the second lasting 
3 years); general secondary education lasts 3 years (grades 10-12). 
Alternatively, students can enter in technical secondary education that has one 
common year with the general secondary (the 10th) and 2 years of technical 
education (11th and 12) (p. 8-9 of the report). The number of years of education 
is therefore 12. Source: 
http://www.ibe.unesco.org/fileadmin/user_upload/Publications/WDE/2010/pdf-versions/Mali.pdf */

tab ed4b ed4a, miss
tab age ed6a if ed5==1, miss
tab age ed4a if ed5==1, miss 
	/*In the case of Mali MICS 2015 there is also inconsistencies such as 
...
replace eduyears = eduhighyear + 12 if (edulevel==4)  
    /*In Mali, the technical/professional education is coded as "4" because 
	previously to 2010 primary or fundamental education was divided in 2 cycles: 
	the first one lasting 6 years and the second one lasting 3 years. Hence, we 
	need to adapt the codes. Professional education assumed to start after 11 
	years of general education. In Mali, the number of years of education is 
	12 = 9 years of primary + 3 years of secondary*/
replace eduyears = eduhighyear + 12 if (edulevel==5) 
	//University starts after 12 years of education 	
replace eduyears = . if (edulevel==3 & eduhighyear > 13) 	
...
gen	child_schoolage = (age>=7 & age<=15)
	/*
	Note: In Mali, the official school entrance age is 7 years. 
...
/*Mali MICS 2015: Improved sanitary facilities for excreta disposal are 
'flush toilets with or without a sewer system', 'septic tank' or 'ventilated 
improved latrines', 'pit latrines with slabs' and 'composting toilets' (p. 160 of
the report). Table WS.5 of this report (p. 161) says that 'flush somewhere else' 
is improved toilet facility. 'Autre' is considered non-improved. 'Open defecation' 
appears separately but we coded as non-improved for consistency. JM 15/06/18*/
...
tab toilet toilet_mdg, miss	
	/*Note: in Mali, the category open defecation (95) 
	as neither improve nor non-improved. But we consider it as non-improved to 
	be consistent with the SDG definition */	
...
save "$path_out/mli_mics15.dta", replace 
