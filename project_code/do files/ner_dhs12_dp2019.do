...
2019 Global Multidimensional Poverty Index - Niger DHS 2012 [STATA do-file]. 
...
/*Niger DHS 2012: Anthropometric information were recorded for women 15-49 years, 
as well as all children under the age of five, were measured and weighed in one 
out of every two households (p.8). Anthropometric information was not collected 
from men 15-59. */
...
use "$path_in/NIKR61FL.DTA", clear 
...
drop if b5==0 
	...
	//Niger DHS 2012: 956 observations deleted
...
gen str30 datalab = "children_nutri_ner" 
...
standards */
use "$path_out/children_nutri_ner_z_rc.dta", clear 
...
sort ind_id
duplicates report ind_id
save "$path_out/NER12_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_ner_z_rc.xls"
erase "$path_out/children_nutri_ner_prev_rc.xls"
erase "$path_out/children_nutri_ner_z_rc.dta"
...
use "$path_in/NIBR61FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Niger DHS 2012: these figures are identical
...
sort ind_id
save "$path_out/NER12_BR.dta", replace	
...
use "$path_in/NIIR61FL.DTA", clear
...
sort ind_id
save "$path_out/NER12_IR.dta", replace
...
gen str30 datalab = "girl_nutri_ner" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_ner_z.dta", clear 
...
sort ind_id
save "$path_out/NER12_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_ner_z.xls"
erase "$path_out/girl_nutri_ner_prev.xls"
erase "$path_out/girl_nutri_ner_z.dta"
...
use "$path_in/NIMR61FL.DTA", clear 
...
sort ind_id
save "$path_out/NER12_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Niger, anthropometric data was NOT collected 
for men*/
...
use "$path_in/NIPR61FL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/NER12_BR.dta"

drop _merge

erase "$path_out/NER12_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/NER12_IR.dta"
...
erase "$path_out/NER12_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/NER12_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Niger DHS 2012, 5 girls 15-19 were identified as measured 
...
*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/NER12_MR.dta"
...
tab hb13 hb65 if hv118==1 & men_MR==., miss 
	//No anthropometric data for men in Niger DHS 2012
...
*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/NER12_KR.dta"
...
tab resident, miss
	...
	In the context of Niger DHS 2012, 241 (0.37%) individuals who were 
...
/*
In the context of Niger DHS 2012, height and weight measurements were
...
*** No Eligible subsample for hemoglobin 
*****************************************
...
/*Note in the case of Niger DHS 2012, we recoded the 9 individuals who responded 
as living together as married */
...
//Subnational region
	/*NOTE: The sample for the Niger DHS 2012 was designed to provide 
	estimates of key indicators for the country as a whole, for urban and rural 
	areas separately, and for each of the each of the eight regions of the 
	country (Agadez, Diffa, Dosso, Maradi, Tahoua, Tillaberi, Zinder and Niamey)
	(p.8). */ 
lookfor region
...
gen	child_schoolage = (age>=7 & age<=15)
	/* 
	In Niger, the official school entrance age is 7 years. 
...
********************************************************************************
	//Note: Niger DHS 2012 has no anthropometric data for men, 15-54 years. 
...
*** Standard MPI ***
****************************************
/*In Niger,the report considers the category other (96) as neither improve 
...
clonevar ndwater = hv202  
	//No observation for non-drinking water for Niger DHS 2012


*** Standard MPI ***
****************************************
/*In Niger,the report does not list the category water vendor (63) as improve 
or non-improved. But we consider it as non-improved given that there is no 
clarification on whether the vendors supply clean drinking water  */
...
replace water_mdg = 0 if water==32 | water==42 | water==43 | ///
						 water==61 | water==62 | water==96 | water==63
...
clonevar car = hv212  
	//In the absence of 'car' variable, look for truck, van or lorry
...
save "$path_out/ner_dhs12.dta", replace 
