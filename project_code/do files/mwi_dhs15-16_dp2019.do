...
2019 Global Multidimensional Poverty Index - Malawi DHS 2015-16 [STATA do-file]. 
...
/*Malawi DHS 2015-16: Anthropometric information were recorded for a 
...
use "$path_in/MWKR7HFL.DTA", clear 
...
drop if b5==0 
	...
	//Malawi DHS 2015-16: 824 observations deleted
...
gen str30 datalab = "children_nutri_mwi" 
...
standards */
use "$path_out/children_nutri_mwi_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1 
	/*Note: In Malawi DHS 2015-16, 89 children were replaced as missing because
...
sort ind_id
save "$path_out/MWI15-16_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_mwi_z_rc.xls"
erase "$path_out/children_nutri_mwi_prev_rc.xls"
erase "$path_out/children_nutri_mwi_z_rc.dta"
...
use "$path_in/MWBR7HFL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Malawi DHS 2015-16: these figures are identical
...
sort ind_id
save "$path_out/MWI15-16_BR.dta", replace	
...
use "$path_in/MWIR7HFL.DTA", clear
...
sort ind_id
save "$path_out/MWI15-16_IR.dta", replace
...
gen str30 datalab = "girl_nutri_mwi" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_mwi_z.dta", clear 
...
sort ind_id
save "$path_out/MWI15-16_IR_girls.dta", replace

	//Erase files from folder:
erase "$path_out/girl_nutri_mwi_z.xls"
erase "$path_out/girl_nutri_mwi_prev.xls"
erase "$path_out/girl_nutri_mwi_z.dta"
...
use "$path_in/MWMR7HFL.DTA", clear 
...
sort ind_id
save "$path_out/MWI15-16_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Malawi 2015-16, anthropometric data was NOT collected 
...
sort ind_id
save "$path_out/MWI15-16_MR_boys.dta", replace
...
use "$path_in/MWPR7HFL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MWI15-16_BR.dta"

drop _merge

erase "$path_out/MWI15-16_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MWI15-16_IR.dta"
...
erase "$path_out/MWI15-16_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/MWI15-16_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13==0 & hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: In Malawi DHS 2015-16, 68 girls 15-19 were identified as measured 
...
erase "$path_out/MWI15-16_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MWI15-16_MR.dta"
...
tab hb13 hb65 if hv118==1 & men_MR==., miss 
	//No anthropometric data for men in Malawi DHS 2015-16

drop _merge

erase "$path_out/MWI15-16_MR.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/MWI15-16_KR.dta"
...
erase "$path_out/MWI15-16_KR.dta"
...
tab resident, miss
	...
	In the context of Malawi DHS 2015-16, 2,223 (1.84%) individuals who were 
...
whether there is selection of subsample for anthropometric measures. 
In the context of Malawi DHS 2015-16, height and weight measurements were
collected from children (0-5) and women (15-49) living in 1/3 of the sampled 
households. 
*/
...
//Subnational region
	/* NOTE: The sample for the Malawi DHS 2015-16 was designed to provide 
	estimates of key indicators for the country as a whole, for urban and rural 
	areas separately, and for each of the 28 districts (p.2). Indicators will 
...
codebook shdist, tab (99)	
clonevar region = shdist
recode region (107=105)(206=210)(303=314)(305=315)
tab shdist region, miss
label define lab_reg 101"Chitipa" 102"Karonga" 103"Nkhata Bay" ///
					 104"Rumphi" 105"Mzimba" 106"Likoma" ///
					 201"Kasungu" 202"Nkhotakota" 203"Ntchisi" ///
					 204"Dowa" 205"Salima" 207"Mchinji" 208"Dedza" ///
					 209"Ntcheu" 210"Lilongwe" 301"Mangochi" ///
					 302"Machinga" 304"Chiradzulu" 306"Mwanza" ///
					 307"Thyolo" 308"Mulange" 309"Phalombe" ///
					 310"Chikwawa" 311"Nsanje" 312"Balaka" ///
					 313"Neno" 314"Zomba" 315"Blantyre"
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Malawi, the official school entrance age is 6 years.  
...
replace hh_child_atten = . if hh_child_atten==0 & no_missing_atten==0 
...
********************************************************************************
	//Note: Malawi DHS 2015-16 has no anthropometric data for men.
...
****************************************
	...
	/*Note: In Malawi,the report considers the category other (96) as neither 
...
tab cookingfuel cooking_mdg, miss	
	...
	/*Note that in Malawi DHS 2015-16, the category 'other' cooking fuel is not 
...
save "$path_out/mwi_dhs15-16.dta", replace 
