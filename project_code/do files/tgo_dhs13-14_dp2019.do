...
2019 Global Multidimensional Poverty Index - Togo DHS 2013-14 [STATA do-file]. 
...
/*Togo DHS 2013-14: In a subsample of 1 in every 2 households, all men aged 
15-59 years were eligible for the survey. In this same subsample, hemoglobin 
test was carried out in order to estimate the prevalence of anemia in all women, 
all men and all children aged 6-59 months identified in the households. 
In this same subsample, all women and all children under 5 years were eligible 
to be measured and weighted in order to assess their nutritional status. (p.7). 
Anthropometric information was not collected from men(p.333)*/
...
use "$path_in/TGKR61FL.DTA", clear 
...
replace ind_id = v001*10000 + v002*100 + (line+90) if duplicate!=0 
...
gen str30 datalab = "children_nutri_tgo" 
...
use "$path_out/children_nutri_tgo_z_rc.dta", clear 
...
count if _fwei==1 | _flen==1
	/* Note: in the context of Togo DHS 2013-14, 21 children were replaced as
...
erase "$path_out/children_nutri_tgo_z_rc.xls"
erase "$path_out/children_nutri_tgo_prev_rc.xls"
erase "$path_out/children_nutri_tgo_z_rc.dta"

	
	//Save a temp file for merging with PR:
save "$path_out/TGO13-14_KR.dta", replace
...
use "$path_in/TGBR61FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Togo DHS 2013-14: these figures are identical 
...
sort ind_id
save "$path_out/TGO13-14_BR.dta", replace		
...
use "$path_in/TGIR61FL.DTA", clear
...
duplicates report ind_id 
	//Togo DHS 2013-14: 9,480 obs, no duplicates 
...
sort  ind_id
save "$path_out/TGO13-14_IR.dta", replace
...
summ weight  
	/*4,832 obs. The other observations are missing because only 1 in every 2 
	women were measured */
...
count if v012>=15 & v012<=19 
	//Total number of girls in the IR recode: 1,733 individuals
...
gen str30 datalab = "girl_nutri_tgo" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_tgo_z.dta", clear 
...
sort ind_id
	...
	//Erase files from folder:
erase "$path_out/girl_nutri_tgo_z.xls"
erase "$path_out/girl_nutri_tgo_prev.xls"
erase "$path_out/girl_nutri_tgo_z.dta"

	
	//Save a temp file for merging with PR:
save "$path_out/TGO13-14_IR_girls.dta", replace
...
use "$path_in/TGMR61FL.DTA", clear 
...
duplicates report ind_id 
	//4,476 individuals, no duplicates 
...
sort ind_id
save "$path_out/TGO13-14_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Togo 2013-14, anthropometric data was NOT collected 
for men */
...
use "$path_in/TGPR61FL.DTA", clear

*** Generate a household unique key variable at the household level using: 
*** hv001=cluster number 
*** hv002=household number
gen double hh_id = hv001*100 + hv002 
format hh_id %20.0g
...
*** Merging BR Recode 
*****************************************
merge m:m ind_id using "$path_out/TGO13-14_BR.dta"

drop _merge

erase "$path_out/TGO13-14_BR.dta"



*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TGO13-14_IR.dta"
...
erase "$path_out/TGO13-14_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/TGO13-14_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13==0 & hv117==1 & teen_IR==. & (hv105>=15 & hv105<=19), miss
	/*Note: in Togo DHS 2013-14, 24 girls 15-19 were identified as measured 
...
erase "$path_out/TGO13-14_IR_girls.dta"


*** Merging MR Recode 
*****************************************
	//No anthropometric data for men in Togo DHS 2013-14

merge 1:1 ind_id using "$path_out/TGO13-14_MR.dta"

tab men_MR hv118, miss col 

drop _merge

erase "$path_out/TGO13-14_MR.dta"
...
*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/TGO13-14_KR.dta"
...
erase "$path_out/TGO13-14_KR.dta"
...
tab resident, miss 
	...
	In the context of Togo DHS 2013-14, 12,090 (20.81%) individuals who were 
	non-usual residents were dropped from the sample. This figure is huge. 
	However, further check reveal that most of these individuals are not 
	part of the anthropometric subsample whom we will retain for the global 
	MPI computation. To be precise, within the subsample, which is of our 
	interest, the non-usual residents who are excluded is 376 individuals (or 
	1.62% of the subsample that will be retained).  
	*/
...
********************************************************************************
	...
	/*The Togo DHS 2013-14 report indicate that one in two
	households in the survey was selected for a male survey. In the households 
	selected for the male survey, all eligible women age 15-49 and children 
	under age 5 were measured for their height and weight. Therefore, we use 
	the variable hv027 to identify households selected for male interview which 
	then allows for the identification of the anthropometric subsample */

gen subsample = 1 if hv027==1
...
*****************************************
	...
	data. Do note that in the case of Togo, the hemoglobin test includes men 
	while the anthropometric subsample does not include men. */
...
//Subnational region
	/*NOTE: Togo DH 2013-14: the survey has produced representative results for 
	the whole country, the urban and rural areas separately, for each of the 5
	administrative regions and for the municipality of Lomé. For the needs of 
	the analysis, the municipality of Lomé and the Maritime region were modified 
	to create the Greater Agglomeration of Lomé consisting in the municipality 
	of Lomé and the urban part of the Gulf Prefecture. This way, in the present 
	report, results from the survey are reported for the whole country, for the 
	urban and rural areas separately, for the Great Agglomeration of Lomé, for 
	the Maritime region without the Great Agglomeration of Lomé, and for each of 
	the other 4 administrative regions (p.333) */
codebook hv024 shregion, tab (100) 	
clonevar region = shregion
codebook region, tab (99)
label define region_lab 1 "Lomé" ///
						2 "Maritime (ex. Lomé)" ///
						3 "Plateaux" 4 "Centrale" ///
						5 "Kara" 6 "Savanes"
...
gen	child_schoolage = (age>=6 & age<=14)
	/*
	Note: In Togo, the official school entrance age for primary school is 6 
...
tab hh_child_atten if subsample==1, miss
...
********************************************************************************
	//Note: Togo DHS 2013-14 has no anthropometric data for adult men
...
replace hh_no_low_bmi = 1 if no_fem_eligible==1 & subsample==1
	/*Under this section, replace households that are part of the subsample but 
	do not have the applicable population, that is, women 15-49 years, as 
...
replace low_bmi_byage = 0 if low_bmiage==0 & age_month!=.
	...
	information for women and BMI-for-age for teenagers. Deactivate this line 
	and activate the following command line when there is BMI information for 
	women and men, as well as BMI-for-age for teenagers */
...
replace hh_no_low_bmiage = 1 if no_fem_eligible==1 & subsample==1
	/*Replace households that are part of the subsample but do not have the 
	applicable population, that is, women 15-49 years, as non-deprived in 
	nutrition */
...
replace hh_no_underweight = 1 if no_child_eligible==1 & subsample==1
	/*Replace households that are part of the subsample but do not have the 
	applicable population, that is, children 0-5 as non-deprived in nutrition*/ 
...
replace hh_no_stunting = 1 if no_child_eligible==1 & subsample==1
	/*Replace households that are part of the subsample but do not have the 
	applicable population, that is, children 0-5 as non-deprived in nutrition*/ 
...
replace hh_no_uw_st = 1 if no_child_eligible==1 & subsample==1
	/*Replace households that are part of the subsample but do not have the 
	applicable population, that is, children 0-5 as non-deprived in nutrition*/ 
...
replace hh_nutrition_uw_st = 1 if no_child_fem_eligible==1 & subsample==1 
 	/*Replace households that do not have the applicable population, that is, 
...
replace water_mdg = 0 if water==32 | water==42 | water==43 | ///
						 water==61 | water==62 | water==96 | water==72 
	/*Deprived if it is "unprotected well", "unprotected spring", "tanker truck"
	  "surface water (river/lake, etc)", "cart with small tank","other" 
       water in plastic bag (code 72), following report */
...
save "$path_out/tgo_dhs13-14.dta", replace 
