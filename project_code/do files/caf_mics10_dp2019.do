...
2019 Global Multidimensional Poverty Index - Central African Republic MICS 2010 
...
*** Working Folder Path ***
global path_in G:/My Drive/Work/GitHub/MPI//project_data/DHS MICS data files/Central_African_Republic_2010_MICS_Datasets
...
/*Central African Republic: Anthropometric data was collected only for childen 
under 5 years. */
...
gen str30 datalab = "children_nutri_caf" 
...
use "$path_out/children_nutri_caf_z_rc.dta", clear 
...
erase "$path_out/children_nutri_caf_z_rc.xls"
erase "$path_out/children_nutri_caf_prev_rc.xls"
erase "$path_out/children_nutri_caf_z_rc.dta"
...
********************************************************************************
	...
	/* Note: There is no birth history data file for Central African Republic 
	MICS 2010. Hence this section has been deactivated */	
...
tab cm1 cm8, miss
	...
	child mortality. But in CAF MICS 2010, 17 women who had never had a child 
	but who had a child who died. These mismatch will be recoded later in the 
	dofile */
...
********************************************************************************
	...
	/*In the context of Central African Republic MICS 2010, there is male data. 
	However, men were not asked about child mortality which is the key
	variable extracted from this section. In addition, there is no information 
	on religion, ethnicity or insurance. Hence this section is deactivated.*/
...
*** Merging BR Recode 
*****************************************
 	//There is no birth history file for Central African Republic MICS 2010
...
*****************************************
	/*Since there is no information on child mortality from men, this variable 
...
***********************************************
	...
	data is only collected from children. However, we continue to 
	generate this variable in this do-file so as to be consistent.
	Since we don't have the required information from men in this dataset, this 
...
//Subnational region
	/*It should be noted that hh7 has 17 unique values, representing the 17 
	prefectures in the country. However, due to security concerns, the number of 
	surveys carried out in the prefecture of Vakaga (13) is very low. As such 
	the results are NOT representative for this prefecture. Indeed, tables in 
	the report present figures for 16 prefectures, excluding results for 
	Vakaga 
	In addition, the labels were redone following the report. */
codebook hh7, tab (99)
clonevar region = hh7
label define lab_reg 1"Ombella-M'Poko" ///
2"Lobaye" ///
3"Mambéré-Kadéï" ///
4"Nana-Mambéré" ///
5"Sangha-Mbaéré" ///
...
1) Six years of compulsory primary school (ages 6-11)
2) Four years of compulsory F2 (Secondary) (ages 12-15)
3) Three years of Upper Secondary (16-18)
4) Higher education, where length depends on the branch and subject.
Reference: 
http://www.ibe.unesco.org/fileadmin/user_upload/Publications/WDE/2010/pdf-versions/Central_African_Republic.pdf*/

tab ed4b ed4a, miss
tab age ed6a if ed5==1, miss
	/*In the case of CAF MICS 2010, there is inconsistency such as 
...
replace eduyears = 0 if edulevel<=1 & eduhighyear==.   
	...
	is unknown*/	
replace eduyears=14 if eduhighyear==21
replace eduyears=15 if eduhighyear==22
replace eduyears=16 if eduhighyear==23
replace eduyears=15 if eduhighyear==32
replace eduyears=14 if eduhighyear==41
replace eduyears=15 if eduhighyear==42
replace eduyears=16 if eduhighyear==43
replace eduyears=15 if eduhighyear==52
replace eduyears=16 if eduhighyear==53
replace eduyears=17 if eduhighyear==54
replace eduyears=18 if eduhighyear==55

replace eduyears=. if eduyears!=. & (eduhighyear==98 | eduhighyear==99)
...
gen	child_schoolage = (age>=6 & age<=14)
	/*Note: In Central African Republic, the official school entrance age is 
...
********************************************************************************
	/*NOTE: Central African Republic MICS 2010: No information on child 
	mortality from men */
...
drop temp_f
	...
	/* In the case of Central African Republic, this variable takes missing 
	value because the survey did not collect information on child mortality 
	from men */	
...
************************************************************************
	/*In the case of Central African Republic, there is no birth history data. 
	This means, there is no information on the date of death of children who 
	have died. As such we are not able to construct the indicator on child 
	mortality that occured in the last 5 years. Instead, we identify individuals 
	as deprived if any children died in the household */
...
********************************************************************
/* Note: CAF MICS 2010: According to the survey report (p.147) latrine hole with 
slab is considered non-deprived, as is flush to somewhere else or do not know.
As such for the purpose of the global MPI, we have followed the report. */


gen	toilet_mdg = ((toilet<23 | toilet==24 | toilet==31) & shared_toilet!=1) 
	/*Household is assigned a value of '1' if it uses improved sanitation and 
	does not share toilet with other households  */

replace toilet_mdg = 0 if (toilet<23 | toilet==24 | toilet==31) & shared_toilet==1 
...
home, roundtrip */
********************************************************************
/* Note: In CAF MICS 2010 survey report (p. 140) the category missing is 
considered non-improved. We have followed the country report */

gen	water_mdg = 1 if water==11 | water==12 | water==13 | water==14 |  ///
					 water==21 | water==31 | water==41 | water==51 |  ///
					 water==91 
					 
replace water_mdg = 0 if water==22 | water==23 | water==32 | water==42 | ///
						 water==61 | water==71 | water==81 | water==96 | ///
						 water==99
...
replace water_mdg = 0 if water==91 & ndwater==32 
...
*** Rename key global MPI indicators for estimation ***
	/* Note: In the case of CAF MICS 2010, there is no birth history file. 
