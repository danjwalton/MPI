...
2019 Global Multidimensional Poverty Index - Comoros DHS-MICS 2012
...
/*In Comoros, anthropometric measures are available for women (15-49) 
and children under 5 in all the sampled households (p.6). Anthropometric data 
These data are not available for men, but there is a male questionnaire (not
including anthropometry, which was asked of a subsample of 1/2 of eligible 
men */
...
use "$path_in/KMKR61FL.DTA", clear 
...
drop if b5==0 
	//Children who are not alive are excluded. 127 observations were dropped.
...
gen str30 datalab = "children_nutri_com" 
...
use "$path_out/children_nutri_com_z_rc.dta", clear 
...
erase "$path_out/children_nutri_com_z_rc.xls"
erase "$path_out/children_nutri_com_prev_rc.xls"
erase "$path_out/children_nutri_com_z_rc.dta"


	//Save a temp file for merging with PR:
save "$path_out/COM12_KR.dta", replace
...
compare tot_child_died tot_child_died_2
	//Comoros DHS-MICS 2012: these figures are identical
...
sort ind_id
save "$path_out/COM12_BR.dta", replace	
...
sort  ind_id
save "$path_out/COM12_IR.dta", replace
...
count if v012>=15 & v012<=19
	//Total number of girls in the IR recode: 1,295 girls	
...
gen str30 datalab = "girl_nutri_com" 
lab var datalab "Working file"
...
use "$path_out/girl_nutri_com_z.dta", clear 
...
sort ind_id
save "$path_out/COM12_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_com_z.xls"
erase "$path_out/girl_nutri_com_prev.xls"
erase "$path_out/girl_nutri_com_z.dta"
...
sort ind_id
save "$path_out/COM12_MR.dta", replace
...
********************************************************************************
/*Note: In the case of Comoros, anthropometric data was not collected 
for men */
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/COM12_BR.dta"

drop _merge

erase "$path_out/COM12_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/COM12_IR.dta"
...
erase "$path_out/COM12_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/COM12_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss 
	/*Note: In Comoros 2012, 18 girls 15-19 were identified as measured 
...
erase "$path_out/COM12_IR_girls.dta"


*** Merging MR Recode 
*****************************************
	//No anthropometric data for men in Comoros DHS-MICS 2012
merge 1:1 ind_id using "$path_out/COM12_MR.dta"

tab men_MR hv118, miss col

drop _merge

erase "$path_out/COM12_MR.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/COM12_KR.dta"
...
tab resident, miss
	...
	In the context of Comoros DHS-MICS 2012, 305 (1.24%) individuals who were 
...
/*
For Comoros DHS-MICS 2012, height and weight measurements were
collected from children (0-5) and women (15-49) living in all of the sampled 
households. Hence there is no subsample selection for anthropometric measures.
*/
...
//Subnational region
	/*NOTE: The sample for the Comoros DHS-MICS 2012 was designed to provide 
	estimates of key indicators for the country as a whole, for urban and rural 
	areas separately, and for each of the three main islands of the country 
    (Mwali, Ndzuwani, Ngazidja) (p.6). */ 
codebook hv024, tab (99)
clonevar region = hv024
label define lab_reg 1"Mwali (Mohéli)" 2"Ndzuwani (Ndzouani or Anjouan)" ///
					 3"Ngazidja (Grande Comore)" 
...
gen	child_schoolage = (age>=6 & age<=14)
	/*Note: In Comoros, the official school entrance age is 6 years.  
...
********************************************************************************
/*Note: Comoros DHS-MICS 2012 has no anthropometric data for men, 15-54 years */
...
*** BMI Indicator for Men 15-54/15-59 years ***
******************************************************************* 
...
home, roundtrip */
********************************************************************
	/* Note: Comoros DHS-MICS 2012: rainwater is considered unimproved, 
	according to the survey report (p.10) */
...
replace water_mdg = 0 if water==32 | water==42 | water==43 | ///
						 water==51 | water==61 | water==62 | water==96 
...
gen computer = . 
	//No computer variable in Comoros
...
char _dta[cty] "Comoros"
char _dta[ccty] "COM"
char _dta[year] "2012" 	
char _dta[survey] "DHS-MICS"
char _dta[ccnum] "174"
...
save "$path_out/com_dhs-mics12.dta", replace 
