...
2019 Global Multidimensional Poverty Index - Gabon DHS 2012 [STATA do-file]. 
...
/*Gabon DHS 2012: Anthropometric information were collected from a subsample of 
2/3 of households for eligible women 15-49 and children 0-5 (p.6). 
...
use "$path_in/GAKR60FL.DTA", clear 
...
drop if b5==0 
	...
	//Gabon DHS 2012: 320 observations deleted
...
gen str30 datalab = "children_nutri_gab" 
...
use "$path_out/children_nutri_gab_z_rc.dta", clear 
...
tab wasting, miss
		...
		/* Note: In the context of GAB DHS 2012, 78 children are
		replaced as '.' because they have extreme z-scores which
...
sort ind_id
save "$path_out/GAB12_KR.dta", replace


	//Erase files from folder:
erase "$path_out/children_nutri_gab_z_rc.xls"
erase "$path_out/children_nutri_gab_prev_rc.xls"
erase "$path_out/children_nutri_gab_z_rc.dta"
...
use "$path_in/GABR60FL.DTA", clear
...
compare tot_child_died tot_child_died_2
	//Gabon 2012: these figures are identical
...
sort ind_id
save "$path_out/GAB12_BR.dta", replace	
...
use "$path_in/GAIR60FL.DTA", clear
...
sort  ind_id
save "$path_out/GAB12_IR.dta", replace
...
count if v012>=15 & v012<=19
	//Total number of girls in the IR recode: 1834	
...
gen str30 datalab = "girl_nutri_gab" 
lab var datalab "Working file"
...
/*We now turn to using the dta file that was created and that contains 
the calculated z-scores */
use "$path_out/girl_nutri_gab_z.dta", clear 
...
sort ind_id
save "$path_out/GAB12_IR_girls.dta", replace


	//Erase files from folder:
erase "$path_out/girl_nutri_gab_z.xls"
erase "$path_out/girl_nutri_gab_prev.xls"
erase "$path_out/girl_nutri_gab_z.dta"
...
use "$path_in/GAMR60FL.DTA", clear 
...
sort ind_id
save "$path_out/GAB12_MR.dta", replace
...
********************************************************************************
/*Note: In the case of GABON 2012, no anthropometric data was collected 
...
use "$path_in/GAPR60FL.DTA", clear
...
*** Merging BR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GAB12_BR.dta"

drop _merge

erase "$path_out/GAB12_BR.dta"


*** Merging IR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GAB12_IR.dta"
...
erase "$path_out/GAB12_IR.dta"


*** Merging IR Recode: 15-19 years girls 
*****************************************
merge 1:1 ind_id using "$path_out/GAB12_IR_girls.dta"
...
tab ha40 if ha65==1 & ha13 ==0 & hv117==1 & teen_IR ==. & (hv105>=15 & hv105<=19), miss 
	/*Note: In Gabon DHS 2012, 31 girls 15-19 were identified as measured 
...
erase "$path_out/GAB12_IR_girls.dta"


*** Merging MR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GAB12_MR.dta"
...
tab hb13 hb65 if hv118==1 & men_MR ==., miss 
	//No anthropometric data for men in Gabon DHS 2012

drop _merge

erase "$path_out/GAB12_MR.dta"


*** Merging KR Recode 
*****************************************
merge 1:1 ind_id using "$path_out/GAB12_KR.dta"
...
tab resident, miss 
	...
	In the context of Gabon DHS 2012, 1,066 (2.5%) individuals who were 
...
In the context of Gabon DHS 2012, height and weight measurements were
collected from children (0-5) and women (15-49) living in 2/3 of the sampled 
households. 
*/
...
//Subnational region
	/*NOTE: The sample for Gabon DHS 2012 was designed to provide 
	estimates of key indicators for the country as a whole, for urban and rural 
	areas separately, and for each of the 9 provinces and the grouped cities of 
	Libreville and Port-Gentil (p.6). */ 

codebook hv024, tab (100)	
clonevar region = hv024
lab var region "Region for subnational decomposition"
label define region_lab 1 "Libreville/Port-Gentil" ///
						2 "Estuaire" ///
						3 "Haut-Ogooué" ///
						4 "Moyen-Ogooué" ///
						5 "Ngounié" ///
...
gen	child_schoolage = (age>=6 & age<=14)
	/*Note: In Gabon, the official school entrance age is 6 years.  
...
********************************************************************************
/*Note: Gabon DHS 2012 has no anthropometric data for men, 15-54 years */
...
foreach var in ha40 hb40 {
			 ...
			 //Check if it is restricted to only an age group or full sample> Gabon, it is restricted to females aged 15-49
			 drop inf_`var'
			 }
***
...
clonevar ndwater = hv202  
	//Gabon DHS 2012: hv202 present, but all missing values 
...
home, roundtrip */
********************************************************************
gen	water_mdg = 1 if water==11 | water==12 | water==13 | water==31 | ///
					 water==32 | water==51 | water==71  
	...
	/* According to the Report, spring water is considered a non-improved source 
	   of drinking water. (p. 14)*/
	
replace water_mdg = 0 if water==33 | water==34 | water==43 | ///
						 water==61 | water==96 | water==41  
...
gen	wall_imp = 1 
replace wall_imp = 0 if wall<=32 | wall==96  
...
clonevar animal_cart = hv243c	
	//In Gabon DHS 2012, hv243c exists but is all missing values
...
save "$path_out/gab_dhs12.dta", replace 
